...
import { closeSync, existsSync, fsyncSync, mkdirSync, openSync, copyFileSync, readFileSync, rmSync, readdirSync, lstatSync, realpathSync, renameSync, writeFileSync } from "node:fs";
...
export function isConfinedUpdatePath(root, path, { allowRoot = false } = {}) {
  const base = realpathSync(root), candidate = resolve(path), rel = relative(base, candidate);
  if (!(allowRoot || rel.length > 0) || rel === ".." || rel.startsWith(`..${sep}`) || isAbsolute(rel)) return false;
  let existing = candidate;
  while (!existsSync(existing)) { const parent = dirname(existing); if (parent === existing) return false; existing = parent; }
  if (lstatSync(existing).isSymbolicLink()) return false;
  const physicalRel = relative(base, realpathSync(existing));
  return !physicalRel || (!physicalRel.startsWith(`..${sep}`) && physicalRel !== ".." && !isAbsolute(physicalRel));
}
