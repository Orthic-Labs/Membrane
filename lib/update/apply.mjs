...
export function isConfinedUpdatePath(root, path, { allowRoot = false } = {}) {
  const base = realpathSync(root), candidate = resolve(path);
  ...
  const physicalCandidate = resolve(realpathSync(existing), relative(existing, candidate));
  const physicalRel = relative(base, physicalCandidate);
  return (allowRoot || physicalRel.length > 0)
    && physicalRel !== ".."
