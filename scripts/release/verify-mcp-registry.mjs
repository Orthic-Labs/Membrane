...
export async function verifyMcpRegistry({ directory = root, requirePublished = true } = {}) {
  const base = nativeDirectory(directory);
  const [server, npm] = await Promise.all([json(resolve(base, "server.json")), json(resolve(base, "dist/npm/package.json"))]);
  ...
  if (Object.keys(server.nativeArtifacts ?? {}).sort().join(",") !== platforms.join(",")) fail("native artifact map must contain only current Windows targets");
  ...
  for (const platform of platforms) {
    ...
    if (requirePublished) {
      ...
      if (!safeId(artifact.platformReceipt)) fail("published Windows platform receipt missing");
    }
  }
  ...
  if (server.signatureRequirements?.windows?.platform !== "authenticode" || server.signatureRequirements.windows.target !== "windows-x86_64" || Object.keys(server.signatureRequirements ?? {}).sort().join(",") !== "artifact,windows") fail("Windows platform signature requirements missing or out-of-scope requirements present");
