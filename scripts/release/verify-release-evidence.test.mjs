...
import { validateReceipt } from "./verify-platform-artifacts.mjs";
...
const fixture = () => {
  ...
  const platform = (os, suffix) => {
    const base = { schema: "membrane.platform-acceptance.v1", receiptId: `${os}-1`, mode: "installed-local", commit: "a".repeat(40), releaseGeneration: "c".repeat(64), version: "v1.2.3", platform: os, artifact: { name: `${os}.artifact`, sha256: "d".repeat(64) }, trust: { authenticode: "pass", timestamp: "pass", publisher: "Adrian_Dsouza" }, lifecycle: { install: "pass", startup: "pass", update: "pass", uninstall: "pass" }, environment: { host: "windows-laptop", bypassWarnings: false } };
  ...
  return { root, manifest: { schema: "membrane.release-evidence.v1", release: { tag: "v1.2.3", commit: "a".repeat(40), tree: "b".repeat(40), generation: "c".repeat(64), target: "windows-x86_64", artifact_sha256: artifact.sha256, vector_dispatch: "CORTEX_VECTOR_DISPATCH_V2" }, artifact, sbom: file("sbom"), provenance: file("provenance"), toolchain: file("toolchain"), tests: [file("test")], signatures: [trust("ed25519")], platform_trust: trust("authenticode"), compatibility: file("compatibility"), install_receipts: [platform("windows", "windows")], event_history: { status: "sealed", receipt: file("history") } } };
};
...
test("non-installed modes cannot become release evidence", () => {
  const { root, manifest } = fixture();
  const item = manifest.install_receipts[0];
  const receipt = JSON.parse(readFileSync(join(root, item.receipt.path), "utf8"));
  receipt.mode = "source-ready";
  writeFileSync(join(root, item.receipt.path), JSON.stringify(receipt));
  item.receipt.sha256 = sha(JSON.stringify(receipt));
  assert.throws(() => validateReceipt(receipt), /mode invalid/);
  assert.throws(() => verifyReleaseEvidence(manifest, root), /mode invalid/);
});
