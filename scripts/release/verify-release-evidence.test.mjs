...
const fixture = () => {
  ...
  const platform = (os, suffix) => {
    const base = { schema: "membrane.platform-acceptance.v1", receiptId: `${os}-1`, mode: "clean-vm", commit: "a".repeat(40), releaseGeneration: "c".repeat(64), version: "v1.2.3", platform: os, artifact: { name: `${os}.artifact`, sha256: "d".repeat(64) }, trust: { codesign: "pass", notarization: "pass", staple: "pass", gatekeeper: "pass" }, lifecycle: { install: "pass", startup: "pass", update: "pass", uninstall: "pass" }, environment: { clean: true, machineDigest: "e".repeat(64), bypassWarnings: false } };
...
test("modified receipts and missing Mac installed proof fail closed", () => { const { root, manifest } = fixture(); manifest.sbom.sha256 = "0".repeat(64); assert.throws(() => verifyReleaseEvidence(manifest, root), /sbom hash mismatch/); const second = fixture(); second.manifest.install_receipts.pop(); assert.throws(() => verifyReleaseEvidence(second.manifest, second.root), /installed macOS receipt/); });
