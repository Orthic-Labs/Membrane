...
const TARGETS = new Set(["mac-arm64", "mac-x64", "windows-x64", "windows-arm64"]);
...
export function verifyReleaseEvidence(manifest, root = process.cwd()) {
  ...
  for (const trust of trusts) {
    if (!trust || !["ed25519", "apple-notary", "windows-authenticode"].includes(trust.kind) || !field(trust, "identity")) fail("untrusted signing identity");
  ...
  const expectedPlatform = release.target.startsWith("mac-") ? "apple-notary" : "windows-authenticode";
  if (manifest.platform_trust?.kind !== expectedPlatform) fail(`${release.target} requires ${expectedPlatform} receipt`);
  ...
  if (!installed.has("macos") || !installed.has("windows")) fail("installed macOS and Windows receipts required");
