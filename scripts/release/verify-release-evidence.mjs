...
export function verifyReleaseEvidence(manifest, root = process.cwd()) {
  ...
  for (const trust of trusts) {
    if (!trust || !["ed25519", "authenticode"].includes(trust.kind) || !field(trust, "identity")) fail("untrusted signing identity");
  ...
  if (manifest.platform_trust?.kind !== "authenticode") fail(`${release.target} requires Authenticode receipt`);
  ...
  for (const item of manifest.install_receipts) {
    if (!item || item.os !== "windows" || item.vector_dispatch !== "CORTEX_VECTOR_DISPATCH_V2") fail("invalid installed Windows vector-dispatch receipt");
  ...
  if (!installed.has("windows") || installed.size !== 1) fail("one installed Windows receipt required");
