...
| Path | Consumed by |
| --- | --- |
| `evidence/qualification/` | `tools/productization/generate-support-matrix.mjs`, which derives `docs/support-matrix.md`, `docs/support-matrix.json`, the README support-tier block, and `server.json` platformReceipt fields from these receipts. |
| `evidence/releases/` | `scripts/release/npm/resolve-platform-artifacts.mjs`, `scripts/release/oci/generate-oci-release.mjs`, and their tests. |
| `evidence/platform/` | Referenced from `docs/release/platform-acceptance.md`. |
| `evidence/g3/` | Kept as current-generation qualification evidence. |
...
`evidence/g2/` (391 files) and `evidence/productization/` (257 files) were
...
`evidence/productization/` are gitignored (see `.gitignore`) so this history
