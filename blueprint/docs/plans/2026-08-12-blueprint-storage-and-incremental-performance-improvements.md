# Blueprint best-of-market execution plan
...
Repository: canonical `/Volumes/D/claude/blueprint` only; `blueprint2` is forbidden.
...
|---|---:|---|---|---|
| CX-R001–R004 | Blueprint integration owner / P2 | 0 → 1 | Existing query/doctor schemas stay readable while U0–U5 fields are additive | Coverage + truthfulness cells; P1 disposition + P2 lattice receipts |
| CX-R005–R008 | Blueprint integration owner / P3 | 0 → 2 | Old registry path remains authoritative until one pipeline has consumer parity | Truthfulness + compatibility cells; P3 end-to-end receipt |
| CX-R009–R012 | Blueprint graph owner / P4 | 0 → 3 | Last valid generation remains queryable; no new snapshot schema adopted early | Correctness + snapshot + recovery cells; P4 churn/equivalence receipt |
| CX-R013–R016 | Membrane Hub owner + Blueprint service owner / P5 | 0; Membrane P1 protocol release | Existing lifecycle remains installed until Hub protocol compatibility is proven | Lifecycle + portability cells; joint installed receipt |
| CX-R017–R020 | Blueprint storage/query owner / P10 | 0 → 4 → 6 | Exact API & stored schema stay compatible; migration is rebuildable/backout-safe | Query + storage + recovery cells; P6/P10 receipts |
| CX-R021–R024 | Blueprint provider owner / P8 | 0 → 2 → 3 → 7 | New providers are disabled/unadvertised until positive production qualification | Coverage + truthfulness + compatibility cells; P7/P8 receipts |
| CX-R025–R028 | Blueprint retrieval owner / P9 | 0 → 3 → 8 | Exact/structural order remains default; semantic/policy additions stay additive or disabled | Retrieval + correctness + privacy cells; P8/P9 receipts |
| CX-R029–R032 | Blueprint integration owner / P10 | 0 → 4 → 6 | No export/schema becomes default before recovery & old-reader behavior pass | Compatibility + recovery + delivery cells; P10 receipt |
| CX-R033–R036 | Blueprint release owner / P10 | 0 then all owners | No performance/market/delivery claim from partial packages | Evidence + comparative + portability + delivery cells; final release receipt |
...
- Freeze `benchmark-protocol.v1.json`, `blueprint-competitors.v1.json`, atomic `CX-H` closure, public-surface inventory, supported-version window, & exact canonical plan digests. Every later receipt names those digests.
...
- Rank with visible exact, lexical, semantic, graph centrality, change relevance, ownership/history, diversity, & source freshness evidence. Blueprint never assigns packet authority or authorization.
...
- Release sequence is strict: Membrane Hub protocol commit/release + compatibility receipt → Blueprint child adoption → installed joint Mac/Windows process-census/fault receipt with matching protocol/artifact hashes → Blueprint commit/push → parent pin/push.
- Commit/push Blueprint; then pin/push parent gitlink; verify remote SHAs.
...
- No source mutation/codemod surface in Blueprint core.
