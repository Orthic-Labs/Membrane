...
function allowedTarget(root, path) {
  ...
  const files = ["CLAUDE.md", "AGENTS.md", "BLUEPRINT-AGENT.md", ".mcp.json", join(".cursor", "rules", "blueprint.mdc"), join(".claude", "settings.json")];
...
function installMcpEntry(state, root) {
  ...
  mergeJsonFile(root, state, path, (value) => ({
    ...
    mcpServers: {
      ...(value.mcpServers ?? {}),
      blueprint: { command: process.execPath, args: [SERVER_SCRIPT] },
    },
  }));
}
...
function grantCheck(root) {
  ...
  process.stdout.write(JSON.stringify({ hookSpecificOutput: { hookEventName: "PreToolUse", permissionDecision: allowed ? "allow" : "deny", permissionDecisionReason: allowed ? "Blueprint task-scoped grant present." : "Run blueprint_expand for this path to issue a widened Blueprint grant." } }));
  return 0;
}
