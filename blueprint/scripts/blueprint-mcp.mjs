...
// root is bound at process start (--root, --repo-id, BLUEPRINT_REPO_ROOTS); tool
// inputs never accept an unrestricted repoRoot. Exactly six frozen tools, nine
// URI-addressable resources (eight static plus blueprint://effects), and six
...
function parseRootConfig() {
  ...
  const envRoots = String(process.env.BLUEPRINT_REPO_ROOTS ?? "").split(":").filter(Boolean);
...
function buildService({ root, repoId, envRoots = [] } = {}) {
  ...
  if (!entries.length) {
    throw new Error("blueprint-mcp requires --root, --repo-id, or BLUEPRINT_REPO_ROOTS");
  }
  const registry = new RootRegistry(entries);
  return createBlueprintApplicationService({ outDir: OUT_DIR, rootRegistry: registry, allowEmbeddedRoot: false });
}
    ...
    call: (service) => async (input) => {
      ...
      if (input.generation !== undefined) {
        ...
        if (observed !== input.generation) {
          throw new BlueprintError("generation_mismatch", "Requested generation is not current.", { expected: input.generation, observed });
        }
      }
...
function promptMessages(prompt) {
  ...
  return {
    messages: [{
      role: "user",
      content: { type: "text", text: `Work through "${prompt.name}" using these Blueprint tools:\n${lines.join("\n")}` },
    }],
  };
}
...
export function createBlueprintMcpServer(options = {}) {
  ...
  for (const uri of RESOURCE_URIS) {
    server.resource(uri, uri, { mimeType: "application/json", description: `Blueprint resource: ${uri}` }, async () => ({
      contents: [{ uri, mimeType: "application/json", text: JSON.stringify(redactForEgress(resourceForUri(uri, { service })), null, 2) }],
    }));
  }
  server.resource("blueprint://effects", "blueprint://effects", { mimeType: "application/json", description: "Declared effect profile per MCP tool" }, async () => ({
    contents: [{ uri: "blueprint://effects", mimeType: "application/json", text: JSON.stringify(TOOL_EFFECTS, null, 2) }],
  }));

  for (const prompt of PROMPTS) {
    if (!promptByName(prompt.name)) throw new Error(`blueprint prompt registry is missing ${prompt.name}`);
...
// Programmatic entrypoint used by `blueprint mcp serve` (scripts/cli/commands.mjs)
// and by embedders: bind the root at start, then serve over stdio.
export async function startBlueprintMcpServer({ root, repoId } = {}) {
  const server = createBlueprintMcpServer({ root, repoId });
...
if (process.argv[1] && realpathSync(resolve(process.argv[1])) === realpathSync(fileURLToPath(import.meta.url))) {
  ...
  const server = createBlueprintMcpServer({ root, repoId, envRoots });
