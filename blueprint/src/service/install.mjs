...
// The sole headless entry point is `blueprint service run` (foreground).
...
export function installService({ root = process.cwd(), logDir = null, dryRun = false } = {}) {
  const logs = logDir ?? join(homedir(), ".blueprint", "logs");
...
export function serviceControlPlan(action) {
  throw Object.assign(new Error(`OS service control forbidden per D-S03 — use blueprint service run (requested ${action})`), { code: "os_registration_forbidden" });
}
