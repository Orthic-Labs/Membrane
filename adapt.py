"""adapt — Orthic Morph Taste entry (backward-compatible facade).

Implementation is split along product boundaries:
  - ``taste``       durable preferences → MemRight (apply_actions, add_rule, …)
  ...
  - ``doctor``      multiwriter conformance (Blueprint/Beacon = not-yet)
  - ``workspace_runtime`` parent MemRight/session/mirror import boundary

CLI entry points:
  python3 adapt.py …          # legacy Taste flags
  python3 adapt.py doctor …   # Morph Doctor
...
# --- re-exports (tests + callers keep `import adapt`) ---
RULES_FILE = taste.RULES_FILE
DIGEST_FILE = taste.DIGEST_FILE
WORKSPACE_ROOT = taste.WORKSPACE_ROOT
MEMRIGHT_MUTATION_TIMEOUT_SECONDS = taste.MEMRIGHT_MUTATION_TIMEOUT_SECONDS
...
_run_memright = taste._run_memright
