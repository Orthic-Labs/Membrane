...
|---|---|---|---|---|
| 1 | **Feedback rail** — per-candidate recall self-learning; `get`→used, delete/supersede→contradicted; verified `contradicted` = veto-until-superseded (sha-aware); shared `recall_scored` and live `/recall` both apply it; persisted `context_feedback` (schema v7); `metrics.feedback`. *Honesty note (Sol audit 2026-07-16): wired and test-validated, but `context_feedback` holds 0 production rows — the loop has never fired on real traffic; treat as LIVE-but-unexercised.* | LIVE (unexercised) | `memright-core/effectiveness.rs`, `memright/feedback.rs`, `store.rs`, `serve.rs` (`/feedback`), `main.rs` (`feedback` verb) | [plan](plans/2026-07-15-rightcontext-feedback-rail.md) |
