...
Native implementation in `engine/crates/membrane-runtime/src/provenance.rs`. If the runtime
...
| `scopeGrant`         | Caller-supplied scope-grant value that authorised the read; may be a token or digest. |
...
- **Credentials, secrets, or tokens unrelated to a scope grant.** The adapter
  does not discover, redact, or synthesise credentials. `scopeGrant` preserves
  a caller-supplied token or digest verbatim, so the journal is sensitive local
  data when callers supply a credential-shaped grant.
...
The adapter does not enforce retention. Its current behavior is:

- Keep the journal for the lifetime of the installation.
- Receipt-bound uninstall can remove only paths it owns; it does not imply a
  blanket data-root wipe (see §6).
...
The journal is runtime data. Use receipt-bound Membrane uninstall only for
paths recorded in its ownership table:

```sh
# Removes only receipt-authorized candidates.
membrane uninstall
```

Install does **not** delete the journal; an install preserves the trail across
upgrades. The `observe` verb emits
