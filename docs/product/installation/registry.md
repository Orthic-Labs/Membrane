...
1. **Identity** — `server.name`, `server.npm.mcpName`, and
   ...
   `dist/npm/package.json`'s `name` both equal `@membrane/membrane`; `server.version`
   ...
   `npm:@membrane/membrane@<version>` route. This half is delegated to
...
`membrane_blueprint` is a real, dispatchable MCP tool with a matching
operations entry, schema/version bindings, golden fixtures, & closed error
taxonomy. Registry metadata therefore marks it
`"contractCoverage": "operations_registry"`, exactly like every other
exposed Membrane tool.
...
`tests/mcp-registry/verify-server-json.test.mjs` prove this against the real
repo & synthetic registries, so any tool addition, removal, or contract drift
must update `server.json` in the same change or these tests fail.
...
- No registry publication, signing, or credentialed package upload is performed
  here; shared Membrane release tooling owns those actions.
