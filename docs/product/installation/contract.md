...
(loopback HTTP & bounded CLI/MCP transports) needs to know it is talking to
...
Explicit requests bind installed storage & component identity with Hub on or off.
Resident startup-generation checks apply when using Hub IPC; they do not require
starting Hub for ordinary operations. See [execution lifecycle boundary](../../architecture/execution-lifecycle-boundary.md).
