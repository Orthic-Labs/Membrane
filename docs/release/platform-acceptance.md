...
`mode: source-ready` records contract readiness only. Only `mode: clean-vm`, with a pseudonymous machine digest & explicit `bypassWarnings: false`, can pass artifact acceptance. Run `node scripts/release/verify-platform-artifacts.mjs CONTRACT.json RECEIPT.json`; retain resulting receipts under `evidence/platform/`.
