...
`release/evidence/release-evidence.v1.schema.json` defines offline, hash-bound evidence. `node scripts/release/verify-release-evidence.mjs --manifest evidence/releases/RELEASE.json` recomputes every listed file hash before accepting source, SBOM, provenance, toolchain, test, compatibility, signature, platform-trust, installed-platform & event-history receipts.
