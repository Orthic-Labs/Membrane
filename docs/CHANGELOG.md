...
- Flipped `CORTEX_VECTOR_DISPATCH_V2` to default-on: resident in-process f32 index (`retrieve_hybrid_indexed`) is now the live recall path on both hosts.
- Retained immediate environment-variable fallback: `CORTEX_VECTOR_DISPATCH_V2=0|false|off|legacy` restores legacy scalar-A `retrieve_hybrid` routing on next store open.
...
- Public surfaces use Membrane headers, telemetry registry provider, & state/telemetry titles.
...
- Absorbed RMS lifecycle v20, checkpoints, telemetry, migration/backout, doctor, native MCP/federation, installer, & typed virtual scopes into Cortex/Membrane.
...
- CodeRight now consumes `cortex` & `cortex-core` from exact Membrane revision `944ea3ad`; retired root-repository pins are removed.
