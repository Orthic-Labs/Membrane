...
adapter (Blueprint, Cortex, custom client adapters) implements and is validated
...
| `engine/crates/membrane-testkit/` | The canonical Blueprint and Cortex conformance fixture corpus, embedded as JSON and exposed via `golden_fixtures()`. |
| `docs/reference/examples/providers/blueprint_example/` | Reference Blueprint adapter built on the SDK. |
| `docs/reference/examples/providers/cortex_example/`  | Reference Cortex adapter built on the SDK. |
| `docs/reference/providers/` | This README plus per-example docs. |
...
canonical Blueprint and Cortex fixture sets as JSON and exposes them via
...
4. Add a CLI entry point that runs `run_conformance` and prints the
   ...
   `docs/reference/examples/providers/{blueprint,cortex}_example/` are the canonical
   shape.
