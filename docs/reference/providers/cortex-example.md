...
```
docs/reference/examples/providers/cortex_example/
├── Cargo.toml      # member of docs/reference/examples/providers/ workspace
├── README.md       # this file
└── src/
...
```sh
rightkit cargo run --manifest-path docs/reference/examples/providers/cortex_example/Cargo.toml --bin cortex_example
```
...
* The adapter is intentionally pure (no I/O, no clock, no randomness).
  A real Cortex adapter will write to the durable `MemoryStore`; those
...
* The proposal id and checkpoint id in the response are pinned by the
  fixtures. A real Cortex adapter would mint these from the durable
