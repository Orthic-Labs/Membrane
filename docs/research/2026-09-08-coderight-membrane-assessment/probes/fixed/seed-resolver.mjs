import { hydrateNodesByIds } from "../../../../../blueprint/src/graph/store-sqlite.mjs";
import { symbolAuthorityOrder } from "../../../../../blueprint/src/graph/symbol-authority-order.mjs";

const STOPWORDS = new Set(["a", "an", "and", "are", "as", "at", "be", "by", "for", "from", "in", "is", "it", "of", "on", "or", "that", "the", "this", "to", "with"]);

function terms(task) {
  const raw = String(task ?? "");
  const whole = raw.toLowerCase().split(/[^a-z0-9_./:-]+/i).map((t) => t.toLowerCase())
...
export function resolveSeeds(db, task, { generationId, seedIds = [], anchors = [], maxSeeds = 8, allowAmbiguousTaskSeeds = false } = {}) {
  ...
  const qualifiedRows = [];
  const byQualified = db.prepare(`SELECT id, ${symbolAuthorityOrder(db)} AS authorityRank FROM symbols WHERE generation_id = ? AND lower(qualified_name) = ? ORDER BY authorityRank,id LIMIT ?`);
  ...
  const exactRows = [];
  const byName = db.prepare(`SELECT id, ${symbolAuthorityOrder(db)} AS authorityRank FROM symbols WHERE generation_id = ? AND lower(name) = ? ORDER BY authorityRank,path,id LIMIT ?`);
