...
import { hydrateEdgesByIds, hydrateNodesByIds, traversalNeighbors } from "../../../../../blueprint/src/graph/store-sqlite.mjs";
import { semanticAuthorityForFact, semanticAuthorityRankForFact } from "../../../../../blueprint/src/graph/evidence-authority.mjs";
import { resolveSeeds } from "./seed-resolver.mjs";
import { selectTraversalPolicy } from "../../../../../blueprint/src/graph/traversal-policy.mjs";
...
export function recallCircuitToCandidateSet(circuit, options = {}) {
  ...
  for (const path of circuit.paths ?? []) {
    __rows.push(path.nodes[0]);
    __rows.push(path.nodes.at(-1));
  }
  for (const __n of __rows) {
    const path = (circuit.paths ?? []).find((p) => p.nodes.includes(__n)) ?? { nodes: [__n], evidence: [] };
    const terminal = __n;
