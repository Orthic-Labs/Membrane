# CodeRight Context & Memory Kernel (CCK)

## Research-backed architecture and implementation plan

**Research cutoff:** 24 July 2026, inclusive
**Prepared:** 25 July 2026
**Primary target:** CodeRight’s Rust/Axum daemon, with adapters for Claude Code, Codex, and other agent harnesses
...
Build a **native Context & Memory Kernel inside the CodeRight daemon**. Do not make an external MCP server, provider proxy, vector database, `CLAUDE.md`, or `AGENTS.md` file the canonical source of memory. Those should be adapters or export surfaces.
...
- **External harnesses:** capabilities declared explicitly. MCP-only integration cannot provide the same enforcement as native or proxy-level integration.

### Initial launch gates

The first generally enabled release should meet all of the following on a representative CodeRight replay suite:
...
- At least **20% median reduction in measured billed input tokens per successful task** on long sessions.
...
- Every model call has a reproducible **context manifest** explaining what was included, omitted, compressed, retrieved, and ordered.
- Every behavior-changing recommendation requires explicit human approval and a versioned rollback path.

These are design targets, not industry-standard thresholds. CodeRight should change them after collecting its own baselines.
...
The newest directly relevant source included is the 23 July 2026 *Agentic Context Management* preprint. It supports the lifecycle framing—architecting, ingesting, scoping, anticipating, compacting, and consolidating—but its reference-implementation benchmark is author-reported and is not used as proof of expected CodeRight performance.[^agentic-context-management]
...
## 3. CodeRight current-state assessment

CodeRight is unusually well positioned to build this natively. Its daemon already owns the lifecycle that a context kernel needs: sessions, event persistence, tool execution, permissions, provider calls, orchestration, usage accounting, and learning proposals.

### 3.1 Existing primitives to retain

| Existing capability | Internal evidence | Decision |
|---|---|---|
| Rust/Axum daemon is the source of truth for sessions, tools, storage, orchestration, and runtime evidence. | `README.md`; `docs/ARCHITECTURE.md` | Put CCK in the daemon, not the UI or an external MCP. |
...
| Token-budget and LLM summarizers, breaker, output reservation, memory-block reinjection. | `engine/crates/engine/src/compactor.rs` | Route through CCK policy and context manifests. |
...
The existing `engine`, `storage`, `telemetry`, `learning`, `tools`, `providers`, `cache`, `skeletonization`, and `orchestration` crates call into this kernel. Provider- and harness-specific transport remains outside the policy core.
...
flowchart TB
    subgraph Surfaces["Agent surfaces"]
        ...
        MCP["MCP / hook adapters"]
    end

    subgraph Daemon["CodeRight daemon"]
        Session["Session & orchestration runtime"]
        CCK["Context & Memory Kernel"]
        Policy["Permission / trust policy"]
        Learn["Learning & recommendation engine"]
        OTel["Telemetry / eval recorder"]
    end

    subgraph CCKLayers["CCK internals"]
...
8. **External adapters may have reduced capability, but never altered semantics.**
9. **Synchronization transports immutable operations, not database pages.**
10. **Content is absent from exported telemetry unless the user explicitly enables a bounded diagnostic capture.**
...
CodeRight’s existing authoritative compaction and archive code already establishes the correct fail-closed behavior. CCK should standardize the packet and measurement around it rather than replace the safety contract.
...
There must be **one semantic model of context and memory**, but not one visibility namespace.
...
### 12.5 Integration capability levels
...
- map Codex thread/session identifiers to CodeRight canonical sessions;
...
Use an Agent File-like portable state concept for interoperability, but keep CodeRight’s normalized schema and authority model canonical.[^agent-file] Remnic’s host-agnostic core plus thin adapter pattern is also a useful precedent for inspectable local memory.[^remnic]
...
Do not display an estimated output-token saving as measured. Headroom correctly notes that unseen counterfactual output is an estimate; CodeRight should go further and reserve “saved” for matched comparisons.
...
## 21. Repository integration plan

### 21.1 Existing file changes
...
| `engine/crates/skeletonization` | Expose AST representation service through CCK. |
...
### 21.2 Preserve existing contracts
...
CCK should centralize and extend these primitives, not regress them.
...
- Every CodeRight native model call produces a complete manifest.
- Provider totals reconcile with session totals.
- Replaying a manifest reconstructs byte-equivalent provider input where permitted.
...
### Phase 1 — Kernel skeleton and deterministic assembler

**Build**

- `context_kernel` crate.
- `ContextItem`, scopes, authority, trust, validity.
- Tiered assembler and model profiles.
- Canonical serializer and tool ordering.
- Existing transcript path adapted through the kernel without changing selection.

**Acceptance**

- Byte-equivalent or semantically equivalent prompts to baseline.
- No task-quality change.
- Cache-break explanations available.
- Property tests for scope and stable serialization.

**Rollback**

- Switch engine to legacy context builder.
...
### Phase 7 — Harness adapters

**Build**

- MCP server and adapter capability contract.
- Claude Code hook/MCP integration.
- Codex MCP/proxy integration.
...
- adapters can be individually disabled without affecting native CodeRight.
...
CCK is production-ready only when:
...
The best CodeRight system is not “Headroom plus a vector database.” It is a **daemon-owned context operating system**:
...
The immediate priority is not embeddings or a learned compressor. It is **Phase 0: context manifests, replayability, and outcome-linked telemetry**. Once every model call can be reconstructed and graded, CodeRight can determine—using its own workloads—which compression, retrieval, embedding, graph, and scheduling techniques actually reduce cost and improve performance.
...
| Evaluation | existing CodeRight benchmark/storage stack + deterministic repository graders | matched replay and regression gates |

### Embedding and reranker selection rule

Do not choose a default by reputation. Select only after a CodeRight benchmark that includes:
