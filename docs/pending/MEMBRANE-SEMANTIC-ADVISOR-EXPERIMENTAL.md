...
Everything in `MEMBRANE-PENDING-IMPLEMENTATION.md` must work with this document deleted. Nothing
here is a prerequisite for anything there. If the advisor is disabled, unavailable, times out,
...
Subordinate to the canonical doctrines in `docs/subsystems/`, and to
`MEMBRANE-PENDING-IMPLEMENTATION.md` for every deterministic mechanism it plugs into.
...
Extends the host capability table in `MEMBRANE-PENDING-IMPLEMENTATION.md` §12.1.
...
`docs/subsystems/CODERIGHT-MEMBRANE-OBSERVABILITY-LEARNING-AND-EVAL-INTEGRATION.md`. It is not
reproduced here.
