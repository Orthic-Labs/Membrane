...
|---|---|---|
| CTX-001 | `MemDb::open` is not the sole SQLite open authority: `hub_readonly_db.rs:37,60` and `cli.rs:1453` open the Cortex and event-ledger databases directly, bypassing the migration ladder, the WAL/busy-timeout pragmas and the generation check. Consolidating or explicitly sanctioning those read paths is the remaining work. | Cortex; in-repo. |
