...
During concurrent subsystem integration the branch must pass Ledger wiring checks and managed Rust validation. Native runtime tests freeze MCP discoverability, daemon ownership, provider registration, Push coexistence and qualification fence. The final authoring patch is content-hash guarded and may modify only its explicit manifest paths. Rust test binaries, application binaries, packaging, installation, activation and release qualification are intentionally not run under the current no-build constraint.
