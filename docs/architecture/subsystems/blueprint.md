...
watcher/freshness role is resident **only inside the active tray-owned daemon**. With tray
...
resident service. Tray owns its resident lifecycle; its child daemon hosts the watcher. Membrane ships
...
21. A tray-daemon-hosted query path as the primary machine-to-machine route, plus bounded one-shot operations when tray-owned daemon is inactive.
...
Tray-daemon-hosted Blueprint query role, or bounded one-shot for an explicit direct
Blueprint request while tray-owned daemon is inactive
...
## 17.1 Tray-daemon-hosted query path is primary

The primary machine-to-machine Blueprint path is Blueprint running under an
active tray-owned daemon. Blueprint MUST NOT run as an independently resident daemon, register
an OS service, or be auto-started by a client or agent.

When tray-owned daemon is inactive, an explicit user/agent request MAY run a **bounded one-shot
...
A one-shot operation MUST NOT daemonize, start a watcher, start tray or Hub, register a
...
- **Membrane context requests never fall back.** With tray-owned daemon inactive (`hub_inactive`), a Membrane
...
- **Direct Blueprint consumers use one stable client that routes for them.** A
  ...
  example — addresses one client that selects the tray-daemon path or a bounded
...
The Blueprint query role inside tray-owned daemon owns:
...
one-shot routing. It routes an enrolled root through the tray-owned daemon when active;
with the tray inactive it performs a bounded one-shot & exits. If an enrolled root
...
The daemon-hosted query role owns queries.

Watcher and query roles may share lifecycle inside the tray-owned daemon, but they remain separate
responsibilities. The watcher exists only while the tray-owned daemon is active.
...
attempted while the tray-owned daemon holds the writer role MUST route through it or fail with
...
hosted — inside the tray-owned daemon, or in a bounded one-shot operation. Removing the resident
...
CLI, MCP, SDK, tray-daemon-hosted query role, & bounded one-shot operations call it.
...
```text
Membrane
→ active tray-owned daemon
→ daemon-hosted Blueprint client
→ recall
→ RecallCircuit
```

Membrane never spawns a new Node process for normal Blueprint queries and never
uses Blueprint one-shot as a fallback. With tray-owned daemon inactive, Membrane returns typed
...
- warm tray-daemon-hosted recall p50/p95;
- cold command latency;
- tray-daemon-hosted query IPC overhead;
- bounded one-shot operation total latency, tray inactive (process start to exit);
- build time;
- incremental build time;
- edit→truthful-query latency;
- peak RSS;
- steady tray-daemon-hosted Blueprint RSS;
- DB/index size;
- CPU;
- subprocess count;
- handle count;
...
- 10-file edit;
- 100-file edit;
- warm tray-daemon-hosted recall;
...
At equal RecallCircuit semantics, tray-daemon-hosted path should materially
...
The bounded one-shot path is a tray-off access mode for explicit Blueprint
...
inside the tray-owned daemon. Its independently resident daemon lifecycle is not retained.
...
A generated `docs/architecture/runtime-truth.md` that reports no synthesized components/flows is an honest incomplete artifact, not proof that the architecture capability exists.
...
| Tray-daemon-hosted service/protocol role | `src/service/**` |
...
## Train D — tray-daemon-hosted seam + admission

Deliver:

- protocol `recall`;
- server/client support;
- persistent tray-daemon-hosted query path;
...
- tray-daemon-hosted & one-shot modes are semantically identical, & tray-daemon-hosted
...
The bullets below are architecture acceptance criteria, not completion state.
Current open/closed state is derived only in
[`../../pending/README.md`](../../pending/README.md) from subsystem canons.
...
- Daemon-hosted Blueprint is the primary machine-to-machine path while tray-owned daemon is active.
...
- Product unavailable, root-not-enrolled, graph-missing, stale, resident-owner-active, tray-daemon-inactive (`hub_inactive` compatibility token), & watcher-startup-failed states remain typed & distinct.
- One direct client/application service owns daemon routing, bounded tray-off one-shot, enrolled graph-missing singleflight initial build, & one retry; consumers do not reimplement recovery.
...
- tray-daemon-hosted read requests use generation-pinned sessions.
...
Blueprint uses a tray-daemon-hosted query role for repeated agent queries and a bounded
one-shot only for explicit direct Blueprint operations while tray-owned daemon is inactive.
