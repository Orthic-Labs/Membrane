...
Watcher enrollment initializes an absent or unsealed graph before publishing enabled configuration; initialization failure must leave enrollment unsuccessful. Resident startup admits subscriptions before queued cold reconciliation, reports pending reconciliation honestly, & never presents an uninitialized repository as healthy. Re-enrollment after explicit initialization starts its previously inactive actor. Automatic acceptance uses read-only observation so query-triggered repair cannot hide watcher failure.
