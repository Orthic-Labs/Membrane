...
**Status:** Current capability architecture; runtime topology follows the tray-daemon process decision
...
- [The Findings Lane](../pending/capabilities/blueprint/findings-lane.md);
...
Build one daemon-hosted **Membrane Live Diagnostics** runtime that combines exact Blueprint
...
tray-owned daemon lifecycle; Hub is an on-demand presentation client. Blueprint remains an independently bounded Membrane subsystem and retains
...
| External engine lifecycle | Membrane daemon / Live Diagnostics | Start, supervise, reuse, isolate, restart, and retire language services/analyzers |
...
The tray-owned daemon owns one `DiagnosticsSupervisor`. It owns qualified engine instances keyed by:
...
Live Diagnostics remains one capability inside the tray-owned daemon regardless of physical packaging. Start at
