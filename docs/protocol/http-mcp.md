...
- Binds `127.0.0.1` only (`Ipv4Addr::LOCALHOST`), the same convention as the
  existing loopback Crypt API (`membrane-runtime::serve::run`).
...
- Sources its bearer token from the same on-disk credential the loopback
  Crypt API already uses (`serve::configured_api_token`, i.e.
  `CRYPT_API_TOKEN` / `CRYPT_API_TOKEN_FILE` / the `api-token` file beside the
