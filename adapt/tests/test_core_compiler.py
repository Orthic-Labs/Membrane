...
from adapt import cli as adapt_cli
...
def test_adapt_compile_core_cli_is_an_early_non_cortex_path(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(adapt_cli.pipeline, "scanner_available", lambda: True)
    monkeypatch.setattr(adapt_cli.adapt_llm, "lane_available", lambda _lane: True)
    monkeypatch.setattr(adapt_cli.taste_runtime, "load_json",
                        lambda _path: {r["id"]: r for r in _records()})
    calls = []
    monkeypatch.setattr(
