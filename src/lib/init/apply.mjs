...
export function applyInitPlan({ root = process.cwd(), plan = null, build = true, watchConfigPath = defaultConfigPath() } = {}) {
  ...
  try {
    ...
    stateTrusted = true;
    for (const action of resolved.actions) if (action.path && (!isConfinedPath(root, action.path) || !allowedTarget(root, action.path))) throw new Error("state_invalid");
...
export function uninstallInit({ root = process.cwd(), watchConfigPath = defaultConfigPath() } = {}) {
  ...
  if (!isConfinedPath(root, path)) return { schemaVersion: 1, ok: false, action: "uninstall_failed", root, restored: [], error: "state_invalid", idempotent: false };
