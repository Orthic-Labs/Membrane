...
def _run_taste(memright: Path, live_db: Path, db: Path, rules, cases, out: Path):
    ...
    try:
        ...
        for rule in rules:
            runner.value_ab.put_rule(memright, db, port, rule)
        return runner.replay_all(memright, db, port, cases, out / "taste-queries.jsonl")
    finally:
        runner.value_ab._stop_service(service)


def _run_existing(memright: Path, source_db: Path, db: Path, cases, out: Path):
    runner.value_ab.snapshot_live_db(source_db, db)
    port = runner.value_ab._free_port()
    service = runner.value_ab._start_service(memright, db, port)
    try:
        runner.value_ab._wait_ready(port)
        return runner.replay_all(memright, db, port, cases, out / "adapt-queries.jsonl")
    finally:
        runner.value_ab._stop_service(service)
...
def main(argv: list[str] | None = None) -> int:
    ...
    taste_ranked = _run_taste(
        args.memright_bin, args.live_db, args.out / "taste.db", taste, taste_cases, args.out
    )
    adapt_ranked = _run_existing(
        args.memright_bin, args.production_db, args.out / "adapt.db", adapt_cases, args.out
    )
