...
Arms:
  A  current MemRight retrieval only
  T  A + the real 52-rule CommandCode Taste file always loaded
  B  copied MemRight + all 57 raw usable Adapt records, retrieved normally
...
API_WORKER = ROOT / "tools/skills/coder/scripts/api-worker.py"
...
def replay_all(memright: Path, db: Path, port: int, cases: list[dict],
    ...
    output = _run_via_port(
        memright, db, port, ["replay", "--input", str(input_path), "-k", str(TOP_K)])
...
def _run_replay_db(memright: Path, live_db: Path, db: Path, cases: list[dict],
    ...
    service = value_ab._start_service(memright, db, port)
    try:
        value_ab._wait_ready(port)
        for record in records or []:
            value_ab.put_pref_record(
                memright, db, port, record["id"], record.get("scope", "D--Claude"),
                value_ab.adapt_record_envelope(record),
            )
        ranked = replay_all(memright, db, port, cases, input_path)
    finally:
...
def run_retrieval(value_set: dict, treatment: dict, cases: list[dict], out: Path,
                  live_db: Path, memright: Path, resume: bool) -> dict:
    ...
    ranked_a, integrity_a = _run_replay_db(
        memright, live_db, out / "A.db", cases, out / "inputs/A.jsonl")
    ranked_b, integrity_b = _run_replay_db(
        memright, live_db, out / "B.db", cases, out / "inputs/B.jsonl",
        treatment["records"])
    post_ok, post_count, post_msg = value_ab.integrity_check(live_db)
    if not post_ok or post_count != pre_count:
        raise RuntimeError("live MemRight DB changed during isolated experiment")
    result = {
...
def analyze(value_set: dict, cases: list[dict], retrieval: dict, actor: dict,
    ...
    lines += ["", "A=MemRight; T=A+CommandCode Taste; B=Adapt retrieved; "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--memright-bin", type=Path, default=DEFAULT_MEMRIGHT)
