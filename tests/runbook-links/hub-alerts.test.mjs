...
const map = fs.readFileSync(path.join(root, 'docs/product/troubleshooting/hub-alerts.md'), 'utf8');
const stableReasons = [
...
test('every stable Hub reason has a precise local runbook link and safe action', () => {
  for (const reason of stableReasons) {
    ...
    const target = fs.readFileSync(path.join(root, 'docs/product/troubleshooting', file), 'utf8');
...
test('operations index links canonical Hub and recovery runbooks', () => {
  const operations = fs.readFileSync(path.join(root, 'docs/product/operations/README.md'), 'utf8');
