...
/**
 * Reads the four reference artifacts from disk (relative to workspaceRoot),
 * verifies every one still matches its frozen sha256, and checks that the Mac
 * host receipt declares the pinned config/input identity.
...
export function verifyBakeoffReference({ workspaceRoot = process.cwd() } = {}) {
  ...
  for (const field of ['configSha256', 'inputDigest', 'manifestSha256', 'gitHead']) {
    if (!macReceipt[field]) return { status: 'mutated', reason: `Mac host receipt is missing ${field}`, checks };
  }
  ...
  return {
    status: 'verified',
    reason: 'bakeoff config/input/receipt and Mac host receipt are unchanged since the completed bakeoff; the bakeoff was not rerun',
    checks,
    arms,
