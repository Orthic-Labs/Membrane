...
const metadata = { sha256: artifactDigest(artifact), packageName: "@membrane/membrane-linux-x64", platform: "linux-x64", version: "0.1.0", signature: { algorithm: "ed25519", keyId: "fixture-key", value: "fixture-signature", digest: artifactDigest(artifact) } };
...
test("digest and signed metadata are checked before dispatch", async () => {
  ...
  assert.equal(result.dispatch("ok"), "@membrane/membrane-linux-x64:ok");
...
test("signature verification and package binding are mandatory", async () => {
  ...
  await assert.rejects(() => bootstrap({ artifact, metadata: { ...metadata, packageName: "@membrane/other" }, platform: "linux", arch: "x64", verifySignature: async () => true }), ArtifactVerificationError);
