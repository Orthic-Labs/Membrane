...
test("MBR-013: product truth is derived from live source (9 tools, 7 adapters)", async () => {
  const truth = await computeProductTruth();
  assert.equal(truth.schema, "orthic.product-truth.v1");
  assert.equal(truth.toolCount, 9);
  assert.equal(truth.tools.length, 9);
...
test("MBR-013: the check fails when README says six tools while source exposes nine", async () => {
  const truth = await computeProductTruth();
  ...
  assert.ok(failures.some((f) => /claims 6 tools but source exposes 9/.test(f)), failures.join("; "));
...
test("MBR-013: the check fails when a README-linked generated doc is absent", async () => {
  const truth = await computeProductTruth();
  const readme = "- **MCP server** — nine tools over stdio (`membrane_context`, `membrane_source_read`, `membrane_knowledge_propose`, `membrane_checkpoint_save`, `membrane_checkpoint_load`, `membrane_working_context`, `membrane_temporal_fact`, `membrane_scratchpad`, `membrane_feedback`). See [docs/product-truth.md](docs/product-truth.md).";
...
test("MBR-013: the check fails when the README omits the generated-doc link", async () => {
  const truth = await computeProductTruth();
  const readme = "- **MCP server** — nine tools over stdio (`membrane_context`, `membrane_source_read`, `membrane_knowledge_propose`, `membrane_checkpoint_save`, `membrane_checkpoint_load`, `membrane_working_context`, `membrane_temporal_fact`, `membrane_scratchpad`, `membrane_feedback`).";
