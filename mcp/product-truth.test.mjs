...
test("MBR-013: product truth is derived from live source (24 tools, 7 adapters)", async () => {
  const truth = await computeProductTruth();
  assert.equal(truth.schema, "membrane.product-truth.v1");
  assert.equal(truth.toolCount, 24);
  assert.equal(truth.tools.length, 24);
  assert.ok(truth.tools.includes("membrane_working_context"));
  assert.ok(truth.tools.includes("membrane_feedback"));
  assert.ok(truth.tools.includes("membrane_memory_read"));
...
test("MBR-013: the check fails when README says six tools while source exposes twenty-four", async () => {
  const truth = await computeProductTruth();
  ...
  assert.ok(failures.some((f) => /claims 6 tools but source exposes 24/.test(f)), failures.join("; "));
...
test("MBR-013: the check fails when a README-linked generated doc is absent", async () => {
  const truth = await computeProductTruth();
  const readme = "- **MCP server** — ten tools over stdio (`membrane_context`, `membrane_source_read`, `membrane_blueprint`, `membrane_knowledge_propose`, `membrane_checkpoint_save`, `membrane_checkpoint_load`, `membrane_working_context`, `membrane_temporal_fact`, `membrane_scratchpad`, `membrane_feedback`). See [docs/reference/product-truth.md](docs/reference/product-truth.md).";
