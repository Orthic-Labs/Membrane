...
test("E1 fails closed without installed event-ledger evidence", () => {
  const result = runBenchmark({ workspaceRoot, scenarioRunner: () => ({ model_tool_exercised: true }) });
...
test("E1 rejects caller booleans when persisted tool observation is absent", () => {
  ...
  db.close();
  const result = runBenchmark({ workspaceRoot, eventDbPath: dbPath, model: "test-model", hardware: "test-host", warmCold: "warm", adapters: ["codex"], scenarioRunner: (scenario) => ({ ...sources.get(scenario), model_tool_exercised: true }) });
  assert.equal(result.status, "blocked_incomplete_path");
...
test("E1 accepts ten unique traces with full ledger and archive proof", () => {
  const { dbPath, sources } = fixture();
  const result = runBenchmark({ workspaceRoot, eventDbPath: dbPath, model: "test-model", hardware: "test-host", warmCold: "warm", adapters: ["codex"], scenarioRunner: (scenario) => sources.get(scenario) });
