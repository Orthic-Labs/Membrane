...
const TOOL_DEFINITIONS = [
  ...
  { name: "membrane_ledger", description: "Navigate the resident Ledger document index, or ingest one explicitly granted bounded document snapshot. Search and diagnostics are source-scoped; final prompt admission remains Pull-owned.", inputSchema: { type: "object", required: ["repository", "caller", "operation"], additionalProperties: false, properties: { repository: { type: "string" }, caller: CALLER_SCHEMA, operation: { type: "string", enum: ["recall", "literal", "outline", "sync", "status", "activate", "erase", "ingest", "backlinks", "related", "manifests", "drift"] }, query: { type: "string", minLength: 1, maxLength: 4096 }, k: { type: "integer", minimum: 1, maximum: 32 }, path: { type: "string", minLength: 1, maxLength: 4096 }, docId: { type: "string" }, nodeId: { type: "string" }, scopeGrantId: { type: "string", minLength: 1 }, taskId: { type: "string", minLength: 1 }, sessionId: { type: "string", minLength: 1 }, expectedContentHash: { type: "string" }, sourceRef: { type: "string", minLength: 1, maxLength: 8192 }, sourceRevision: { type: "string", minLength: 1, maxLength: 8192 }, title: { type: "string", maxLength: 1024 }, format: { type: "string", minLength: 1, maxLength: 128 }, rawInput: { oneOf: [{ type: "string", maxLength: 8388608 }, { type: "array", maxItems: 8388608, items: { type: "integer", minimum: 0, maximum: 255 } }] }, maxRawBytes: { type: "integer", minimum: 1, maximum: 8388608 }, continuationCursor: { type: "string" }, maxSections: { type: "integer", minimum: 1, maximum: 256 }, limit: { type: "integer", minimum: 1, maximum: 256 }, mode: { type: "string", enum: ["legacy_scan", "shadow", "ledger_fts"] }, fromManifest: { type: "string" }, toManifest: { type: "string" }, deadlineMs: { type: "integer", minimum: 1, maximum: 30000 }, taskGrantLevel: { type: "string" } } } },
...
async function callTool(name, args, trace = {}, lifecycle) {
  ...
  if (name === "membrane_ledger") {
    ...
    switch (args.operation) {
      ...
      case "ingest":
        for (const field of ["path", "sourceRef", "format", "rawInput", "scopeGrantId", "taskId", "sessionId"]) {
          if (args[field] === undefined || args[field] === null || (typeof args[field] === "string" && !args[field].trim())) throw new Error(`ledger ingest requires ${field}`);
        }
        commandArgs.push("ingest", "--repo", binding.root, "--path", String(args.path), "--source-ref", String(args.sourceRef), "--format", String(args.format), "--raw-input", typeof args.rawInput === "string" ? args.rawInput : JSON.stringify(args.rawInput), "--scope-grant-id", String(args.scopeGrantId), "--task-id", String(args.taskId), "--session-id", String(args.sessionId), "--max-raw-bytes", String(args.maxRawBytes ?? 8388608));
        if (args.title) commandArgs.push("--title", String(args.title));
        if (args.sourceRevision) commandArgs.push("--source-revision", String(args.sourceRevision));
        break;
