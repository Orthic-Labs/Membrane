...
const ingress = require('./observable-ingress.cjs');
const { appendObservableEvent, _internal } = ingress;
const { buildObservableEvent } = require('./observable-event.cjs');
const { resolveDefaultIngressTarget, resolveRuntimeConfigIdentity, resetIngressResolutionCache } = _internal;
...
test('a membrane-local-v1 runtime.json resolves the default ingress identity', () => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'membrane-runtime-'));
  const configPath = path.join(directory, 'runtime.json');
  fs.writeFileSync(configPath, JSON.stringify({ schemaVersion: 1, serviceId: 'membrane-local-v1', host: '127.0.0.1', port: 47851 }));
  const resolved = resolveDefaultIngressTarget(configPath);
  ...
  assert.equal(path.basename(configPath), 'runtime.json');
  fs.rmSync(directory, { recursive: true, force: true });
});

test('default path is used (and honored) when MEMBRANE_TELEMETRY_INGRESS is unset and the resolved db file exists', () => {
  withEnv({ MEMBRANE_TELEMETRY_INGRESS: undefined }, () => {
    ...
    withEnv({ CORTEX_DB: dbPath }, () => {
      _internal._setCachedDefaultTargetForTest({ ok: true, target: path.join(directory, 'context-telemetry-ingress.jsonl'), dbPath });
...
test('explicit MEMBRANE_TELEMETRY_INGRESS still overrides the default path, bypassing the drain-evidence gate', () => {
  ...
  withEnv({ MEMBRANE_TELEMETRY_INGRESS: target, CORTEX_DB: missingDbPath }, () => {
...
test('unresolvable config (wrong serviceId / schemaVersion / host identity) returns ok:false for each mismatch', () => {
  ...
  const cases = [
    { schemaVersion: 1, serviceId: 'not-membrane-local-v1', host: '127.0.0.1', port: 47851 },
    { schemaVersion: 2, serviceId: 'membrane-local-v1', host: '127.0.0.1', port: 47851 },
    { schemaVersion: 1, serviceId: 'membrane-local-v1', host: '0.0.0.0', port: 47851 },
  ];
