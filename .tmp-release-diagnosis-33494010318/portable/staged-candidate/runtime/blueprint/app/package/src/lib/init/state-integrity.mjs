...
function keyDir() {
  if (platform() === "win32") return join(process.env.LOCALAPPDATA ?? homedir(), "Membrane", "Blueprint", "state-keys");
  return join(process.env.XDG_STATE_HOME ?? join(homedir(), ".local", "state"), "membrane", "blueprint", "state-keys");
}
