# LLMLingua-2 ONNX Spike — `D:\Claude\cr-cortex-wip`

> Migration note: this evidence was produced in CodeRight before durable memory
> moved into Membrane. Paths below are historical; corresponding source now lives
> under `engine/crates/cortex*`.

**Branch:** `wip/cortex-context-engine`
...
`engine/crates/cortex/assets/llmlingua/` (677 MB + 2.8 MB). These are what the Rust
...
1. Replace the `compress_llmlingua_onnx` stub in `engine/crates/cortex/src/compress.rs`
...
3. Asset resolution: `CORTEX_LLMLINGUA_MODEL` / `CORTEX_LLMLINGUA_TOKENIZER`
...
`engine/crates/cortex/src/compress.rs`. Same model + tokenizer, same top-K-by-
...
embedded as `include_str!`, runs `cortex::compress::compress(text, 0.5)`,
compares the output's word set to the golden `engine/crates/cortex/tests/llmlingua_golden.json`
(pre-recorded Python llmlingua at rate=0.5). Threshold: 0.90 (the plan's gate).

Run command:
```bash
$env:ORT_DYLIB_PATH = "...onnxruntime.dll"
cargo test -p cortex --features llmlingua-onnx compress::tests::compress_matches_python_jaccard
```
...
`engine/crates/cortex/assets/llmlingua/README.md`):

1. `$CORTEX_LLMLINGUA_MODEL` / `$CORTEX_LLMLINGUA_TOKENIZER` (explicit override)
2. `engine/crates/cortex/assets/llmlingua/{model.onnx,tokenizer.json}` (default)
...
3. **Workspace test green.** `cargo test --workspace -- --test-threads=1`:
   ...
   / 49 / 47 / 46 / 34 / **27 (cortex baseline)** / ... — all green.
...
- **runc.rs** (+3 tests): `shell_override_program_only_appends_platform_switch`, `shell_override_program_and_switch_used_verbatim`, `run_capped_no_spill_when_output_fits`. The first two close the loop on the `CORTEX_RUNC_SHELL` env-var path that the resolver advertises; the third is the negative case (no spill file when output fits).
...
- **store.rs** (+1 test, task 6 L8): `dream_now_on_empty_store_returns_zero_status` — covers the entry point the `cortex curate` CLI verb uses, ensuring it doesn't panic on a fresh DB.

### Final test counts
| Suite | Count | Δ from baseline |
|---|---|---|
| `cargo test -p cortex` (default features) | **39 passed** | +12 |
| `cargo test -p cortex --features llmlingua-onnx` | **53 passed** | +13 |
...
- Pre-existing scaffold fmt diffs in `prep.rs`/`skel.rs`/`serve.rs`/`compact.rs`/`runc.rs`/`lib.rs` and the pre-existing clippy warnings in `coderight-config` and various cortex modules — not my code; not silently reformatted.
