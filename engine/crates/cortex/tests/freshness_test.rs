...
#[test]
fn blueprint_without_base_commit_keeps_head_overlay_available_without_claiming_currency() {
...
fn filesystem_probe_binds_blueprint_to_commit_and_reports_verified_dirty_overlay() {
    ...
    let mut clean_probe = FilesystemFreshnessProbe::new(repo_root.clone(), &store)
        .with_blueprint_endpoint(endpoint.clone());
    let clean = evaluate_freshness(&mut clean_probe, 3);
    ...
    .unwrap();
    let mut dirty_probe =
        FilesystemFreshnessProbe::new(repo_root, &store).with_blueprint_endpoint(endpoint);
    let dirty = evaluate_freshness(&mut dirty_probe, 3);
