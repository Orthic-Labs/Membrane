use cortex::{CheckpointSourceRefV1, CheckpointV1};
use sha2::Digest;
...
fn source_status(root: &std::path::Path, source_ref: &str, expected_hash: &str) -> String {
    cortex::checkpoint::resolve_source_refs(
...
fn run_expand(database: &std::path::Path, spill: &std::path::Path, anchor: &str) -> Output {
...
fn worktree_document_identifier_compatibility_corpus() {
    ...
    let expected_hash = cortex::outline::build_outline(
...
fn anchor_identifier_compatibility_corpus() {
    ...
    let output = run_expand(&database, &spill, &format!("mr://anchor/{digest}"));
    ...
    for (anchor, error) in [
        ...
        ("mr://anchor/abc".into(), "invalid anchor digest"),
        (
            format!("mr://anchor/{}", "g".repeat(64)),
            "invalid anchor digest",
        ),
    ] {
        let output = run_expand(&database, &spill, &anchor);
        assert!(!output.status.success());
        assert_eq!(terminal_error(output), error);
    }

    let missing = run_expand(
        &database,
        &spill,
...
fn worktree_document_rejections_are_typed_before_io() {
    use cortex::identifier::{WorktreeDocRef, WorktreeDocRefError};
...
fn anchor_rejections_are_typed_before_io() {
    use cortex::identifier::{AnchorRef, AnchorRefError};
