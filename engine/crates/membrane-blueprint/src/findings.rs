...
fn findings_get(_g: &Generation, req: &BlueprintRequest, ctx: &RequestContext, bundle: Value, stale: bool, state: &Path) -> Result<Value, BlueprintError> { let mut out=bundle; let prefixes=paths(req); if !prefixes.is_empty(){filter_bundle(&mut out,&prefixes);} let delta=req.input.get("baselineGeneration").and_then(Value::as_str).map(|n| baseline_delta(ctx,state,n,&out).unwrap_or_else(|_|json!({"added":[],"persistent":[],"resolved":[],"changed":[],"unknown":true}))); out["kind"]=json!("findings.get"); out["root"]=json!(ctx.scope.repo_root); out["freshness"]=json!(if stale{"stale"}else{"current"}); out["delta"]=delta.unwrap_or(Value::Null); bound_response(&out,ctx)?; Ok(out) }
...
// Lane LIB5 (r5 windows closure): delegate to lib_findings_specifier's
// `candidate_paths`, which strips/rewrites an explicit `.js`/`.jsx`/`.mjs`/
// `.cjs` extension before completing candidates (the TS `.js` -> `.ts`
// substitution this inline resolver previously lacked -- see the "KNOWN GAP"
// note at the top of lib_findings_specifier.rs, now closed).
fn resolve_candidates(from:&str,spec:&str,files:&BTreeSet<String>)->Vec<String>{let mut out:Vec<String>=crate::lib_findings_specifier::candidate_paths(from,spec).into_iter().filter(|p|files.contains(p)).collect();out.sort();out.dedup();out}
fn norm(v:&str)->String{let s=v.replace('\\',"/");let mut out:Vec<&str>=Vec::new();for part in s.split('/'){match part{""|"."=>{},".."=>{out.pop();},p=>out.push(p),}}out.join("/")}
