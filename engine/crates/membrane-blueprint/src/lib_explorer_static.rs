//! Native port of `blueprint/src/lib/explorer/static.mjs`.
//!
//! Serves the local explorer UI's static assets. No native equivalent was
//! found for `serveExplorerAsset` (confirmed via grep). This is a faithful
//! standalone port: same four routes, same response headers (content-type,
//! cache-control, CSP, X-Content-Type-Options), same 404 behavior for any
//! other path, and byte-identical HTML/CSS/JS payloads copied verbatim from
//! the legacy module (first-party Blueprint source, not third-party content).
...
/// Mirrors the legacy `ASSETS` map exactly: `/`, `/index.html`,
/// `/explorer.css`, `/explorer.js`.
pub fn explorer_asset(path: &str) -> Option<ExplorerAsset> {
    match path {
        "/" | "/index.html" => Some(ExplorerAsset { content_type: "text/html; charset=utf-8", body: HTML }),
        "/explorer.css" => Some(ExplorerAsset { content_type: "text/css; charset=utf-8", body: CSS }),
        "/explorer.js" => Some(ExplorerAsset { content_type: "text/javascript; charset=utf-8", body: JS }),
        _ => None,
    }
}

pub const EXPLORER_CSP: &str = "default-src 'self'; connect-src 'self'; script-src 'self'; style-src 'self'; img-src 'none'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'";

/// A minimal, transport-agnostic response the caller adapts to its HTTP
/// server (the legacy module writes directly to a Node `http.ServerResponse`;
/// this port separates path resolution/headers from that I/O boundary).
#[derive(Debug, Clone)]
...
/// Mirrors `serveExplorerAsset(request, response)`'s decision logic: resolve
/// the request path (query string ignored, matching `new URL(...).pathname`)
/// against the known asset map, and build the exact response headers/body/
/// status the legacy handler would write.
pub fn serve_explorer_asset(request_path: &str) -> ExplorerResponse {
    let pathname = request_path.split(['?', '#']).next().unwrap_or("/");
    match explorer_asset(pathname) {
        None => ExplorerResponse {
            status: 404,
            headers: vec![("content-type", "text/plain; charset=utf-8".to_owned())],
            body: "not found".to_owned(),
        },
        Some(asset) => ExplorerResponse {
            status: 200,
            headers: vec![
                ("content-type", asset.content_type.to_owned()),
                ("cache-control", "no-store".to_owned()),
                ("content-security-policy", EXPLORER_CSP.to_owned()),
                ("x-content-type-options", "nosniff".to_owned()),
            ],
            body: asset.body.to_owned(),
        },
    }
}
