...
fn fixture() -> tempfile::TempDir {
    ...
    let source = include_str!("fixtures/languages/zig/runner.zig");
    fs::write(dir.path().join("runner.zig"), source).unwrap();
    dir
}

#[test]
fn zig_functions_are_extracted() {
    let dir = fixture();
...
#[test]
fn zig_contains_edges_link_file_to_symbols() {
    let dir = fixture();
    let generation = build_generation(dir.path(), &GraphOptions::default()).unwrap();
