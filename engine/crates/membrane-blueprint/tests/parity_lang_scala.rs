...
fn fixture() -> tempfile::TempDir {
    ...
    let source = include_str!("fixtures/languages/scala/Runner.scala");
    fs::write(dir.path().join("Runner.scala"), source).unwrap();
    dir
}

#[test]
fn scala_functions_are_extracted() {
    let dir = fixture();
...
#[test]
fn scala_contains_edges_link_file_to_symbols() {
    let dir = fixture();
    let generation = build_generation(dir.path(), &GraphOptions::default()).unwrap();
