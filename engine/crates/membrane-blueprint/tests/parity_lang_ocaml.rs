...
fn fixture() -> tempfile::TempDir {
    ...
    let source = include_str!("fixtures/languages/ocaml/runner.ml");
    fs::write(dir.path().join("runner.ml"), source).unwrap();
    dir
}

#[test]
fn ocaml_values_are_extracted() {
    let dir = fixture();
...
#[test]
fn ocaml_contains_edges_link_file_to_symbols() {
    let dir = fixture();
    let generation = build_generation(dir.path(), &GraphOptions::default()).unwrap();
