...
//! Git access, no Cortex DB queries. Producers feed it via JSON; consumers
...
mod tests {
    ...
    fn protected_candidate_wins_duplicate_source_hash_over_higher_scored_live_candidate() {
        ...
        protected.provider = Some("cortex".into());
    ...
    #[test]
    fn fresh_blueprint_repo_code_survives_cortex_live_and_skill_block_flood() {
        let mut candidates = Vec::new();
        for index in 0..16 {
            let mut memory = candidate(
                &format!("memory:role:relevant-{index:02}"),
    ...
    fn git_identity_lane_rejects_source_kind_spoof() {
        ...
        for index in 0..2 {
            ...
            spoof.provider = Some("cortex".into());
    ...
    fn candidate_provider_is_preserved_with_candidate_set_fallback() {
        ...
        input_value["candidate_set"]["candidates"][1]["provider"] =
            serde_json::Value::String("cortex".into());
        ...
        assert_eq!(block_providers["b"], "cortex");
    ...
    fn planner_records_selected_delivery_without_claiming_rendering() {
        ...
        assert_eq!(packet["providerAccounting"]["cortex"]["selectedTokens"], 40);
        assert_eq!(packet["providerAccounting"]["cortex"]["renderedTokens"], 0);
    ...
    fn receipt_status_is_scoped_to_candidate_set_provider() {
        ...
        assert_eq!(receipt["provider"], "cortex");
