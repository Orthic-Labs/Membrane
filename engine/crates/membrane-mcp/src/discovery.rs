...
pub fn initialize_response() -> Value {
    json!({"protocolVersion": PROTOCOL, "capabilities": {"tools": {}, "resources": {}, "prompts": {}}, "serverInfo": {"name": SERVER_NAME, "version": SERVER_VERSION}, "instructions": "Use membrane_context for federated context through /federate. Never expect raw memory CRUD. Membrane workflow prompts are bounded to operations in operations/operations/operations-index.v1.golden.json and grant no authority beyond those operations. Membrane MCP resources are bounded by the accessGrants declared in operations/resources/*.json; reads outside those grants return a typed resource_access_denied rejection with no content leak."})
}
