...
fn with_fixture_env<T>(script: &Path, run: impl FnOnce() -> T) -> T {
    ...
    std::env::set_var("CRYPT_FEDERATION_SCRIPT", script);
    let result = run();
    std::env::remove_var("CRYPT_FEDERATION_SCRIPT");
    result
}
...
fn federate_route_returns_envelope_with_transport_telemetry() {
    ...
    let (status, payload) = with_fixture_env(&script, || {
        crypt::serve::route_for_tests(&store, "POST", "/federate", &body)
    });
...
fn federate_route_second_call_reuses_the_worker() {
    ...
    let (first, second) = with_fixture_env(&script, || {
        let first = crypt::serve::route_for_tests(&store, "POST", "/federate", &body);
        let second = crypt::serve::route_for_tests(&store, "POST", "/federate", &body);
        (first, second)
    });
...
fn federate_route_missing_fields_are_400_and_dead_worker_is_502() {
    let store = crypt::MemoryStore::new();
    let (status, _) = crypt::serve::route_for_tests(&store, "POST", "/federate", "{}");
