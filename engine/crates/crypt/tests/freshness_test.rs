...
fn overlay_digest_covers_entries_beyond_the_return_cap() {
    ...
    let entries = (0..70)
        .map(|index| crypt::freshness::OverlayEntry {
...
fn partial_reindex_degrades_blueprint_without_disabling_stable_overlay_or_skills() {
    ...
    let mut probe = ScriptedProbe {
        ...
        overlays: VecDeque::from([OverlayObservation {
            stable: true,
            entries: vec![crypt::freshness::OverlayEntry {
...
fn filesystem_probe_binds_blueprint_to_commit_and_reports_verified_dirty_overlay() {
    ...
    let clean = crypt::freshness::evaluate_repository_freshness(
        &store,
    ...
    .unwrap();
    let dirty = crypt::freshness::evaluate_repository_freshness(
        &store,
        repo.path().canonicalize().unwrap(),
    );
...
fn filesystem_probe_reads_a_checkpointed_wal_graph_db_without_sidecars() {
    ...
    let store = crypt::MemoryStore::new();
    let verdict = crypt::freshness::evaluate_repository_freshness(
        &store,
        repo.path().canonicalize().unwrap(),
    );
