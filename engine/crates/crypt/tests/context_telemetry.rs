...
use crypt::memdb::{backout_v12_to_v11, backout_v13_to_v12, LATEST_SCHEMA_VERSION};
...
fn route_with_startup(
    ...
    crypt::serve::route_for_tests_with_startup_claim(
        store,
        identity,
        claim,
        "POST",
...
fn bounded_unknown_closure_links_each_unresolved_delivery_once() {
    ...
    let (code, body) = crypt::serve::route_for_tests_with_startup_claim(
        &store,
        &identity,
        &claim,
        "POST",
...
fn checked_in_registry_matches_canonical_parent_workspace_registry() {
    ...
    assert_eq!(
        local_sha,
        "1d7de4f7c27bbaa78c3dbe5f88cef4f413489df90666e37d5109a55b6f581217"
    );
...
fn telemetry_endpoint_requires_an_active_startup_lease() {
    ...
    let (status, _) =
        crypt::serve::route_for_tests(&store, "POST", "/v1/telemetry/events:batch", &body);
...
fn manual_put_get_delete_dual_write_complete_canonical_attribution() {
    use crypt::store::MemoryEventContext;
    use crypt_core::MemoryTier;
...
fn memory_batch_dual_write_preserves_each_items_family_producer_and_session() {
    use crypt::store::{MemoryBatchItem, MemoryBatchRequest};
...
fn feedback_write_emits_content_free_canonical_event() {
    use crypt::feedback::{FeedbackRecord, FeedbackSource};
    ...
    assert!(rows.iter().all(|row| row.5 == "crypt"));
