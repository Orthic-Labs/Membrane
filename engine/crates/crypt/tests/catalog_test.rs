...
//!
//! Coverage:
//!   - catalog opens WITHOUT touching the Crypt DB (separate file enforced)
...
use crypt::serve::{route_with_catalog_and_metrics_for_tests, route_with_catalog_for_tests};
...
#[test]
fn catalog_opens_at_separate_path_and_does_not_touch_crypt_db() {
...
fn scope_grant_issuance_and_lookup_round_trip() {
    ...
    let grant = crypt::catalog::lookup_grant(&catalog, "sg-1")
        .unwrap()
...
fn scope_grant_revoke_blocks_subsequent_plan_context() {
    ...
    assert!(crypt::catalog::revoke_scope_grant(&catalog, "sg-revoke").unwrap());
    let grant = crypt::catalog::lookup_grant(&catalog, "sg-revoke")
        .unwrap()
        .unwrap();
...
fn scope_grant_with_elapsed_ttl_is_observed_as_expired() {
    let catalog = new_catalog();
    let mut grant = crypt::catalog::issue_scope_grant(
        &catalog,
        "sg-ttl",
        "claude-mm",
        &["D--Claude".into()],
...
fn unknown_grant_lookup_returns_none() {
    let catalog = new_catalog();
    let found = crypt::catalog::lookup_grant(&catalog, "sg-nope").unwrap();
...
fn plan_context_admits_candidates_and_persists_content_free_receipts() {
    ...
    let receipts_count = crypt::catalog::count_receipts(&catalog).unwrap();
    assert!(receipts_count >= 2, "receipts persisted: {receipts_count}");
    let events_count = crypt::catalog::count_events(&catalog).unwrap();
    assert!(
        events_count >= 1,
...
fn health_surfaces_catalog_schema_planner_metrics_without_repo_content() {
    ...
    assert_eq!(v["catalog"]["cryptDbUntouched"], true);
...
fn plan_context_records_receipt_with_synthesised_bytes_sha() {
    ...
    .unwrap();
    let count = crypt::catalog::count_receipts_for_trace(&catalog, "trace-0").unwrap();
    assert_eq!(count, 1);
}
...
#[test]
fn scope_grant_persists_in_catalog_not_crypt_db() {
    let catalog = new_catalog();
    ...
    let grant = crypt::catalog::lookup_grant(&catalog, "sg-cross")
        .unwrap()
...
fn frozen_fixture_p95_under_50ms_above_direct_admission() {
    ...
    for _ in 0..100 {
        let input = crypt_core::planner::PlannerInput {
        ...
        let _ = crypt_core::planner::plan(&input).unwrap();
