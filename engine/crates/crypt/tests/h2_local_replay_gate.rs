...
fn store_input(
    projections: Vec<crypt::doc_projection::DocumentProjectionV1>,
...
fn h2_local_clone_replay_logs_numeric_loss_and_keeps_h2_disabled() {
    ...
    let source = MemDb::open(&source_path).expect("open source DB");
    ...
    let baseline = MemDb::open(&source_path).expect("reopen source DB");
    let h2_clone = MemDb::open(&clone_path).expect("open H2 clone DB");
    replace_doc_projections(
        &h2_clone,
        &store_input(project_markdown(
            markdown,
