use crypt::{CheckpointSourceRefV1, CheckpointV1};
use sha2::Digest;
...
fn source_status(root: &std::path::Path, source_ref: &str, expected_hash: &str) -> String {
    crypt::checkpoint::resolve_source_refs(&checkpoint_with_source(source_ref, expected_hash), root)
        [0]
    .status
    .clone()
}
...
fn run_expand(database: &std::path::Path, spill: &std::path::Path, anchor: &str) -> Output {
    Command::new(env!("CARGO_BIN_EXE_crypt"))
        .args([
            "--db",
...
fn worktree_document_identifier_compatibility_corpus() {
    ...
    let expected_hash = crypt::outline::build_outline(
