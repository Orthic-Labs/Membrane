//! `crypt skill-read <name>` resolves a workspace skill body cross-repo (via --root),
...
fn skill_read_resolves_body_and_guards_traversal() {
    let bin = env!("CARGO_BIN_EXE_crypt");
...
fn skill_read_serves_from_engine_without_skills_directory() {
    ...
    let out = std::process::Command::new(bin)
        .env("CRYPT_DB", db.to_str().unwrap())
        .env("RIGHTCONTEXT_SKILL_EVENT_PATH", &event_path)
        .env("CRYPT_CLIENT", "claude")
