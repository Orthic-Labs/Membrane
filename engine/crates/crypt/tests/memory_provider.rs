//! G5 Lane B — Crypt durable-memory provider contract tests.
//!
...
//! Crypt compatibility API as `crypt::memory_provider`.

use crypt::memdb::MemDb;
use crypt::memory_provider::{
...
fn put(store: &MemoryStore, name: &str, content: &str, scope: &str, _score: f64) -> String {
    store
        .try_put(name, content, scope, crypt_core::MemoryTier::Semantic)
        .unwrap();
    format!("{}/{}", scope, name)
}
...
/// rather than lexical: Crypt classifies every candidate `data_only` and
