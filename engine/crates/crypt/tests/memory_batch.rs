...
fn memories_batch_commits_every_item_and_returns_truthful_receipts() {
    ...
    let (status, payload) =
        crypt::serve::route_for_tests(&store, "POST", "/v1/memories:batch", &body);
...
fn identical_retry_is_noop_and_conflicting_batch_id_is_atomic_409() {
    ...
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/v1/memories:batch", &body).0,
        201
    );
    ...
    let (conflict_status, conflict_payload) =
        crypt::serve::route_for_tests(&store, "POST", "/v1/memories:batch", &changed);
...
fn generic_put_persists_validated_write_attribution_and_rejects_family_typos() {
    ...
    .to_string();
    let (status, payload) = crypt::serve::route_for_tests(&store, "POST", "/put", &body);
    ...
    .to_string();
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/put", &typo).0,
        400
    );
    assert_eq!(
        store
...
fn generic_put_round_trips_camel_case_lifecycle_input() {
    ...
    .to_string();
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/put", &body).0,
        200
    );
    ...
    let get = crypt::serve::route_for_tests(
        &store,
        "POST",
        "/get",
...
fn lifecycle_route_rejects_bad_confidence_and_priority_without_writes() {
    for body in [
        ...
        let store = MemoryStore::open(MemDb::open_in_memory());
        let response = crypt::serve::route_for_tests(&store, "POST", "/put", body);
...
fn batch_supersession_log_failure_rolls_back_new_and_old_rows() {
    ...
    replacement
        ...
        .unwrap()
        .insert("supersedes".into(), json!("D--Claude/old-rule"));
    let response = crypt::serve::route_for_tests(
        &store,
        "POST",
        "/v1/memories:batch",
...
fn http_put_get_list_delete_emit_complete_linked_lifecycles_with_opaque_identity() {
    ...
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/put", &put).0,
        200
    );
    let get = format!(r#"{{"id":"global/edge",{attribution}}}"#);
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/get", &get).0,
        200
    );
    let list = format!(r#"{{"scope":"global",{attribution}}}"#);
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/list", &list).0,
        200
    );
    let delete = format!(r#"{{"id":"global/edge",{attribution}}}"#);
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/delete", &delete).0,
        200
    );
...
fn http_validation_missing_targets_and_empty_lists_close_with_typed_terminals() {
    ...
    for (path, body, expected) in cases {
        assert_eq!(
            crypt::serve::route_for_tests(&store, "POST", path, body).0,
            expected,
            "{path}"
        );
    }
...
fn memories_batch_accounts_success_duplicate_conflict_and_invalid_envelope() {
    ...
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/v1/memories:batch", &conflict).0,
        409
    );
    assert_eq!(
        crypt::serve::route_for_tests(&store, "POST", "/v1/memories:batch", "{").0,
        400
    );
...
fn http_put_commit_failure_rolls_back_memory_and_records_failed_terminal() {
    ...
    store
        ...
        .unwrap();
    let (status, _) = crypt::serve::route_for_tests(
        &store,
        "POST",
        "/put",
...
fn http_list_provider_failure_is_not_reported_as_an_empty_result() {
    ...
    let (status, _) =
        crypt::serve::route_for_tests(&store, "POST", "/list", r#"{"scope":"global"}"#);
    assert_eq!(status, 500);
