# G5 Lane B — Crypt provider integration note
...
- `tools/crypt/crates/crypt/src/memory_provider.rs` — new module
- `tools/crypt/crates/crypt/tests/memory_provider.rs` — new contract tests
- `tools/crypt/crates/crypt/src/store.rs` — additive 8-line change
...
```diff
--- a/tools/crypt/crates/crypt/src/lib.rs
+++ b/tools/crypt/crates/crypt/src/lib.rs
@@ -6,6 +6,7 @@
 pub mod compress;
 pub mod memdb;
...
and the `ContextCandidateSet` types so the planner service can `use crypt::memory_provider::*;`.
...
`crypt::memory_provider::produce_candidate_set` directly.

## Tests

`cargo test --manifest-path tools/crypt/Cargo.toml --test memory_provider`
...
- `cargo test --manifest-path tools/crypt/Cargo.toml` — full workspace,
  243 tests pass (94 store + 8 db_first + 4 embedder_probe + 17
  memory_provider + 117 crypt-core + 3 crypt-format + others).
- The forbidden hot-file list is untouched: `git diff --stat
  main...HEAD -- tools/lib/context-contracts.schema.json
  tools/crypt/crates/crypt/src/{main,lib,serve,memdb}.rs
