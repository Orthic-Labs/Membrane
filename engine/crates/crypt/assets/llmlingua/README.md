...
for the `compress` ONNX path (`crypt compress --rate 0.5` with the
...
From `tools/crypt`, in an isolated venv (the system Python's `transformers`
...
Copy-Item spike\export\model.onnx    crates\crypt\assets\llmlingua\model.onnx
Copy-Item spike\export\tokenizer.json crates\crypt\assets\llmlingua\tokenizer.json
```
...
1. `$CRYPT_LLMLINGUA_MODEL` and `$CRYPT_LLMLINGUA_TOKENIZER` env vars
   (explicit override)
2. `crates/crypt/assets/llmlingua/model.onnx` and `tokenizer.json`
