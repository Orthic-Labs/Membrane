...
impl TeamPolicySyncV1 {
    pub fn has_valid_bounds(&self) -> bool {
        self.schema_version == TEAM_POLICY_SCHEMA_VERSION
            && safe_id(&self.policy_id)
            && safe_id(&self.tenant_id)
            && safe_id(&self.team_id)
            && safe_id(&self.user_id)
            && self.generation > 0
            && safe_id(&self.key_rotation_id)
            && safe_id(&self.audit_export_id)
            && valid_digest(&self.admin_policy_sha256)
            && self.envelope.tenant_id == self.tenant_id
            && self.envelope.team_id == self.team_id
            && self.envelope.generation == self.generation
            && safe_id(&self.envelope.envelope_id)
            && safe_id(&self.envelope.key_id)
            && safe_nonce(&self.envelope.replay_nonce)
            && valid_digest(&self.envelope.ciphertext_sha256)
            && self.scopes.len() == 3
            && self
                .scopes
                .iter()
                .any(|scope| matches!(scope, TeamPolicyScopeV1::Tenant))
            && self
                .scopes
                .iter()
                .any(|scope| matches!(scope, TeamPolicyScopeV1::Team))
            && self
                .scopes
                .iter()
                .any(|scope| matches!(scope, TeamPolicyScopeV1::User))
            && !self
                .scopes
                .iter()
                .any(|scope| matches!(scope, TeamPolicyScopeV1::LocalRoot))
