...
fn service_workspace_root(db: &Path) -> PathBuf {
    std::env::var_os("WORKSPACE_ROOT")
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
        .or_else(|| {
            let memory = db.parent()?;
            let cache = memory.parent()?;
            let tools = cache.parent()?;
            if memory.file_name().is_some_and(|name| name == "memory")
                && cache.file_name().is_some_and(|name| name == ".cache")
                && tools.file_name().is_some_and(|name| name == "tools")
            {
        ...
        .unwrap_or_else(|| skill_workspace_root(None))
}
...
fn run_main() -> Result<(), String> {
    ...
    match cli.cmd {
        ...
        Cmd::Put {
            ...
            // cwd-slug shape like D--Claude-heardright.
            if scope != "global" && !scope.chars().take(3).collect::<String>().ends_with("--") {
                eprintln!(
                    "warning: scope {scope:?} looks hand-typed — project scopes are cwd slugs \
                     like D--Claude-heardright; a novel scope quietly forks the corpus"
                );
            }
...
mod tests {
    ...
    fn ancient_pending_is_retained_without_ttl_and_future_state_is_rejected() {
        ...
        let future = super::PutStateRecord {
            ...
            created_at_secs: super::pending_put_now()
                .saturating_add(super::MAX_PENDING_CLOCK_SKEW.as_secs() + 1),
            state: super::PutState::Pending,
        };
    ...
    fn cross_process_filesystem_lock_excludes_a_second_process() {
        ...
        command
            .arg("--exact")
            .arg("tests::cross_process_lock_child_helper")
            .arg("--ignored")
            .arg("--nocapture")
            .arg("--test-threads=1")
