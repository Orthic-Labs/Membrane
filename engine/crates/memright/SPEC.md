# MemRight — Rust memory SDK (parity tracker)
...
`memright` is the product: store + scope model + scoring + ingest + serve + CLI. CR-the-app keeps using
`coderight-memory` in-process (single scope); the workspace uses `memright serve` over the existing
...
| Migrate + cutover | `migrate-all` over real markdown, recall parity vs Python, flip hooks to `memright serve`, retire mem.py | P6 | **DONE — Rust is live, mem.py retired** |

## Status (2026-06-30) — CUTOVER COMPLETE

All six phases done. The workspace recall engine is the Rust `memright` binary on `:8765`; **Python
`mem.py` is deleted**. The hooks (`recall_memory.py`/`ingest_memory.py`) lazy-start `memright` with
...
`target/debug/memright.exe`, so the migration + serve ran on the hash embedder. Fixed by building the
fastembed binary to a **stable path** (`tools/bin/memright.exe`). With the real BGE binary, recall
matched Python: same retrieved set, cos within ~0.02.

**Operational requirements — onnxruntime bundled (2026-06-30).** Running MemRight needs (1) the compiled
...
**Storage convergence — DONE (2026-06-30).** MemRight stores **quantized turbovec** vectors via
...
matches f32 within ~0.001, same retrieved set). MemRight follows CR's lightweight design.

**Remaining (larger product decision, not blocking):** CodeRight's daemon still wires
`coderight-memory` (retriever/registry/dream/tiers) directly for its *session* memory; MemRight is the
*workspace multi-project* layer. They now share the same core + quantization. Full unification (CR's
daemon consuming MemRight's store/recall wholesale) would mean reconciling CR's session-tier/registry
features with MemRight's scope/kind/markdown recall — a real product decision, not a quick win.

**Cutover steps (after parity holds — production-mutating, repoints the live recall system):**
1. Migrate the production store (`migrate-all` + `migrate-blueprint`, real BGE) into `memright.db`.
2. Run `memright serve` on `:8765` (with `ORT_DYLIB_PATH` set); stop `mem.py serve`.
3. Repoint the hook lazy-start to launch the `memright` binary (set `ORT_DYLIB_PATH`).
4. Retire `mem.py`; update `CONTEXT-ENGINEERING.md` (engine = Rust).

**Operational note for distribution:** running MemRight needs the compiled binary + an `onnxruntime.dll`
