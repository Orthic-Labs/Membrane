...
for the `compress` ONNX path (`memright compress --rate 0.5` with the
...
From `tools/memright`, in an isolated venv (the system Python's `transformers`
...
Copy-Item spike\export\model.onnx    crates\memright\assets\llmlingua\model.onnx
Copy-Item spike\export\tokenizer.json crates\memright\assets\llmlingua\tokenizer.json
```
...
1. `$MEMRIGHT_LLMLINGUA_MODEL` and `$MEMRIGHT_LLMLINGUA_TOKENIZER` env vars
   (explicit override)
2. `crates/memright/assets/llmlingua/model.onnx` and `tokenizer.json`
