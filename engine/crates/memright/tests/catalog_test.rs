...
//!
//! Coverage:
//!   - catalog opens WITHOUT touching the MemRight DB (separate file enforced)
...
use memright::catalog::{
    default_catalog_path, record_receipt, ContextCatalog, GrantStatus, CATALOG_SCHEMA_VERSION,
};
use memright::memdb::MemDb;
use memright::planner_metrics::{LastFallback, PlannerLatency};
use memright::serve::{route_with_catalog_and_metrics_for_tests, route_with_catalog_for_tests};
...
#[test]
fn catalog_opens_at_separate_path_and_does_not_touch_memright_db() {
    let dir = tempfile::tempdir().unwrap();
    let memright_path = dir.path().join("memright-engine.db");
    let catalog_path = dir.path().join("catalog.db");
    let sentinel = b"MEMRIGHT_DB_SENTINEL_BYTES";
    std::fs::write(&memright_path, sentinel).unwrap();

    let _catalog = ContextCatalog::open(&catalog_path).unwrap();
    assert_eq!(std::fs::read(&memright_path).unwrap(), sentinel);
...
#[test]
fn default_catalog_path_avoids_memright_db_directory() {
...
fn scope_grant_issuance_and_lookup_round_trip() {
    ...
    let grant = memright::catalog::lookup_grant(&catalog, "sg-1")
        .unwrap()
...
fn scope_grant_revoke_blocks_subsequent_plan_context() {
    ...
    assert!(memright::catalog::revoke_scope_grant(&catalog, "sg-revoke").unwrap());
    let grant = memright::catalog::lookup_grant(&catalog, "sg-revoke")
        .unwrap()
        .unwrap();
...
fn scope_grant_with_elapsed_ttl_is_observed_as_expired() {
    let catalog = new_catalog();
    let mut grant = memright::catalog::issue_scope_grant(
        &catalog,
        "sg-ttl",
        "claude-mm",
        &["D--Claude".into()],
...
fn unknown_grant_lookup_returns_none() {
    let catalog = new_catalog();
    let found = memright::catalog::lookup_grant(&catalog, "sg-nope").unwrap();
...
fn plan_context_admits_candidates_and_persists_content_free_receipts() {
    ...
    let receipts_count = memright::catalog::count_receipts(&catalog).unwrap();
    assert!(receipts_count >= 2, "receipts persisted: {receipts_count}");
    let events_count = memright::catalog::count_events(&catalog).unwrap();
    assert!(
        events_count >= 1,
...
fn health_surfaces_catalog_schema_planner_metrics_without_repo_content() {
    ...
    assert_eq!(v["catalog"]["memrightDbUntouched"], true);
...
fn plan_context_records_receipt_with_synthesised_bytes_sha() {
    ...
    .unwrap();
    let count = memright::catalog::count_receipts_for_trace(&catalog, "trace-0").unwrap();
    assert_eq!(count, 1);
}
...
#[test]
fn scope_grant_persists_in_catalog_not_memright_db() {
    let catalog = new_catalog();
    ...
    let grant = memright::catalog::lookup_grant(&catalog, "sg-cross")
        .unwrap()
...
fn frozen_fixture_p95_under_50ms_above_direct_admission() {
    ...
    for _ in 0..100 {
        let input = memright_core::planner::PlannerInput {
        ...
        let _ = memright_core::planner::plan(&input).unwrap();
