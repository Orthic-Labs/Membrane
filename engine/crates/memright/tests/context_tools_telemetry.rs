...
fn failed_skel_invocation_is_persisted_without_content() {
    ...
    let db = temp.path().join("memright.db");
    let missing = temp.path().join("missing.rs");

    let output = Command::new(env!("CARGO_BIN_EXE_memright"))
        .args([
            "--db",
            db.to_str().unwrap(),
            "skel",
...
fn runc_links_to_the_exact_recommendation_opportunity() {
    ...
    let create = Command::new(env!("CARGO_BIN_EXE_memright"))
        .args([
            "--db",
            db.to_str().unwrap(),
    ...
    let run = Command::new(env!("CARGO_BIN_EXE_memright"))
        .current_dir(temp.path())
        .args([
            "--db",
            db.to_str().unwrap(),
