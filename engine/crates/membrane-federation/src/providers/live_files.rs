...
use membrane_provider_sdk::{
    CapabilityV1, Provider, ProviderContext, ProviderError, ProviderOutput,
};
...
impl GitObjectSource for NativeGitObjectSource {
    fn read_blob(
        ...
        let commit_id =
            gix::ObjectId::from_hex(commit.as_bytes()).map_err(|_| FileReadError::Unavailable)?;
...
impl Default for LiveFilesProvider {
    fn default() -> Self {
        Self::new(
            Arc::new(FilesystemWorkingTree),
            Arc::new(NativeGitObjectSource),
        )
    }
}
...
impl Provider for LiveFilesProvider {
    async fn provide(&self, context: &ProviderContext) -> Result<ProviderOutput, ProviderError> {
        ...
        if grant.repository_id != context.repository_id || grant.session_id != context.session_id {
            return Ok(boundary_output(
                FederationProviderStatusV1::Failed,
...
pub fn produce_entries(
    ...
    let Some(overlay_digest) = context
        .freshness
        .overlay_digest
        .as_deref()
        .filter(|value| !value.trim().is_empty())
    ...
    for (path, entry) in normalized.into_iter().take(cap) {
        if context.is_cancelled() {
            ...
            output
                .omissions
                .push(omission("request_cancelled", ReasonCode::ProviderCancelled));
            break;
        }
        if context.is_deadline_exhausted() {
            output.status = FederationProviderStatusV1::Partial;
            output
                .omissions
                .push(omission("deadline_exhausted", ReasonCode::ProviderTimeout));
            break;
        }
        ...
        let snapshot_path = if let Some(old_path) = entry.old_path.as_deref() {
            let Some(old_path) = normalize_overlay_path(old_path) else {
                output.warnings.push(warning(
                    ReasonCode::ScopeGrantInvalid,
                    "dirty_overlay_old_path_rejected",
                ));
                output.omissions.push(omission(
                    "dirty_overlay_old_path_rejected",
                    ReasonCode::ScopeGrantInvalid,
        ...
        if !is_new {
            match git_objects.read_blob(
                root,
                base_commit,
                &snapshot_path,
                MAX_HISTORICAL_BLOB_BYTES,
            ) {
                Ok(bytes) => {
                    let value =
                        String::from_utf8(bytes).expect("Git object source rejects binary bytes");
                    output.candidates.push(snapshot_candidate(
                        base_commit,
                        overlay_digest,
                        &snapshot_path,
                        &value,
        ...
        output.candidates.push(overlay_candidate(
            base_commit,
            overlay_digest,
            &path,
            &working_hash,
    ...
    output.status = if output.omissions.is_empty() {
...
fn snapshot_candidate(base: &str, digest: &str, path: &str, text: &str) -> CandidateV1 {
    CandidateV1 {
        id: format!("git-snapshot:{}:{path}", &base[..base.len().min(12)]),
        layer: 3,
        provider: Some(PROVIDER_ID.as_str().to_owned()),
        source_kind: "repo_code".to_owned(),
        source_ref: format!("{path}@{}", &base[..base.len().min(12)]),
        source_hash: digest_str(text),
        trust_class: "workspace_tracked".to_owned(),
        instruction_policy: "data_only".to_owned(),
        provider_score: 0.45,
        score_components: BTreeMap::from([
            (String::from("git_snapshot_relevance"), 0.45),
            (String::from("freshness"), 0.5),
        ]),
        base_commit: Some(base.to_owned()),
        overlay_digest: Some(digest.to_owned()),
        freshness_class: Some(FreshnessClass::CommittedSnapshot),
        snapshot_id: None,
        estimated_tokens: 24,
        protected: false,
        exact: true,
        recoverable: true,
        resolver: format!("git show {base}:{path}"),
        text: format!("committed snapshot path={path}"),
    }
}
...
fn overlay_candidate(
    ...
    CandidateV1 {
        id: format!("git-overlay:{path}"),
        layer: 3,
        provider: Some(PROVIDER_ID.as_str().to_owned()),
        source_kind: "live_overlay".to_owned(),
        ...
        provider_score: 0.7,
        score_components: BTreeMap::from([
            (String::from("git_overlay_relevance"), 0.7),
            (String::from("freshness"), 1.0),
        ]),
        base_commit: Some(base.to_owned()),
        overlay_digest: Some(digest.to_owned()),
        freshness_class: Some(FreshnessClass::DirtyOverlay),
        snapshot_id: None,
        estimated_tokens: 32,
        protected: false,
        exact: false,
        recoverable: true,
        resolver: format!("git diff --no-ext-diff {base} -- {path}"),
        text: format!("working-tree status={} path={path}", status),
    }
}
...
pub fn normalize_overlay_path(value: &str) -> Option<String> {
    ...
    if raw.is_empty()
        || raw.len() > MAX_OVERLAY_PATH_CHARS
        || raw != raw.trim()
        || raw.starts_with('/')
...
fn confined_path(root: &Path, relative: &str) -> Option<PathBuf> {
    ...
    if candidate.exists() && has_symlink_component(&canonical_root, &candidate) {
        return None;
    }
    Some(candidate)
}
...
fn warning(reason: ReasonCode, detail: &str) -> ProviderWarningV1 {
    ProviderWarningV1 {
        ...
        stage: Some("live_overlay".to_owned()),
        message: None,
    }
}
fn omission(detail: &str, reason: ReasonCode) -> ProviderOmissionV1 {
    ProviderOmissionV1 {
        provider: PROVIDER_ID,
        reason,
        candidate_id: None,
...
fn read_omission(error: FileReadError) -> ProviderOmissionV1 {
    omission(
        match error {
            FileReadError::Unavailable => "overlay_unavailable",
            FileReadError::Oversized => "overlay_oversized",
            FileReadError::Binary => "overlay_binary",
        },
        ReasonCode::ProviderMalformed,
    )
}
