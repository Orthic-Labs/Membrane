...
use membrane_provider_sdk::{BlueprintSource, Provider, ProviderContext, ProviderError, ProviderOutput};
use membrane_protocol::{FederationProviderStatusV1, ProviderId, ProviderOmissionV1, ProviderWarningV1, ReasonCode, WarningSeverity, PROVIDER_OUTPUT_SCHEMA_VERSION};
...
impl BlueprintProvider {
    ...
    pub fn new(_source: Arc<dyn BlueprintSource>) -> Self {
        Self { contextual_source: None, candidate_cap: BLUEPRINT_DEFAULT_CAP }
    }
    ...
    {
        Self { contextual_source: Some(source), candidate_cap: BLUEPRINT_DEFAULT_CAP }
    }
    ...
    pub fn with_contextual_source_pair(
        ...
        Self { contextual_source: Some(contextual_source), candidate_cap: BLUEPRINT_DEFAULT_CAP }
    }
    ...
    pub fn candidate_cap(&self) -> usize { self.candidate_cap }

    async fn provide_inner(&self, context: &ProviderContext) -> Result<ProviderOutput, ProviderError> {
        if context.is_cancelled() { return Err(ProviderError::Cancelled); }
        if context.is_deadline_exhausted() { return Err(ProviderError::DeadlineExceeded); }
        ...
        let response = match match self.contextual_source.as_ref() {
            Some(source) => source.query_with_context(&query, context.deadline, context.cancellation.clone()).await,
        ...
        if context.is_cancelled() { return Err(ProviderError::Cancelled); }
        if context.is_deadline_exhausted() { return Err(ProviderError::DeadlineExceeded); }
        if let Some(expected) = expected_generation.as_deref().filter(|value| !value.is_empty()) {
            if response.generation.as_deref() != Some(expected) || response.value.generation != expected {
                return Ok(generation_gap(expected, response.generation.as_deref().unwrap_or(&response.value.generation)));
            }
        }
...
impl Provider for BlueprintProvider {
    ...
    fn list_capabilities(&self) -> Vec<membrane_provider_sdk::CapabilityV1> {
        vec![membrane_provider_sdk::CapabilityV1 { name: "membrane_blueprint".into(), schema_version: 1, error_version: 1 }]
    }
}
...
fn gap_output(error: ProviderError) -> ProviderOutput {
    let (reason, detail) = match error {
        ProviderError::Unavailable(_) | ProviderError::MissingSource(_) => (ReasonCode::ProviderUnavailable, "blueprint_unavailable"),
    ...
    ProviderOutput {
        ...
        generation: None,
        candidates: Vec::new(),
        warnings: vec![ProviderWarningV1 { provider: ProviderId::Blueprint, reason, severity: WarningSeverity::Warning, detail_id: Some(detail.into()), stage: Some("blueprint_source".into()), message: None }],
        omissions: vec![ProviderOmissionV1 { provider: ProviderId::Blueprint, reason, candidate_id: None, detail_id: Some(detail.into()), stage: Some("blueprint_source".into()) }],
        diagnostics: None,
        extensions: Default::default(),
    }
}
...
fn generation_gap(expected: &str, observed: &str) -> ProviderOutput {
    ProviderOutput {
        ...
        candidates: Vec::new(),
        warnings: vec![ProviderWarningV1 { provider: ProviderId::Blueprint, reason: ReasonCode::GenerationIncoherent, severity: WarningSeverity::Warning, detail_id: Some("generation_incoherent".into()), stage: Some("generation_admission".into()), message: None }],
        omissions: vec![ProviderOmissionV1 { provider: ProviderId::Blueprint, reason: ReasonCode::GenerationIncoherent, candidate_id: None, detail_id: Some(format!("expected:{expected}")), stage: Some("generation_admission".into()) }],
        diagnostics: None,
        extensions: Default::default(),
    }
}
...
fn oversized_output(cap: usize) -> ProviderOutput {
    ProviderOutput {
        ...
        generation: None,
        candidates: Vec::new(),
        warnings: vec![ProviderWarningV1 { provider: ProviderId::Blueprint, reason: ReasonCode::ProviderMalformed, severity: WarningSeverity::Warning, detail_id: Some("candidate_cap_exceeded".into()), stage: Some("blueprint_bounds".into()), message: None }],
        omissions: vec![ProviderOmissionV1 { provider: ProviderId::Blueprint, reason: ReasonCode::ProviderMalformed, candidate_id: None, detail_id: Some(format!("cap:{cap}")), stage: Some("blueprint_bounds".into()) }],
        diagnostics: None,
        extensions: Default::default(),
    }
}
