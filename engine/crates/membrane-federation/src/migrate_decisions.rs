...
use membrane_provider_sdk::{
    DecisionRecord, DecisionRecordSource, SourceQuery, SourceResponse, SourceResult, SourceWarning,
};
...
impl DecisionMatch {
    ...
    pub fn generation(mut self, value: impl Into<String>) -> Self {
        self.linked_generation = Some(value.into());
        self
    }
    pub fn lifecycle(mut self, value: impl Into<String>) -> Self {
...
impl std::fmt::Display for DecisionReadError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Io(error) => write!(formatter, "decision JSONL read failed: {error}"),
        }
    }
}
...
impl DecisionJsonlReader {
    ...
    pub fn read_matching<P: AsRef<Path>>(
        &self,
        path: P,
        matching: &DecisionMatch,
    ) -> Result<DecisionReadReport, DecisionReadError> {
        ...
        Box::pin(async move {
            ...
            for diagnostic in &report.diagnostics {
                warnings.push(SourceWarning {
                    code: if diagnostic.code == "source_unavailable" {
                        "provider_unavailable".to_owned()
                    } else {
                        "provider_malformed".to_owned()
                    },
            ...
            if matching.repository_id != repository_id
                || (expected_generation.is_some()
                    && matching.linked_generation != expected_generation)
            {
            ...
            if matching.repository_id.is_empty() {
                return Ok(SourceResponse {
                    value: Vec::new(),
                    generation: None,
            ...
            let records = std::mem::take(&mut report.records)
                ...
                .map(|mut record| {
                    if let Some(evidence) =
                        evidence.iter().find(|evidence| evidence.id == record.id)
                    {
                        for trigger in &evidence.revisit_triggers {
                            let marker = format!("{REVISIT_MARKER}{trigger}");
                            if !record.risks.iter().any(|risk| risk == &marker) {
                                record.risks.push(marker);
                            }
                        }
...
pub fn read_decisions<P: AsRef<Path>>(path: P) -> Result<DecisionReadReport, DecisionReadError> {
    ...
    for (index, raw) in String::from_utf8_lossy(&bytes).lines().enumerate() {
        ...
        let value: Value = match serde_json::from_str(raw) {
            Ok(value) => value,
            Err(_) => {
                report.diagnostics.push(diagnostic(line, "invalid_json"));
                continue;
            }
        };
        let Some(object) = value.as_object() else {
            report
                .diagnostics
                .push(diagnostic(line, "record_not_object"));
            continue;
        };
        if object.get("schemaVersion").and_then(Value::as_u64) != Some(1) {
            report
                .diagnostics
                .push(diagnostic(line, "unsupported_schema_version"));
            continue;
        }
        ...
        let Some(repository_id) = string_field(object, &["repositoryId", "repository_id"]) else {
            report
                .diagnostics
                .push(diagnostic(line, "missing_repository_id"));
            continue;
        };
        let Some(generation) = string_field(
            object,
            &["linkedGraphGeneration", "linked_generation", "generation"],
        ) else {
            report
                .diagnostics
                .push(diagnostic(line, "missing_linked_generation"));
            continue;
        };
        let Some(rationale) = string_field(object, &["rationale", "reason"]) else {
            report
                .diagnostics
                .push(diagnostic(line, "missing_rationale"));
            continue;
        };
        let alternatives = match string_array(object, &["alternatives"]) {
            Ok(values) => values,
            Err(code) => {
                report.diagnostics.push(diagnostic(line, code));
                continue;
            }
        };
        ...
        let evidence = DecisionEvidence {
            ...
            revisit_triggers: string_array(
                object,
                &["revisitTriggers", "reviewTriggers", "review_triggers"],
            )
            .unwrap_or_default(),
        };
...
fn string_array(object: &Map<String, Value>, names: &[&str]) -> Result<Vec<String>, &'static str> {
    ...
    let Some(array) = value.as_array() else {
        return Err("array_field_invalid");
    };
...
fn filter_report(mut report: DecisionReadReport, matching: &DecisionMatch) -> DecisionReadReport {
    ...
    for (record, detail) in report.records.into_iter().zip(report.evidence.into_iter()) {
        if record.repository_id != matching.repository_id
            || detail.repository_id != matching.repository_id
        {
            continue;
        }
        if !matching.scope_id.is_empty()
            && detail.scope_id.as_deref() != Some(matching.scope_id.as_str())
        {
            continue;
        }
        ...
        if let Some(lifecycle) = matching.lifecycle.as_deref() {
            if detail.lifecycle.as_deref() != Some(lifecycle) {
                continue;
            }
        ...
        if let Some(mode) = matching.mode.as_deref() {
            if detail.mode.as_deref() != Some(mode) {
                continue;
            }
        }
        records.push(record);
