//! Typed, generation-bound access to the resident Blueprint daemon.
//!
...
//! The transport is injectable so embedded composition and daemon IPC share
//! exactly one request, cap, cache, and response-validation path.

use membrane_provider_sdk::source::{BlueprintResult, BlueprintSource, SourceQuery, SourceResponse, SourceResult};
...
impl BlueprintQuery {
    ...
    pub fn from_source(source: &SourceQuery, expected: Option<String>, bounds: BlueprintBounds, deadline: Duration) -> Self {
        Self {
...
pub trait BlueprintTransport: Send + Sync {
    fn exchange(&self, request: &BlueprintWireRequest, bounds: BlueprintBounds, deadline: Duration, cancellation: CancellationToken) -> Result<BlueprintWireResponse, BlueprintClientError>;
}

#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum BlueprintClientError {
    #[error("Blueprint daemon is unavailable: {0}")]
    Unavailable(String),
    ...
    #[error("Blueprint daemon returned typed error {code}: {message}")]
...
impl<T: BlueprintTransport> BlueprintClient<T> {
    ...
    pub fn query_with_cancellation(&self, query: &BlueprintQuery, cancellation: CancellationToken) -> Result<BlueprintResult, BlueprintClientError> {
        ...
        let query = BlueprintQuery { bounds: query.bounds.bounded(), ..query.clone() };
        let key = query.cache_key();
        if query.expected_generation.as_deref().is_some_and(|generation| !generation.is_empty()) {
            if let Some(value) = self.cached(&key) {
                return Ok(value);
            }
        }
        let request = self.request(&query, "recall");
        let response = self.transport.exchange(&request, query.bounds, query.deadline, cancellation)?;
    ...
    pub fn resolve_symbol_with_cancellation(&self, query: &BlueprintQuery, symbol: &str, cancellation: CancellationToken) -> Result<BlueprintResult, BlueprintClientError> {
        if cancellation.is_cancelled() { return Err(BlueprintClientError::Cancelled); }
        if query.deadline.is_zero() { return Err(BlueprintClientError::Timeout); }
        let query = BlueprintQuery { symbol: Some(symbol.to_owned()), ..query.clone() };
        let request = self.request(&query, "resolve");
        let response = self.transport.exchange(&request, query.bounds.bounded(), query.deadline, cancellation)?;
    ...
    fn request(&self, query: &BlueprintQuery, method: &str) -> BlueprintWireRequest {
        ...
        BlueprintWireRequest {
            ...
            request_id: if query.request_id.trim().is_empty() { "blueprint-request".into() } else { query.request_id.clone() },
...
impl<T: BlueprintTransport + 'static> BlueprintSource for BlueprintClient<T> {
    fn query<'life0, 'life1, 'async_trait>(&'life0 self, source: &'life1 SourceQuery) -> Pin<Box<dyn Future<Output = SourceResult<BlueprintResult>> + Send + 'async_trait>>
    where
        'life0: 'async_trait,
        'life1: 'async_trait,
        Self: 'async_trait,
    ...
    fn resolve_symbol<'life0, 'life1, 'life2, 'async_trait>(&'life0 self, source: &'life1 SourceQuery, symbol: &'life2 str) -> Pin<Box<dyn Future<Output = SourceResult<BlueprintResult>> + Send + 'async_trait>>
    where
        'life0: 'async_trait,
        'life1: 'async_trait,
        'life2: 'async_trait,
...
fn parse_result(response: BlueprintWireResponse, query: &BlueprintQuery) -> Result<BlueprintResult, BlueprintClientError> {
    let response_bytes = serde_json::to_vec(&response).map_err(|error| BlueprintClientError::Malformed(error.to_string()))?;
    if response_bytes.len() > query.bounds.max_response_bytes {
        return Err(BlueprintClientError::Oversized("response_bytes"));
    }
    if let Some(expected_id) = (!query.request_id.trim().is_empty()).then_some(query.request_id.as_str()) {
        if response.request_id.as_deref() != Some(expected_id) && response.request_id.as_deref() != Some("blueprint-request") {
            return Err(BlueprintClientError::Malformed("response request identity mismatch".into()));
        }
    }
    if !response.ok {
        let error = response.error.unwrap_or(BlueprintWireError { code: None, message: None });
    ...
    let observed = response.generation.or_else(|| raw.get("generationId").and_then(Value::as_str).map(str::to_owned)).or_else(|| query.expected_generation.clone()).unwrap_or_default();
    if let Some(expected) = query.expected_generation.as_deref().filter(|value| !value.is_empty()) {
        if observed != expected {
            return Err(BlueprintClientError::GenerationMismatch { expected: expected.to_owned(), observed });
        }
    }
    let candidate_value = raw.get("candidateSet").and_then(|set| set.get("candidates")).or_else(|| raw.get("candidates")).ok_or_else(|| BlueprintClientError::Malformed("recall result has no candidates".into()))?;
    let candidates = candidate_value.as_array().ok_or_else(|| BlueprintClientError::Malformed("candidates is not an array".into()))?;
    if candidates.len() > query.bounds.max_candidates { return Err(BlueprintClientError::Oversized("candidate_count")); }
    let mut paths = std::collections::BTreeSet::new();
    for value in candidates {
        let Some(path) = value.get("sourceRef").and_then(Value::as_str) else { return Err(BlueprintClientError::Malformed("candidate sourceRef is missing".into())); };
        if path.len() > 4096 { return Err(BlueprintClientError::Oversized("path_length")); }
        paths.insert(path);
        if paths.len() > query.bounds.max_paths { return Err(BlueprintClientError::Oversized("path_count")); }
    }
    let typed = candidates.iter().cloned().map(|value| serde_json::from_value::<CandidateV1>(value).map_err(|error| BlueprintClientError::Malformed(error.to_string()))).collect::<Result<Vec<_>, _>>()?;
    let payload = Some(raw);
    Ok(BlueprintResult { generation: observed, candidates: typed, payload })
}
...
#[cfg(unix)]
#[derive(Debug, Clone)]
pub struct UnixBlueprintTransport { pub endpoint: PathBuf }

#[cfg(unix)]
impl UnixBlueprintTransport {
    pub fn new(endpoint: impl Into<PathBuf>) -> Self { Self { endpoint: endpoint.into() } }
}
...
impl BlueprintTransport for UnixBlueprintTransport {
    fn exchange(&self, request: &BlueprintWireRequest, bounds: BlueprintBounds, deadline: Duration, cancellation: CancellationToken) -> Result<BlueprintWireResponse, BlueprintClientError> {
        ...
        let mut socket = UnixStream::connect(&self.endpoint).map_err(|error| BlueprintClientError::Unavailable(error.to_string()))?;
        socket.set_read_timeout(Some(deadline.min(Duration::from_millis(25)))).map_err(|error| BlueprintClientError::Unavailable(error.to_string()))?;
        socket.set_write_timeout(Some(deadline)).map_err(|error| BlueprintClientError::Unavailable(error.to_string()))?;
        let wire = serde_json::to_vec(request).map_err(|error| BlueprintClientError::Malformed(error.to_string()))?;
        if wire.len() + 1 > bounds.max_response_bytes { return Err(BlueprintClientError::Oversized("request_frame")); }
        socket.write_all(&wire).and_then(|_| socket.write_all(b"\n")).map_err(|error| BlueprintClientError::Unavailable(error.to_string()))?;
        ...
        loop {
            ...
            match reader.read_until(b'\n', &mut line) {
                Ok(0) => return Err(BlueprintClientError::Unavailable("daemon returned no response".into())),
                Ok(_) => break,
        ...
        if line.len() > bounds.max_response_bytes { return Err(BlueprintClientError::Oversized("response_frame")); }
        serde_json::from_slice(&line).map_err(|error| BlueprintClientError::Malformed(error.to_string()))
    }
}
...
#[cfg(not(unix))]
impl UnixBlueprintTransport { pub fn new(endpoint: impl Into<PathBuf>) -> Self { Self { endpoint: endpoint.into() } } }

#[cfg(not(unix))]
impl BlueprintTransport for UnixBlueprintTransport {
    fn exchange(&self, _request: &BlueprintWireRequest, _bounds: BlueprintBounds, _deadline: Duration, _cancellation: CancellationToken) -> Result<BlueprintWireResponse, BlueprintClientError> {
        Err(BlueprintClientError::Unavailable("daemon IPC transport is unavailable on this host".into()))
    }
}
