...
fn decision_normalization_preserves_identity_and_provenance() {
    let candidate = normalize_decision(&record(), "repo-a", Some(&record().generation))
...
fn decision_normalization_rejects_scope_or_generation_drift() {
    assert_eq!(
        normalize_decision(&record(), "other", None),
        Err(DecisionNormalizationError::RepositoryMismatch)
    );
    assert_eq!(
        normalize_decision(
            &record(),
        ...
        Err(DecisionNormalizationError::GenerationMismatch)
    );
}
...
fn jsonl_reader_is_read_only_and_line_diagnostic() {
    let path =
        std::env::temp_dir().join(format!("membrane-decisions-{}.jsonl", std::process::id()));
...
fn jsonl_matching_preserves_scope_lifecycle_mode_and_revisit_provenance() {
    let path = std::env::temp_dir().join(format!(
        "membrane-decisions-match-{}.jsonl",
        std::process::id()
    ));
...
"#;
    std::fs::write(&path, body).expect("fixture write");
    let matching = DecisionMatch::new("repo-a", "scope-a")
        .generation("g2")
        .lifecycle("accepted")
        .mode("edit");
    let report = DecisionJsonlReader::new()
        .read_matching(&path, &matching)
        .expect("read");
    assert_eq!(report.records.len(), 1);
