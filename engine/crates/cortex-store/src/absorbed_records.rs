...
impl AbsorbedStore {
    ...
    pub fn get_session(
        &self,
        session_id: &str,
    ) -> Result<Option<SessionRecord>, AbsorbedStoreError> {
        let conn = self.db.lock();
        ensure_absorbed_schema(&conn)?;
        conn.query_row(
            "SELECT payload_json FROM absorbed_sessions WHERE session_id=?1",
            [session_id],
            |row| row.get::<_, String>(0),
        )
        .optional()?
        .map(|payload| serde_json::from_str(&payload).map_err(AbsorbedStoreError::from))
        .transpose()
    }
    ...
    pub fn delete_session(&self, session_id: &str) -> Result<bool, AbsorbedStoreError> {
        ...
        let event_rows: Vec<(i64, String)> = {
            let mut statement =
                tx.prepare("SELECT seq,event_id FROM absorbed_events WHERE session_id=?1")?;
        ...
        tx.execute(
            "DELETE FROM absorbed_events WHERE session_id=?1",
            [session_id],
        )?;
        tx.execute(
            "DELETE FROM absorbed_tasks WHERE session_id=?1",
            [session_id],
        )?;
        tx.execute(
            "DELETE FROM absorbed_artifacts WHERE session_id=?1",
            [session_id],
        )?;
        let changed = tx.execute(
            "DELETE FROM absorbed_sessions WHERE session_id=?1",
            [session_id],
        )?;
        tx.commit()?;
        Ok(changed != 0)
    ...
    fn append_event_in_tx(
        ...
        let existing: Option<(String, String, i64)> = tx
            .query_row(
                "SELECT payload_json,session_id,seq FROM absorbed_events WHERE event_id=?1",
                [&value.event_id],
        ...
        if let Some((stored, session_id, seq)) = existing {
            if stored == payload && session_id == value.session_id && seq == sequence {
                ...
                return Ok(AppendOutcome::AlreadyPresent(EventCursor {
        ...
        if tx
            .query_row(
                "SELECT 1 FROM absorbed_event_tombstones WHERE session_id=?1 AND seq=?2",
                params![value.session_id, sequence],
                |_| Ok(()),
            )
            .optional()?
            .is_some()
        {
            return Err(AbsorbedStoreError::Tombstoned {
                session_id: value.session_id.clone(),
                seq: value.seq,
            });
        }
        ...
        if value.seq != expected {
            return Err(if value.seq < expected {
                AbsorbedStoreError::DuplicateSequence(value.seq)
            } else {
                AbsorbedStoreError::SequenceGap {
                    expected,
                    actual: value.seq,
                }
            });
        ...
        Ok(AppendOutcome::Inserted(EventCursor {
            session_id: value.session_id.clone(),
            last_seq: value.seq,
        }))
    }
    ...
    pub fn cursor(&self, session_id: &str) -> Result<EventCursor, AbsorbedStoreError> {
        ...
        Ok(EventCursor {
            ...
            last_seq: conn
                .query_row(
                    "SELECT last_seq FROM absorbed_event_cursors WHERE session_id=?1",
                    [session_id],
                    |row| row.get::<_, i64>(0),
                )
                .optional()?
    ...
    pub fn events_range(
        ...
        start_seq: u64,
        end_seq: u64,
    ) -> Result<Vec<SessionEvent>, AbsorbedStoreError> {
        let conn = self.db.lock();
        ensure_absorbed_schema(&conn)?;
        ...
        let rows = statement.query_map(
            params![session_id, start_seq as i64, end_seq as i64],
            |row| row.get::<_, String>(0),
        )?;
        let mut result = Vec::new();
        for row in rows {
            result.push(serde_json::from_str(&row?).map_err(AbsorbedStoreError::from)?);
        }
        Ok(result)
    }
    ...
    pub fn tombstone_event(
        ...
        let event_id: String = tx
            .query_row(
                "SELECT event_id FROM absorbed_events WHERE session_id=?1 AND seq=?2",
                params![session_id, seq as i64],
                |row| row.get(0),
            )
            .optional()?
            .ok_or_else(|| AbsorbedStoreError::EventNotFound {
                session_id: session_id.to_string(),
                seq,
            })?;
        tx.execute(
            "UPDATE absorbed_events SET tombstoned=1 WHERE session_id=?1 AND seq=?2",
            params![session_id, seq as i64],
        )?;
    ...
    pub fn validate_import(events: &[SessionEvent]) -> Result<(), AbsorbedStoreError> {
        ...
        for event in events {
            ...
            if Some(event.session_id.as_str()) != session {
                return Err(AbsorbedStoreError::SessionMismatch);
            }
            if !ids.insert(event.event_id.as_str()) {
                return Err(AbsorbedStoreError::EventIdConflict(event.event_id.clone()));
            }
            if event.seq != expected {
                return Err(if event.seq < expected {
                    AbsorbedStoreError::Reordered
                } else {
                    AbsorbedStoreError::SequenceGap {
                        expected,
                        actual: event.seq,
    ...
    pub fn import_events(&self, events: &[SessionEvent]) -> Result<usize, AbsorbedStoreError> {
        ...
        for event in events {
            ...
            if matches!(
                Self::append_event_in_tx(&tx, event, &payload, &provenance)?,
    ...
    pub fn insert_artifact(&self, value: &ArtifactRecord) -> Result<(), AbsorbedStoreError> {
        self.put_artifact(value)
    }
    pub fn update_artifact(&self, value: &ArtifactRecord) -> Result<(), AbsorbedStoreError> {
        self.put_artifact(value)
    }
    pub fn delete_artifact(&self, artifact_id: &str) -> Result<bool, AbsorbedStoreError> {
        let conn = self.db.lock();
        ensure_absorbed_schema(&conn)?;
        Ok(conn.execute(
            "DELETE FROM absorbed_artifacts WHERE artifact_id=?1",
            [artifact_id],
        )? != 0)
    }
}
