...
//! so Cortex is publishable without dragging in CodeRight's governance DB.
...
impl MemDb {
    ...
    /// Startup-only WAL observations for main Cortex storage then physical event ledger.
