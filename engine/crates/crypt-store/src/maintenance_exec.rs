//! MBR-508: crash-safe, bounded execution of previously-planned Crypt maintenance.
//!
...
//! Crypt SQLite store, and nothing else.
//!
//! [`BoundedMaintenanceOperation`] mirrors `membrane_supervisor::maintenance::MaintenanceOperation`
//! by contract, not by crate dependency: `crypt-store` is already a dependency of
//! `membrane-runtime`, which `membrane-supervisor` also depends on, so a `crypt-store` ->
...
impl MemDb {
    /// Execute a bounded maintenance operation against the Crypt store. See the module docs for
