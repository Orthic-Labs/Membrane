...
//!
//! 2. **Deterministic heuristic** (always available) — Push's own protected-span
//!    compressor, a quality-degraded fallback. Selected when `--no-onnx` is set, when the
//!    `llmlingua-onnx` feature isn't built, or when the ONNX assets are missing.
//!
//! Asset resolution: `$MEMBRANE_PUSH_LLMLINGUA_MODEL` and
//! `$MEMBRANE_PUSH_LLMLINGUA_TOKENIZER` override defaults under this crate's
//! `assets/llmlingua/` directory. The DLL is
...
/// Compact, content-free metadata value for Push telemetry.
...
pub fn compress_with_options(text: &str, rate: f32, no_onnx: bool) -> String {
    ...
    #[cfg(feature = "llmlingua-onnx")]
    {
        if !no_onnx {
            match compress_llmlingua_onnx(text, rate) {
                ...
                Err(err) => {
                    eprintln!(
                        "[membrane.push] llmlingua-onnx unavailable ({err}); using Push heuristic"
                    );
                }
            }
        }
    ...
    compress_prose(text, rate)
}

/// Push-owned deterministic prose reduction. Markdown structure, source spans,
/// links, URLs, and inline code remain intact while long prose runs are reduced.
fn compress_prose(text: &str, rate: f32) -> String {
    let rate = rate.clamp(0.0, 1.0);
    if rate >= 0.999 {
...
fn protected_span_at(line: &str, start: usize) -> Option<(usize, &str)> {
    ...
    is_source_span(token).then_some((end, token))
}
...
///
/// `model_env` / `tokenizer_env` are the values of `$MEMBRANE_PUSH_LLMLINGUA_MODEL`
/// / `$MEMBRANE_PUSH_LLMLINGUA_TOKENIZER`, or `None` if unset. Pulled out of
...
fn llmlingua_tokenizer_path() -> Result<std::path::PathBuf, String> {
    ...
    let tokenizer_env = std::env::var("MEMBRANE_PUSH_LLMLINGUA_TOKENIZER").ok();
...
fn llmlingua_asset_paths() -> Result<(std::path::PathBuf, std::path::PathBuf), String> {
    let manifest = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let model_env = std::env::var("MEMBRANE_PUSH_LLMLINGUA_MODEL").ok();
    let tok_env = std::env::var("MEMBRANE_PUSH_LLMLINGUA_TOKENIZER").ok();
    let (model, _tokenizer, default_dir) =
        resolve_asset_paths(&manifest, model_env.as_deref(), tok_env.as_deref());

    if !model.is_file() {
        return Err(format!(
            "model not found at {} (set $MEMBRANE_PUSH_LLMLINGUA_MODEL, or populate {} from the Push asset bundle; see {})",
            model.display(),
            default_dir.display(),
            default_dir.join("README.md").display()
        ));
    }
...
mod tests {
    ...
    fn missing_model_error_mentions_default_dir_and_readme_pointer() {
        ...
        let bogus = std::env::temp_dir()
            .join("membrane-push-asset-test-DO-NOT-EXIST")
            .join("model.onnx");
        // SAFETY: this test is the only writer of `MEMBRANE_PUSH_LLMLINGUA_MODEL`
        ...
        unsafe {
            std::env::set_var("MEMBRANE_PUSH_LLMLINGUA_MODEL", &bogus);
        }
        let result = llmlingua_asset_paths();
        unsafe {
            std::env::remove_var("MEMBRANE_PUSH_LLMLINGUA_MODEL");
        }
        match result {
            Err(msg) => {
                let manifest = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"));
