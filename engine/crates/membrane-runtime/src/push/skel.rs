...
pub fn skeletonize_to_budget(path: &Path, src: &str, budget_tokens: usize) -> BudgetSkeleton {
    let input_tokens = crate::push::compress::estimate_tokens(src);
    let signature = skeletonize(path, src);
    if crate::push::compress::estimate_tokens(&signature) <= budget_tokens {
    ...
    let public = signature
        ...
        .collect::<Vec<_>>()
        .join("\n");
    if !public.is_empty() && crate::push::compress::estimate_tokens(&public) <= budget_tokens {
        return budget_result(
            public,
            src,
            path,
...
fn budget_result(
    ...
    let output_tokens = crate::push::compress::estimate_tokens(&text);
    BudgetSkeleton {
        text,
        input_tokens,
        output_tokens,
