...
fn outline_token_threshold() -> usize {
    ...
    const PUSH_CONTEXT_TOKEN_LIMIT: usize = 2_048;
    PUSH_CONTEXT_TOKEN_LIMIT.saturating_sub(448)
}

/// `(verb, before, after, meta)` Push observations for a manifest — shared by the CLI and serve
/// telemetry paths. skel rows carry bytes; compress rows carry token counts (meta marks the unit);
/// copy/missing carry 0s.
