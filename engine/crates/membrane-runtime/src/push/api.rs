...
pub fn execute_with_control(operation: &str, arguments: &Value,
    ...
    // A supplied H8 is strictly parsed before the operation runs. A missing,
    // malformed, inexact or identity-unbindable observation is a typed
    // refusal below — it is never silently discarded to obtain success.
    let supplied_ceiling = if arguments.get("remainingContextCeiling").is_some() {
        ...
        Some(match (request_session, task_id) {
            (Some(request_session), Some(task_id)) => crate::pull::selection::parse_request_time_h8(arguments, request_session, task_id),
            // Without request identity the observation cannot be bound to
            // this request at all, so it is not usable host evidence.
            _ => Err(crate::pull::selection::RequestTimeH8Error::Missing),
        })
    } else { None };
    // A supplied-but-invalid H8 refuses on every operation, including
    // identity-free `probe`; the unmeasured fallback cannot launder it.
    let ceiling = match supplied_ceiling {
        Some(Ok(ceiling)) => Some(ceiling),
        Some(Err(_)) => return failure(operation, "push_h8_invalid"),
        None => None,
    };
    let result = (|| -> Result<Value, String> {
        ...
        let task_id = if requires_identity {
            ...
            // A supplied request-time ceiling (H8) was already strictly
            // parsed above: an invalid observation returned `push_h8_invalid`
            // before this point, so `ceiling` is `Some` exactly when the
            // observation validated. `push_session_binding_denied` below is
            // reserved for identity mismatches independent of host evidence.
