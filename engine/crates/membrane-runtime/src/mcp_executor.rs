...
fn lifecycle_result(name: &str, result: Result<Value, crate::cortex_lifecycle::LifecycleError>) -> Value {
...
fn native_action_for(name: &str, arguments: &Value) -> &'static str {
    match name {
        ...
        "membrane_memory" => if arguments["operation"] == "checkpoint_promote" { "proposal" } else { "source_read" },
...
impl NativeMcpExecutor for RuntimeMcpExecutor {
    fn execute(&self, name: &str, arguments: &Value) -> Value {
        if let Err(message) = membrane_mcp::validate_arguments(name, arguments) {
            return error(name, membrane_mcp::invalid_envelope_code(name), message);
        }
        ...
        match name {
            ...
            "membrane_knowledge_review" => lifecycle_result(name,
                crate::cortex_lifecycle::review(&self.store, repository, scope, &arguments["review"])),
            ...
            "membrane_temporal_fact" => {
                ...
                if operation == "record" {
                    ...
                    let emission = json!({"kind":"temporal","scopeId":scope,
                        "text":serde_json::to_string(&fact).unwrap_or_default(),"fact":fact});
                    lifecycle_result(name, crate::cortex_lifecycle::propose(
                        &self.store, repository, scope, &emission))
...
impl RuntimeMcpExecutor {
    fn propose(&self, name: &str, arguments: &Value, repository: &str, scope: &str) -> Value {
        lifecycle_result(name, crate::cortex_lifecycle::propose(
            &self.store, repository, scope, &arguments["emission"]))
    }

    fn memory_operation(&self, name: &str, args: &Value, repository: &str, scope: &str) -> Value {
        let id = args.get("id").and_then(Value::as_str).unwrap_or("");
        let result = match args["operation"].as_str().unwrap_or("") {
            "get" => crate::cortex_lifecycle::resolve_memory(&self.store, scope, id,
                args.get("expectedContentHash").and_then(Value::as_str).unwrap_or(""),
                args.get("offset").and_then(Value::as_u64).unwrap_or(0) as usize,
                args.get("maxChars").and_then(Value::as_u64).unwrap_or(12000) as usize),
            "proposal_status" => crate::cortex_lifecycle::proposal_status(&self.store, repository, scope, id),
            "checkpoint_promote" => crate::cortex_lifecycle::promote_checkpoint(&self.store, repository, scope, id),
