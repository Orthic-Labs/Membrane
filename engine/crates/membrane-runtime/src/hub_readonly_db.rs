...
//! sentinel). Each producer needs the same local crypt-engine database; this
...
/// Resolves the local crypt-engine database path. `CRYPT_DB_PATH` overrides
/// for tests/alternate installs; otherwise the workspace-relative default.
pub fn configured_db_path() -> PathBuf {
    std::env::var_os("CRYPT_DB_PATH")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            configured_workspace_root().join("tools/.cache/memory/crypt-engine.db")
        })
}
...
#[cfg(test)]
pub(crate) fn open_readonly_at(path: &std::path::Path) -> Option<Connection> {
    Connection::open_with_flags(path, OpenFlags::SQLITE_OPEN_READ_ONLY).ok()
}
