"""Crypt provider: the binary defaults to the resolvable workspace exe, not bare `crypt`
...
def _load():
    spec = importlib.util.spec_from_file_location("crypt_provider_mod", _HERE / "crypt.py")
...
def test_default_bin_resolves_workspace_binary_when_present():
    mod = _load()
    os.environ.pop("CRYPT_BIN", None)
    workspace = Path(__file__).resolve().parents[4]
    exe = workspace / "tools" / "bin" / ("crypt.exe" if os.name == "nt" else "crypt")
    resolved = mod._default_bin()
    if exe.exists():
        assert resolved == str(exe), "must resolve the real workspace binary, not bare 'crypt'"
    else:
        assert resolved == "crypt"  # graceful fallback in a bare CI checkout


def test_explicit_env_wins():
    mod = _load()
    os.environ["CRYPT_BIN"] = "/custom/crypt"
    try:
        assert mod._default_bin() == "/custom/crypt"
    finally:
        os.environ.pop("CRYPT_BIN", None)
...
def test_replay_uses_observer_timeout_and_preserves_serve_timing(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("RIGHTCONTEXT_SAMPLE_SOURCE", "replay")
...
def test_replay_no_legacy_rejects_serve_failure_without_cli_fallback(
    ...
    with pytest.raises(
        RuntimeError,
        match="replay crypt serve unavailable; CLI fallback disabled",
    ):
