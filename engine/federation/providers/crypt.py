"""Crypt provider — invokes the Rust `crypt memory-candidates` CLI.

Per dispatch: Crypt remains durable memory only. This provider reads
...
def _ccs_from_serve(task: str, scope: str, max_candidates: int, scope_descriptor: dict | None = None) -> dict | None:
    ...
    the on-mode latency fix: the cold `crypt memory-candidates` CLI reloads fastembed (~3.6s) and
...
def _default_bin() -> str:
    """Resolve the crypt binary. Bare `crypt` is a shim that Python's subprocess cannot
    resolve in the gateway's interpreter on Windows (no PATHEXT/.cmd resolution) — that silently
    returned 0 memory candidates. Default to the real workspace binary; an explicit CRYPT_BIN
    env still wins. crypt.py -> providers -> federation -> crypt -> tools -> <workspace>."""
    env = os.environ.get("CRYPT_BIN")
    if env:
        return env
    workspace = Path(__file__).resolve().parents[4]
    exe = workspace / "tools" / "bin" / ("crypt.exe" if os.name == "nt" else "crypt")
    return str(exe) if exe.exists() else "crypt"
...
def produce_with_observability(
    ...
    if ccs is None:
        if _replay_no_legacy():
            raise RuntimeError(
                "replay crypt serve unavailable; CLI fallback disabled"
            )
        raise RuntimeError(
            "crypt serve unavailable; prompt-path CLI fallback disabled"
        )
