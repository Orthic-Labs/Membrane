"""MemRight provider: the binary defaults to the resolvable workspace exe, not bare `memright`
...
def _load():
    spec = importlib.util.spec_from_file_location("memright_provider_mod", _HERE / "memright.py")
...
def test_default_bin_resolves_workspace_binary_when_present():
    mod = _load()
    os.environ.pop("MEMRIGHT_BIN", None)
    workspace = Path(__file__).resolve().parents[4]
    exe = workspace / "tools" / "bin" / ("memright.exe" if os.name == "nt" else "memright")
    resolved = mod._default_bin()
    if exe.exists():
        assert resolved == str(exe), "must resolve the real workspace binary, not bare 'memright'"
    else:
        assert resolved == "memright"  # graceful fallback in a bare CI checkout


def test_explicit_env_wins():
    mod = _load()
    os.environ["MEMRIGHT_BIN"] = "/custom/memright"
    try:
        assert mod._default_bin() == "/custom/memright"
    finally:
        os.environ.pop("MEMRIGHT_BIN", None)
...
def test_replay_no_legacy_rejects_serve_failure_without_cli_fallback(
    ...
    with pytest.raises(
        RuntimeError,
        match="replay memright serve unavailable; CLI fallback disabled",
    ):
