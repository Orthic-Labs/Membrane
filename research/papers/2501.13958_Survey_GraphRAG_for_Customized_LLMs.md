> Source: arXiv:2501.13958 · https://arxiv.org/abs/2501.13958
...
Qinggang Zhang, Shengyuan Chen, Yuanchen Bei, Zheng Yuan, Huachi Zhou, Zijin Hong, Hao Chen, Yilin Xiao, Chuang Zhou, Junnan Dong, and Xiao Huang are with the The Hong Kong Polytechnic University, Hong Kong SAR, China (e-mail: _{_ qinggangg.zhang, yzheng.yuan, huachi.zhou, zijin.hong, yilin.xiao, chuang-qqzj.zhou, hanson.dong _}_ @connect.polyu.hk, _{_ iyuanchenbei, sundaychenhao _}_ @gmail.com, _{_ sheng-yuan.chen, xiao.huang _}_ @polyu.edu.hk). Yi Chang is with the School of Artificial Intelligence, Jilin University, Changchun, China (e-mail: yichang@jlu.edu.cn). 
...
Knowledge Organization SKETCH<br>Knowledge Retrieval FMEA-KG<br>Graph-CoT  EtD<br>Knowledge IntegrationBenchmark LLAGAGraphGPT CogMG<br>GRAG<br>SimpleQuestion KQAPro FACTKG CRUD UltraDomain<br>Spider<br>WebQ Mintaka TutorQA KGL<br>WebQSP RiTeK<br>CWQ<br>LC-QuAD MuseGraph<br>LC-QuAD v2 InstructGLM<br>MetaQA GLRec DialogGSR<br>MetaQA-3 GNP MEG<br>GrailQA GNN-RAG TOA-KG<br>CencerKG GoR SubgraphRAG<br>Chain-of-Knowledge G-Retriever Graph of Record<br>MindMap RD-P LightRAG MedGraphRAG<br>StructuGraphRAG  KnowGPT CodexGraph<br>KGTransformerLARK RuleRAG GraphRAG ToG-2.0 StructRAG<br>CoK ToG KGP Graph-ret ProLLM<br>GraphRetriever SURGE SayPlan KGR PG-RAG<br>QUEST RHO RoG KELP GraphCoder<br>HotpotQA  AVIS<br><!-- End of picture text -->



<!-- Start of picture text -->
Fig. 1: The development trends in the field of GraphRAG with representative works.<br><!-- End of picture text -->

beyond simple text chunk retrieval to offer more sophisticated knowledge augmentation approaches. These include hierarchical RAG that preserves document structure through multilevel retrieval [12], [13], re-ranking systems that implement two-stage retrieval for higher recall and precision [14], [15], self-querying RAG that automatically decomposes complex queries [16], and adaptive RAG that dynamically adjusts retrieval strategies based on query types [17], [18]. These strategies advance naive RAG systems by improving context awareness, retrieval accuracy, and handling complex queries. 
...
Another key challenge involves **integrating domain knowledge from distributed sources** . Domain knowledge is usually collected from different sources, such as textbooks, research papers, industry reports, technical manuals, and maintenance 
...
The last challenge relates to system **efficiency and scalability** . The entire RAG pipeline - from initial corpus preprocessing and indexing to real-time retrieval and generation - faces significant efficiency bottlenecks [24], [25]. The external knowledge base contains a lot of domain-irrelevant information, while domain-specific terminologies are always sparsely distributed over these documents. RAG systems can be computationally expensive and time-consuming [24], especially when dealing with large-scale knowledge sources, as the model needs to 

3 

search through vast amounts of unstructured text to find relevant information. Moreover, real-time retrieval and cross-document reasoning can introduce considerable latency, negatively impacting user experience. The scalability of RAG is further constrained by declining retrieval quality and accuracy as the size of the knowledge base grows [25], thereby limiting its practical deployment in extensive and dynamic environments. 
...
## _A. Traditional RAG Pipeline_ 

A RAG framework begins by retrieving relevant information from pre-constructed external knowledge bases based on 
...
<!-- Start of picture text -->
sen  1 sen  2 sen  3 ✓ The Great Lakes are a group of five<br>User Query Non-graph RAG vec vec vec 123 CalculationSimilarity Chunks … Query Similarity… Retrievedchunks ✓ large freshwater lakes in North America. They are Lake Superior, Lake Michigan, Lake Huron, […]Organized Texts IntegrationDirect  Lake Superior, Lake Huron, Lake Erie, Lake Ontario<br>Which of the five  Great  Ranking<br>Lakes are shared by the  Text Chunking<br>United States and  Canada ? Great Lakes Fisheries<br>Constructing Customized KGs Refinement Canada Lake Superior United States Lake Erie Canada… Prompt DesignTailored Lake Superior, Lake Huron, Lake Erie, Lake Ontario<br>Entity/Relation Mexico USA<br>Extraction Relational Triples Knowledge Graph Subgraph Retrieval Subgraph Pruning<br>✓<br>✓ Lake Superior,<br>Corpus Indexing with Graphs Topic NodeExtraction … Topic Linking… Fact Linking Query RankingTopic Topic Node Selection Text SummarizationText Summarization ✓ Retrieved Retrieved  ✓ Fine-tuning Lake Huron, Lake Erie, Lake Ontario<br>Knowledge Organization Knowledge Retrieval Knowledge Integration<br><!-- End of picture text -->
...
_2) Knowledge retrieval:_ The knowledge retrieval stage encompasses various methods and strategies designed to efficiently access and retrieve the necessary knowledge from pre-organized repositories, ensuring the selection of relevant information that can enhance the quality of generated outputs. 

Current RAG systems usually involve retrieval methods such as k-nearest neighbor retrieval (KNN), term frequencyinverse document frequency (TF-IDF), and best matching 25 (BM25) to retrieve the relevant content. RETRO [31] employs KNN to extract approximate relevant neighbors from the conducted key-value database by calculating the L2 distance. RETROprompt [40] extends this approach with a few-shot knowledge store, tailoring it for more advanced prompts. 

In addition, some specialized techniques are used prior to the retrieval method to improve the accuracy and efficiency of the retrieval. To capture multifaceted aspects of the query, GAR [41] introduces diverse context generation, enriching the initial query with additional contexts before applying BM25 retrieval. Enhancing this framework, EAR [42] implements a re-ranking process that selects the optimal candidate from multiple expanded queries to improve the retrieval accuracy. Furthermore, in tackling the computational challenges associated with exact retrieval methods like BM25, Doostmohammadi et al. [43] propose a hybrid approach to identify approximate neighbors using sentence transformers for representation and then apply BM25 for re-ranking, effectively balancing accuracy with computational efficiency on large-scale retrieval tasks. Overall, knowledge retrieval combines innovative method- 
...
<!-- Start of picture text -->
Graphs for<br>GNN-ret [44], PG-RAG [45], KGP [46], GraphCoder [47], AVIS [48], SayPlan [49] HotpotQA [50], GraphRetriever [51],<br>Knowledge<br>KAR [26], GRAG [52], KET-RAG [53], PIKE-RAG [54], OG-RAG [55], ArchRAG [56], RAPTOR [18], HippoRAG2 [57]<br>Indexing<br>Knowledge Graph StructRAG [13], GraphRAG [24], FastRAG [58], QUEST [59], Structure-guided Prompt [60]<br>Knowledge Graphs as Construction from Corpus LightRAG [25], MedRAG [61], GraphReader [62], LuminiRAG [63], HippoRAG [64]<br>Organization Knowledge<br>(§IV) Carriers GraphRAG with Existing KnowGPT [5], ToG [21], ToG-2.0 [22], RoG [65], KGR [66], KELP [67], ProLLM [68],<br>Knowledge Graphs SubGraphRAG [69], PoG [70], LEGO-GraphRAG [71], KnowNet [72]<br>Hybrid<br>GoR [73], MedGraphRAG [74], KG2RAG [75], HYBGRAG [76], CodexGraph [77]<br>GraphRAG<br>PG-RAG [45], GraphCoder [47], MedGraphRAG [74], StructuGraphRAG [78],<br>Similarity-based Retriever<br>CancerKG [79], G-Retriever [80]<br>Logical-based Retriever RoG [65], RD-P [81], RuleRAG [82], KGL [83], RiTeK [84]<br>Retrieval<br>Technique GNN-based Retriever GNN-Ret [44], SURGE [85], GNN-RAG [86]<br>LLM-based Retriever ToG [21], LightRAG [25], KGP [46], MEG [87], TQA-KG [88], E 2 GraphRAG [89]<br>Knowledge<br>Retrieval RL-based Retriever KnowGPT [5], Spider [90], GraphRAG-R1 [91]<br>(§V)<br>Multi-round Retrieval GoR [73], DialogGSR [92], Graph-CoT [93]<br>Retrieval<br>Post-retrieval KGR [66], Readi [94], GCR [95], CoK [96]<br>Strategy<br>Hybrid Retrieval StructRAG [13], ToG 2.0 [22], HYBGRAG [76]<br>Node-level Knowledge SKETCH [97], GraphGPT [98]<br>Fine-tuning Path-level Knowledge RoG [65], GLRec [99], KGTransformer [100], MuseGraph [101]<br>Subgraph-level<br>Knowledge GRAG [52], RHO [102], GNP [103], InstructGLM [104], LLAGA [105]<br>Knowledge<br>Integration<br>(§VI)<br>Graph-enhanced Think-on-Graph [21], Graph-CoT [93], LARK [106], Chain-of-Knowledge [107],<br>In-context Chain-of-Thought MindMap [108], KnowledGPT [109]<br>Learning<br>Collaborative Knowledge<br>KELP [67], FMEA-KG [110], EtD [111], PoG [70], CogMG [112]<br>Graph Refinement<br>GraphRAGforCustomizedLLMs<br><!-- End of picture text -->

Fig. 3: The taxonomy for existing GraphRAG methods in the survey. 

ologies with diverse data-sourcing strategies, underpinning the generation of informed and contextually relevant outputs. 

_3) Knowledge Integration:_ The knowledge integration phase in RAG frameworks is crucial for synthesizing coherent and accurate responses based on both retrieved and inherent knowledge. At this stage, researchers utilize LLM to generate the output and employ several preprocessing and efficiency strategies to improve its quality and efficiency. 

The quality of retrieved content can drastically affect generation, with irrelevant or misleading data potentially having deleterious effects. LeanContext [113] addresses this issue by utilizing reinforcement learning to selectively choose sentences that are most pertinent to the query, effectively minimizing the context size and reducing computational costs. Similarly, SELF-RAG [16] introduces a self-reflection mechanism where the language model assesses its own generated and retrieved content, allowing for corrective measures to be implemented during the generation process. SKR [114] proposes a dynamic framework whereby LLMs can rely on their pre-trained knowledge for recognized queries, resorting to retrieval only when necessary. FILCO [115] enhances this by training a context-filtering model that screens out irrelevant data, thereby mitigating hallucination risks. Recent research by Lyu et al. [116] introduces an evaluation method specifically designed to assess the importance of the retrieved content. Their findings suggest that by pruning or re-weighting parts of the retrieval corpus, RAG systems can enhance their performance without 

requiring additional training rounds. In scenarios where the retrieval corpus is limited, approaches such as Selfmem [117] construct a memory pool from LLM-generated results, using an iterative selection framework to enhance generative quality. SAIL [118] forms an instruction-tuning dataset based on retrieval outcomes, fine-tuning LLMs to ground responses in reliable content while excluding distracting elements. 
...
## _B. Limitations of traditional RAG_ 
...
<!-- Start of picture text -->
Query: Traditional RAG: GraphRAG:<br>" How does the  El Niño phenomenon  potentially<br>Related Entities:impact  El Niño phenomenon marine biodiversityMarine biodiversity? " Text Chunks … Vector matching Query … El Niño phenomenon  Marine food web El Niño weather Marine biodiversity Marine species Might have missed Oceanic upwelling Oceanic phytoplankton Ecological balance relationshipscomplexCapture El Niño phenomenon Marine biodiversity Marine speciesOceanic upwelling Marine food webOceanic phytoplankton Ecological balance<br>Oceanic upwelling  Marine food web<br>Oceanic phytoplankton  Unstructured information retrieval may lead to missing.  Graph structures offer rich semantic context and nuanced connections.<br>Marine species Ecological balance<br>Multi-hop Relationships: El Niño phenomenon  Oceanic upwelling Oceanic phytoplankton  2 nd3 rd1 st4 st5 st … Ranked Documents Reasoning biodiversityEl Niño phenomenon  ? Marine  food webMarine Marine species biodiversityMarine  ? Multi-hopreasoning Path Reasoning Oceanic upwelling  El Niño phenomenon  phytoplankton  Marine biodiversity Oceanic food webMarineOceanic upwelling Oceanic phytoplankton Marine speciesEcological balance biodiversityMarine<br>Marine food web<br>Marine biodiversity Hard to reason about distributed domain knowledge in long inputs. Graph structures support sophisticated reasoning across multiple nodes.<br>Marine species<br>Ecological balance<br>Multi-hop Reasoning: Vector Database Scaling uplarger databaseRetrieve in High costincreases Efficiency &scalability Graph Database Scaling uplarger databaseRetrieve in increasesLow cost<br>- El Niño phenomenon impacts  oceanic<br>upwelling . Computationally and time consuming for large vector databases. Graph databases optimize for relationship-based queries and faster retrieval.<br>- Changes in  upwelling  affect the levels of<br>oceanic phytoplankton -  marine biodiversity Oceanic phytoplankton . .  levels influence  Image ✗ ✗ Graph Data Flexibility in A sub-graph Image ✓ ✓ Graph Data<br>Text ✓ phenomenon El Niño  ✗ Numerical Data knowledgesources Text ✓ phenomenon El Niño  ✓ Numerical Data<br>Answer:<br>"The  El Niño phenomenon reduces nutrient- Difficult to integrate with multiple data modality. Graph support multiple data modality integration and real-time update.<br>rich upwelling , leading to a decline in<br>oceanic phytoplankton  populations. As<br>phytoplankton form the base of the  food web affect the entire , their reduction can significantly  marine biodiversity ,  leading  marine  Text AnswerReasoning path Response I‘m still confused… Interpret-ability Text AnswerReasoning path Paths in Graph Response I‘m convinced.<br>to decreased population sizes in  marine  Detailed explanation Detailed explanation<br>species  and potential disruptions in<br>ecological balance ." Can only provide textual information to interpret the answer. Graph structures enhance response interpretability and transparency.<br><!-- End of picture text -->



<!-- Start of picture text -->
Fig. 4: The illustration of the comparison between traditional RAG and GraphRAG.<br><!-- End of picture text -->

query to generate the final output. The goal of the integration process is to seamlessly incorporate the retrieved knowledge into the generated text, thereby enhancing its quality and informativeness. A key design consideration is how to preserve the richness of the retrieved subgraph information in the final text-based prompt without introducing redundancy or misplacing emphasis on less critical aspects of the context. 

## _D. Traditional RAG vs GraphRAG_ 
...
The inherent challenges associated with the construction and maintenance of these graphs raise several research questions, 
...
Research in this area can be categorized into two main directions based on the source of the knowledge graphs: _GraphRAG with KGs constructed from corpus_ , which transforms unstructured text into structured graph representations, and _GraphRAG with existing KGs_ , which focus on how to effectively leverage well-established KGs. Below we present the details of each direction and discuss the limitations of them. 

_1) Knowledge Graph Construction from Corpus:_ This research direction aims to transform unstructured text corpora into structured KGs, enabling efficient and precise information retrieval. It heavily relies on _Open Information Extraction_ techniques, which automate the domain-independent extraction of relational facts from large corpora. These techniques include both traditional OIE approaches [127]–[130] and advanced OIE methods based on LLMs [131], [132]. However, constructing effective KGs from domain-specific corpora remains challenging due to variations in ontology across domains. A key objective in this area is to incorporate inductive biases to design graph structures that capture essential domain-specific information while minimizing unnecessary complexity. 

In scenarios where the query prompt consists of short passages and no external corpus is provided, Structure-guided Prompts [60] generate logically coherent responses by categorizing reasoning tasks into six types based on logical dependencies and organizing the passage’s information using tailored graph structures specific to each type. Other studies have focused on scaling language models to handle multidocument inputs through dynamically constructed knowledge graphs. For instance, [133] transformed sentences into relational triples via Coreference Resolution and Open Information Extraction, subsequently reducing graph redundancy by merging nodes and edges based on TF-IDF overlap. Similarly, AutoKG [134] constructs a weighted knowledge graph where nodes represent keywords extracted from the original text, and edges, evaluated through graph Laplacian learning, represent relationships with assigned weights. To offer more informative knowledge augmentation, QUEST [59], GraphRAG [24], and GraphReader [62] construct attributed KGs where nodes are enriched with side information. Specifically, QUEST emphasizes integrating entities, relations, types, and semantic alignments, while GraphRAG leverages an LLM to generate community summaries as enriched knowledge for related entities, subsequently using these summaries to produce partial answers for the entities. Similarly, GraphReader employs an LLM to summarize text chunks into atomic facts and extract key elements (nouns, verbs, and adjectives). Each key element, combined with its relevant atomic facts, forms a 

node, with links established between nodes sharing elements in the same atom facts. To leverage LLMs for customer service, [135] propose using RAG to retrieve and integrate historical issues. They create an attributed knowledge graph where nodes represent past issues and edges denote their connections. Each node includes a tree structure of the issue’s information, capturing both inter-issue and intra-issue relationships to improve question answering. Recent studies have extended graph structures to more sophisticated forms. For example, StructRAG [13] considers five candidate structure types tailored to different knowledge-intensive tasks: tables for statistical tasks, graphs for long-chain tasks, algorithms for planning tasks, catalogs for summarizing tasks, and chunks for simple single-hop tasks. A DPO-based router is trained to determine the optimal structure for representing knowledge in each case. 

_2) GraphRAG with existing KGs:_ This line of research focuses on utilizing well-established knowledge graphs, which can be either domain-specific—such as Lynx [136] for multilingual compliance services in the legal domain, AceKG [137] for academic applications, SPOKE [138] for biomedical applications, STRING [139] for protein-protein interaction prediction, — or general-purpose, like DBpedia [140] and YAGO [141]. The central challenge is to develop planning algorithms that dynamically retrieve reasoning paths or subgraphs from these KGs, tailored to the specific characteristics of the queries and the underlying graph structures. Researchers are investigating techniques to efficiently navigate these large-scale graphs, aiming to extract meaningful insights while ensuring the retrieved information is both relevant and concise. 

The retrieval of reasoning paths typically involves generating relation paths grounded in KGs as plans, which are then used to extract factual knowledge from the KGs in the form of reasoning chains (relation paths with anchored entities). ProLLM [68] applies graph-based RAG to predict proteinprotein interactions, modeling signaling pathways as reasoning processes where biological signals pass from upstream proteins through intermediates to downstream proteins via the shortest path. Approaches such as RoG [65] and KGR [66] directly utilize LLMs as planning agents to identify optimal plans. To address the low recall of reasoning paths caused by the incompleteness of KGs, ToG [21] employs a beam search algorithm to dynamically explore the most probable relational triples, forming coherent reasoning chains. Its successor, ToG2 [22], enables joint retrieval from a KG and textual documents, through alternating between logic chain extensions on KGs that explore neighboring entities of current ones, and contextual knowledge expansion from retrieved relevant documents. In the biomedical domain, KG-RAG [142] focuses on retrieving accurate and trustworthy biomedical contexts. It optimizes the retrieval process by using techniques such as disease entity recognition, context pruning, and leveraging a biomedical KG [138]. To optimize the planning process, KnowGPT [5] formulates the exploration of plans as a planning problem and solves it using deep reinforcement learning. Meanwhile, KELP [67] adopts a simpler semantic-matching strategy, encoding both the question and candidate knowledge paths in an encoder fine-tuned on a latent semantic space, and selecting the knowledge paths with the smallest semantic 

10 

TABLE I: Representative knowledge retrieval techniques and strategies used in different GraphRAG Systems. 

||Catagory|GraphRAG Model||Input|Im|plementation Details||Output|
|---|---|---|---|---|---|---|---|---|
||||Query Side|Graph Side|Query/Graph Preprocess|Matching|Pruning method||
||Similarity-based|StruGraphRAG [78]<br>CancerKG [79]<br>G-Retriever [80]<br>PG-RAG [45]<br>GraphCoder [47]<br>MedGraphRAG [74]|query embedding<br>keywords<br>query embedding<br>query embedding<br>query slice<br>query|entity embedding<br>entity<br>entity/relation embedding<br>Pseudo-Graph<br>CCG node<br>entity description|BERT/BERT<br>NA/NA<br>SentenceBert/SentenceBert<br>SentenceBert/SentenceBert<br>CodeBERT/CodeBERT<br>SentenceBert/SentenceBert|similarity calculation<br>TF-IDF<br>k-nearest neighbors<br>depth-first search<br>Jaccard index<br>top-down search|✗<br>✗<br>PCST<br>PGR<br>subgraph edit distance<br>bottom-up refine|literal context<br>tabluar results<br>subgraph<br>literal context, path<br>code snippet<br>triple|
|Techniques|Logical-based|RoG [65]<br>RD-P [81]<br>RuleRAG [82]<br>KGL [83]<br>RiTeK [84]|query embedding<br>query embedding<br>query embedding<br>query embedding<br>query embedding|relation path<br>topic entity<br>rule bank<br>clarification path<br>triple graph|Llama2/path generation<br>RoBERTa/RoBERTa<br>Llama2/rule mining<br>Roberta/path generation<br>SentenceBERT/SentenceBERT|breadth-first search<br>path expansion<br>similarity calculation<br>similarity calculation<br>MCTS/R-MCTS|LLM agent<br>path discriminator<br>✗<br>✗<br>LLM agent|reasoning path<br>reasoning path<br>literal context, path<br>literal context<br>reasoning path|
|Retrieval|GNN-based|GNN-Ret [44]<br>SURGE [85]<br>GNN-RAG [86]|subquestions<br>query embedding<br>query embedding|graph feature<br>triple embedding<br>graph feature|SentenceBERT/RGNN<br>T5-small/GNN<br>NA/GNN|DPR<br>similarity calculation<br>similarity calculation|✗<br>DHT<br>✗|subgraph<br>triple<br>literal context|
||LLM-based|KGP [46]<br>Graph RAG [24]<br>ToG [21]<br>LightRAG [25]<br>MEG [87]<br>TQA-KG [88]|query embedding<br>query<br>query embedding<br>keywords<br>query embedding<br>keyword|passage (node description)<br>community summary<br>relation path<br>entity, global keys<br>graph feature<br>triple|T5/T5<br>✗/community detection<br>Llama2/relation exploration<br>GPT-4o/GPT-4o<br>SapBERT/SapBERT<br>entity extract/✗|TF-IDF<br>LLM agent<br>beam search<br>keyword-search<br>token generation<br>keyword-search|LLM agent<br>✗<br>LLM agent<br>global keyword match<br>disambiguation<br>LLM agent|literal context<br>literal context<br>reasoning path<br>literal context<br>literal context<br>subgraph|
||RL-based|KnowGPT [5]<br>Spider [90]|query embedding<br>query embedding|entity/relation embedding<br>entity embedding|BERT<br>NA|similarity calculation<br>similarity calculation|RL agent<br>RL agent|reasoning path<br>subgraph|
|trategy|Multi-round|DialogGSR [92]<br>Graph-CoT [93]<br>GoR [73]|query embedding<br>query embedding<br>query embedding|linearized graph<br>entity embedding<br>entity embedding|T5/T5<br>Llama2/Llama2<br>Mixtral-7B/GAT|subgraph generation<br>similarity calculation<br>similarity calculation|✗<br>LLM agent<br>✗|linearized subgraph<br>subgraph<br>literal context|
|rieval S|Post-retrieval|CoK [96]<br>KGR [66]|pseudo evidence<br>claim|triple<br>entity, triple|NA/NA<br>claim extraction/NA|F2-Verification<br>claim verification|✗<br>✗|final answer<br>final answer|
|Ret<br>|Hybrid Retrieval|StructRAG [13]<br>ToG 2.0 [22]|query embedding<br>keyword|entity embedding<br>entity, relation|Qwen2/Qwen2<br>keyword extract/NA|similarity calculation<br>similarity calculation|LLM agent<br>entity/relation prune|literal context<br>literal context, path|



distance. To balance precision and recall in reasoning chain retrieval, subgraphRAG [69] introduces a lightweight multilayer perceptron to extract subgraphs, discovering key facts within the KGs. This method allows the subgraph size to be adjusted, achieving an efficiency-effectiveness trade-off. More details about the retrieval techniques are presented in Section V. 

_3) Limitations:_ Although using graphs as knowledge carriers is efficient and effective, these methods are limited in several aspects: ( _i_ ) **Lack of high-quality KGs.** For directly using KGs as external knowledge bases, this line of research is constrained by the availability of high-quality KGs. Constructing KGs is resource-intensive, and although powerful KG refinement techniques [143]–[148] have been developed to improve KG completeness, most publicly available KGs remain far from comprehensive. Furthermore, the absence of a unified ontology poses challenges to designing transferable planning algorithms. These limitations have motivated recent research to focus on constructing KGs from corpora. However, this line of work introduces additional challenges: ( _ii_ ) **Trade-off between efficiency and effectiveness.** When constructing KGs from text corpora, the granularity of the extracted knowledge plays a crucial role in balancing efficiency and effectiveness. Preserving fine-grained information results in larger-scale KGs, which may hinder computational efficiency. Conversely, compact KGs may sacrifice important details, leading to potential information loss. This trade-off complicates the knowledge summarization process. Moreover, approaches like GraphRAG [24], which leverage LLMs for knowledge summarization, suffer from the high token costs and unbearable high runtime overhead. 

## _C. Hybrid GraphRAG_ 

This paradigm utilizes graph structures both as carriers of knowledge and as indexing tools. A common approach involves constructing a graph that encapsulates key information from the original text, with each node linked to corresponding text 

chunks. These text chunks act as a complementary knowledge source, providing detailed contextual information. 
...
## _A. Overall Pipeline of Knowledge Retriever_ 
...
|---|---|---|---|---|
|Node-level|SKETCH [97]<br>GraphGPT [98]|Node attribute<br>Node attribute & Graph Embeddings|Concatenate Node Attribute<br>Learning Node Embeddings|LLAMA3-8B<br>Baichuan-7B|
|Path-level|GLRec [99]<br>KGTransformer [100]<br>MuseGraph [101]<br>RoG [65]|User-item description<br>Sampled Knowledge Graph Sequence<br>Node Path & Node attribute<br>Knowledge Graph Path|Path Weight Calculation<br>None<br>COT<br>Instruction Generation|LLAMA-7B<br>Transformers<br>LLAMA-7B<br>LLaMA2-Chat-7B|
||RHO [102]|KG Embeddings & KG SubGraph|None|BART|
|Subgraph-level|GNP [103]<br>InstructGLM [104]<br>LLAGA [105]|Graph & Query Embeddings<br>Node attribute & Graph Embeddings<br>Node attribute & Graph Embeddings|None<br>Flatten subgraph to sequence<br>Flatten subgraph to sequence|FLAN-T5-xlarge<br>LLAMA-7B<br>Vicuna-7B|



which can be summarized as _Graph+Online Web Resource RAG_ [168]–[173]. Further, K2 [174] explores the LLM tuning with multiple retrieval sources, such as the textual corpus, self-instructed LLM knowledge, and online web resources. In future works, hybrid RAG with multiple retrieval resources simultaneously can be a potential research direction for complex LLM system building under a wide range of knowledge. 

optimization. (i) Prompt format choice: The choice of prompt format is crucial for knowledge integration. This is because LLMs are highly sensitive to the prompt format. For example, the order of examples in in-context learning can lead to different responses [178], [179]. (ii) LLM response optimization: Then the appropriate prompt format joins the retrieved content and the questions as input to prompt the LLMs for response generation or further optimization. 
...
## _A. Overall Pipeline of Knowledge Integration_ 
...
> [2] T. Brown, B. Mann, N. Ryder, and et al., “Language models are fewshot learners,” in _Advances in Neural Information Processing Systems_ , 2020. 

16 

- [3] D. Khashabi, S. Min, T. Khot, and et al., “Unifiedqa: Crossing format boundaries with a single qa system,” in _Findings of Empirical Methods in Natural Language Processing_ , 2020. 

- [4] A. Chowdhery, S. Narang, J. Devlin, and et al., “Palm: Scaling language modeling with pathways,” _Journal of Machine Learning Research (JMLR)_ , 2023. 

- [5] Q. Zhang, J. Dong, H. Chen, and et al., “Knowgpt: Knowledge graph based prompting for large language models,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [6] E. J. Hu, yelong shen, P. Wallis, and et al., “LoRA: Low-rank adaptation of large language models,” in _International Conference on Learning Representations_ , 2022. 

- [7] J. He, C. Zhou, X. Ma, and et al., “Towards a unified view of parameterefficient transfer learning,” in _International Conference on Learning Representations_ , 2022. 

- [8] Z. Gekhman, G. Yona, R. Aharoni, , and et al., “Does fine-tuning llms on new knowledge encourage hallucinations?” in _Empirical Methods in Natural Language Processing_ , 2024. 

- [9] Y. Zhai, S. Tong, X. Li, and et al., “Investigating the catastrophic forgetting in multimodal large language model fine-tuning,” in _Conference on Parsimony and Learning_ , 2024. 

- [10] P. Lewis, E. Perez, A. Piktus, and et al., “Retrieval-augmented generation for knowledge-intensive nlp tasks,” in _Advances in Neural Information Processing Systems_ , 2020. 

- [11] Q. Zhang, Z. Xiang, Y. Xiao, L. Wang, J. Li, X. Wang, and J. Su, “Faithfulrag: Fact-level conflict modeling for context-faithful retrievalaugmented generation,” _arXiv preprint arXiv:2506.08938_ , 2025. 

- [12] R. Chen, X. Zhang, J. Wu, and et al., “Multi-level querying using a knowledge pyramid,” _arXiv preprint arXiv:2407.21276_ , 2024. 

- [13] Z. Li, X. Chen, H. Yu, and et al., “Structrag: Boosting knowledge intensive reasoning of llms via inference-time hybrid information structurization,” in _International Conference on Learning Representations_ , 2025. 

- [14] M. Glass, G. Rossiello, M. F. M. Chowdhury, and et al., “Re2g: Retrieve, rerank, generate,” in _North American Chapter of the Association for Computational Linguistics_ , 2022. 

- [15] F. Xu, W. Shi, and E. Choi, “Recomp: Improving retrieval-augmented lms with compression and selective augmentation,” in _International Conference on Learning Representations_ , 2024. 

- [16] A. Asai, Z. Wu, Y. Wang, and et al., “Self-rag: Learning to retrieve, generate, and critique through self-reflection,” in _International Conference on Learning Representations_ , 2024. 

- [17] X. Tang, Q. Gao, J. Li, and et al., “Mba-rag: a bandit approach for adaptive retrieval-augmented generation through question complexity,” in _International Conference on Computational Linguistics_ , 2025. 

- [18] P. Sarthi, S. Abdullah, A. Tuli, and et al., “Raptor: Recursive abstractive processing for tree-organized retrieval,” in _International Conference on Learning Representations_ , 2024. 

- [19] C. Ling, X. Zhao, J. Lu, and et al., “Domain specialization as the key to make large language models disruptive: A comprehensive survey,” _arXiv preprint arXiv:2305.18703_ , 2023. 

- [20] T. Bruckhaus, “Rag does not work for enterprises,” _arXiv preprint arXiv:2406.04369_ , 2024. 

- [21] J. Sun, C. Xu, L. Tang, and et al., “Think-on-graph: Deep and responsible reasoning of large language model on knowledge graph,” in _International Conference on Learning Representations_ , 2024. 

- [22] S. Ma, C. Xu, X. Jiang, and et al., “Think-on-graph 2.0: Deep and interpretable large language model reasoning with knowledge graph-guided retrieval,” in _International Conference on Learning Representations_ , 2025. 

- [23] A. Anthropic, “The claude 3 model family: Opus, sonnet, haiku,” _Claude3 Model Card_ , 2024. 

- [24] D. Edge, H. Trinh, N. Cheng, and et al., “From local to global: A graph rag approach to query-focused summarization,” _arXiv preprint arXiv:2404.16130_ , 2024. 

- [25] Z. Guo, L. Xia, Y. Yu, and et al., “Lightrag: Simple and fast retrievalaugmented generation,” _arXiv preprint arXiv:2410.05779_ , 2024. 

- [26] L. Liang, M. Sun, Z. Gui, and et al., “Kag: Boosting llms in professional domains via knowledge augmented generation,” in _International World Wide Web Conference_ , 2025. 

- [27] B. Peng, Y. Zhu, Y. Liu, and et al., “Graph retrieval-augmented generation: A survey,” _arXiv preprint arXiv:2408.08921_ , 2024. 

- [28] K. Jeon and G. Lee, “Hybrid large language model approach for prompt and sensitive defect management: A comparative analysis of hybrid, nonhybrid, and graphrag approaches,” _Advanced Engineering Informatics_ , 2025. 

- [29] T. T. Procko and O. Ochoa, “Graph retrieval-augmented generation for large language models: A survey,” in _Conference on AI, Science, Engineering, and Technology (AIxSET)_ , 2024. 

- [30] H. Han, Y. Wang, H. Shomer, and et al., “Retrieval-augmented generation with graphs (graphrag),” _arXiv preprint arXiv:2501.00309_ , 2024. 

- [31] S. Borgeaud, A. Mensch, J. Hoffmann, and et al., “Improving language models by retrieving from trillions of tokens,” in _International Conference on Machine Learning_ , 2022. 

- [32] G. Izacard, P. Lewis, M. Lomeli, and et al., “Atlas: Few-shot learning with retrieval augmented language models,” _Journal of Machine Learning Research (JMLR)_ , 2023. 

- [33] Z. Jiang, F. F. Xu, L. Gao, and et al., “Active retrieval augmented generation,” in _Empirical Methods in Natural Language Processing_ , 2023. 

- [34] Y. Gao, Y. Xiong, X. Gao, and et al., “Retrieval-augmented generation for large language models: A survey,” _arXiv preprint arXiv:2312.10997_ , 2023. 

- [35] X. Wang, Z. Wang, X. Gao, and et al., “Searching for best practices in retrieval-augmented generation,” in _Empirical Methods in Natural Language Processing_ , 2024. 

- [36] W. Yu, H. Zhang, X. Pan, and et al., “Chain-of-note: Enhancing robustness in retrieval-augmented language models,” in _Empirical Methods in Natural Language Processing_ , 2024. 

- [37] Z. Zhong, H. Liu, X. Cui, and et al., “Mix-of-granularity: Optimize the chunking granularity for retrieval-augmented generation,” in _International Conference on Computational Linguistics_ , 2025. 

- [38] LlamaIndex, “Llamaindex - build knowledge assistants over your enterprise data,” _Accessed_ , 2024. 

- [39] Z. Wang, H. Yuan, W. Dong, and et al., “Corag: A cost-constrained retrieval optimization system for retrieval-augmented generation,” _arXiv preprint arXiv:2411.00744_ , 2024. 

- [40] X. Chen, L. Li, N. Zhang, and et al., “Decoupling knowledge from memorization: Retrieval-augmented prompt learning,” in _Advances in Neural Information Processing Systems_ , 2022. 

- [41] Y. Mao, P. He, X. Liu, and et al., “Generation-augmented retrieval for open-domain question answering,” in _Association for Computational Linguistics_ , 2021. 

- [42] Y.-S. Chuang, W. Fang, S.-W. Li, and et al., “Expand, rerank, and retrieve: Query reranking for open-domain question answering,” in _Findings of Association for Computational Linguistics_ , 2023. 

- [43] E. Doostmohammadi, T. Norlund, M. Kuhlmann, and et al., “Surfacebased retrieval reduces perplexity of retrieval-augmented language models,” in _Association for Computational Linguistics_ , 2023. 

- [44] Z. Li, Q. Guo, J. Shao, and et al., “Graph neural network enhanced retrieval for question answering of llms,” in _North American Chapter of the Association for Computational Linguistics_ , 2025. 

- [45] X. Liang, S. Niu, S. Zhang, and et al., “Empowering large language models to set up a knowledge retrieval indexer via self-learning,” _arXiv preprint arXiv:2405.16933_ , 2024. 

- [46] Y. Wang, N. Lipka, R. A. Rossi, and et al., “Knowledge graph prompting for multi-document question answering,” in _Conference on Artificial Intelligence_ , 2024. 

- [47] W. Liu, A. Yu, D. Zan, and et al., “Graphcoder: Enhancing repositorylevel code completion via code context graph-based retrieval and language model,” _IEEE/ACM International Conference on Automated Software Engineering (ASE)_ , 2024. 

- [48] Z. Hu, A. Iscen, C. Sun, and et al., “Avis: Autonomous visual information seeking with large language model agent,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [49] K. Rana, J. Haviland, S. Garg, and et al., “Sayplan: Grounding large language models using 3d scene graphs for scalable robot task planning,” in _Conference on Robot Learning_ , 2023. 

- [50] W. Xiong, X. L. Li, S. Iyer, and et al., “Answering complex opendomain questions with multi-hop dense retrieval,” _arXiv preprint arXiv:2009.12756_ , 2020. 

- [51] S. Min, D. Chen, L. Zettlemoyer, and et al., “Knowledge guided text retrieval and reading for open domain question answering,” _arXiv preprint arXiv:1911.03868_ , 2019. 

- [52] Y. Hu, Z. Lei, Z. Zhang, and et al., “Grag: Graph retrieval-augmented generation,” in _Findings of North American Chapter of the Association for Computational Linguistics_ , 2025. 

- [53] Y. Huang, S. Zhang, and X. Xiao, “Ket-rag: A cost-efficient multigranular indexing framework for graph-rag,” in _Proceedings of the 31st ACM SIGKDD Conference on Knowledge Discovery and Data Mining V. 2_ , 2025, pp. 1003–1012. 

17 

- [54] J. Wang, J. Fu, R. Wang, and et al., “Pike-rag: specialized knowledge and rationale augmented generation,” _arXiv preprint arXiv:2501.11551_ , 2025. 

- [55] K. Sharma, P. Kumar, and Y. Li, “Og-rag: Ontology-grounded retrievalaugmented generation for large language models,” _arXiv preprint arXiv:2412.15235_ , 2024. 

- [56] S. Wang, Y. Fang, Y. Zhou, X. Liu, and Y. Ma, “Archrag: Attributed community-based hierarchical retrieval-augmented generation,” _arXiv preprint arXiv:2502.09891_ , 2025. 

- [57] B. J. Gutierrez,´ Y. Shu, W. Qi, S. Zhou, and Y. Su, “From rag to memory: Non-parametric continual learning for large language models,” in _Forty-second International Conference on Machine Learning_ , 2025. 

- [58] A. Abane, A. Bekri, and A. Battou, “Fastrag: Retrieval augmented generation for semi-structured data,” _arXiv preprint_ , 2024. 

- [59] X. Lu, S. Pramanik, R. Saha Roy, and et al., “Answering complex questions by joining multi-document evidence with quasi knowledge graphs,” in _Conference on Information Retrieval_ , 2019. 

- [60] K. Cheng, N. K. Ahmed, T. Willke, and et al., “Structure guided prompt: Instructing large language model in multi-step reasoning by exploring graph structure of the text,” in _Empirical Methods in Natural Language Processing_ , 2024. 

- [61] X. Zhao, S. Liu, S.-Y. Yang, and et al., “Medrag: Enhancing retrievalaugmented generation with knowledge graph-elicited reasoning for healthcare copilot,” in _International World Wide Web Conference_ , 2025. 

- [62] S. Li, Y. He, H. Guo, and et al., “GraphReader: Building graph-based agent to enhance long-context abilities of large language models,” in _Findings of Empirical Methods in Natural Language Processing_ , 2024. 

- [63] L. Martis, “Luminirag: Vision-enhanced graph rag for complex multimodal document understanding,” _Authorea Preprints_ , 2024. 

- [64] B. Jimenez Gutierrez, Y. Shu, Y. Gu, M. Yasunaga, and Y. Su, “Hipporag: Neurobiologically inspired long-term memory for large language models,” _Advances in Neural Information Processing Systems_ , vol. 37, pp. 59 532–59 569, 2024. 

- [65] L. LUO, Y.-F. Li, R. Haf, and et al., “Reasoning on graphs: Faithful and interpretable large language model reasoning,” in _International Conference on Learning Representations_ , 2024. 

- [66] X. Guan, Y. Liu, H. Lin, and et al., “Mitigating large language model hallucinations via autonomous knowledge graph-based retrofitting,” in _Conference on Artificial Intelligence_ , 2024. 

- [67] H. Liu, S. Wang, Y. Zhu, and et al., “Knowledge graph-enhanced large language models via path selection,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [68] M. Jin, H. Xue, Z. Wang, and et al., “Prollm: Protein chain-of-thoughts enhanced llm for protein-protein interaction prediction,” _bioRxiv_ , 2024. 

- [69] M. Li, S. Miao, and P. Li, “Simple is effective: The roles of graphs and large language models in knowledge-graph-based retrieval-augmented generation,” _arXiv preprint arXiv:2410.20724_ , 2024. 

- [70] L. Chen, P. Tong, Z. Jin, and et al., “Plan-on-graph: Self-correcting adaptive planning of large language model on knowledge graphs,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [71] Y. Cao, Z. Gao, Z. Li, and et al., “Lego-graphrag: Modularizing graphbased retrieval-augmented generation for design space exploration,” _arXiv preprint arXiv:2411.05844_ , 2024. 

- [72] Y. Yan, Y. Hou, Y. Xiao, and et al., “Knownet: Guided health information seeking from llms via knowledge graph integration,” _IEEE Transactions on Visualization and Computer Graphics_ , 2024. 

- [73] H. Zhang, T. Feng, and J. You, “Graph of records: Boosting retrieval augmented generation for long-context summarization with graphs,” _arXiv preprint arXiv:2410.11001_ , 2024. 

- [74] J. Wu, J. Zhu, and Y. Qi, “Medical graph rag: Towards safe medical large language model via graph retrieval-augmented generation,” _arXiv preprint arXiv:2408.04187_ , 2024. 

- [75] X. Zhu, Y. Xie, Y. Liu, and et al., “Knowledge graph-guided retrieval augmented generation,” in _North American Chapter of the Association for Computational Linguistics_ , 2025. 

- [76] M.-C. Lee, Q. Zhu, C. Mavromatis, and et al., “Hybgrag: Hybrid retrieval-augmented generation on textual and relational knowledge bases,” in _Association for Computational Linguistics_ , 2025. 

- [77] X. Liu, B. Lan, Z. Hu, and et al., “Codexgraph: Bridging large language models and code repositories via code graph databases,” in _North American Chapter of the Association for Computational Linguistics_ , 2025. 

- [78] X. Zhu, X. Guo, S. Cao, and et al., “Structugraphrag: Structured document-informed knowledge graphs for retrieval-augmented generation,” in _Conference on Artificial Intelligence_ , 2024. 

- [79] M. Gubanov, A. Pyayt, and A. Karolak, “Cancerkg. org-a web-scale, interactive, verifiable knowledge graph-llm hybrid for assisting with optimal cancer treatment and care,” in _International Conference on Information and Knowledge Management_ , 2024. 

- [80] X. He, Y. Tian, Y. Sun, and et al., “G-retriever: Retrieval-augmented generation for textual graph understanding and question answering,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [81] Y. Huang and G. Zeng, “Rd-p: A trustworthy retrieval-augmented prompter with knowledge graphs for llms,” in _International Conference on Information and Knowledge Management_ , 2024. 

- [82] Z. Chen, C. Xu, D. Wang, and et al., “Rulerag: Rule-guided retrievalaugmented generation with language models for question answering,” _arXiv preprint arXiv:2410.22353_ , 2024. 

- [83] Y. Yang, S. Chen, Y. Zhu, and et al., “Intelligent question answering for water conservancy project inspection driven by knowledge graph and large language model collaboration,” _LHB_ , 2024. 

- [84] J. Huang, M. Li, Z. Yao, and et al., “Ritek: A dataset for large language models complex reasoning over textual knowledge graphs,” _arXiv preprint arXiv:2410.13987_ , 2024. 

- [85] M. Kang, J. M. Kwak, J. Baek, and et al., “Knowledge graph-augmented language models for knowledge-grounded dialogue generation,” _arXiv preprint arXiv:2305.18846_ , 2023. 

- [86] Y. Dong, S. Wang, H. Zheng, and et al., “Advanced rag models with graph structures: Optimizing complex knowledge reasoning and text generation,” in _International Symposium on Computer Engineering and Intelligent Communications (ISCEIC)_ , 2024. 

- [87] L. Cabello, C. Martin-Turrero, U. Akujuobi, and et al., “Meg: Medical knowledge-augmented large language models for question answering,” _arXiv preprint arXiv:2411.03883_ , 2024. 

- [88] M. He, A. Zhou, and X. Shi, “Enhancing textbook question answering with knowledge graph-augmented large language models,” in _Asian Conference on Machine Learning_ , 2024. 

- [89] Y. Zhao, J. Zhu, Y. Guo, K. He, and X. Li, “Eˆ 2graphrag: Streamlining graph-based rag for high efficiency and effectiveness,” _arXiv preprint arXiv:2505.24226_ , 2025. 

- [90] T. Shen, E. Cambria, J. Wang, and et al., “Insight at the right spot: Provide decisive subgraph information to graph llm with reinforcement learning,” _Information Fusion_ , 2025. 

- [91] C. Yu, K. Zhao, Y. Li, H. Chang, M. Feng, X. Jiang, Y. Sun, J. Li, Y. Zhang, J. Li _et al._ , “Graphrag-r1: Graph retrieval-augmented generation with process-constrained reinforcement learning,” _arXiv preprint arXiv:2507.23581_ , 2025. 

- [92] J. Park, M. Joo, J.-K. Kim, and et al., “Generative subgraph retrieval for knowledge graph-grounded dialog generation,” in _Empirical Methods in Natural Language Processing_ , 2024. 

- [93] B. Jin, C. Xie, J. Zhang, and et al., “Graph chain-of-thought: Augmenting large language models by reasoning on graphs,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [94] S. Cheng, Z. Zhuang, Y. Xu, and et al., “Call me when necessary: Llms can efficiently and faithfully reason over structured environments,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [95] L. Luo, Z. Zhao, C. Gong, and et al., “Graph-constrained reasoning: Faithful reasoning on knowledge graphs with large language models,” _arXiv preprint arXiv:2410.13080_ , 2024. 

- [96] J. Wang, Q. Sun, N. Chen, and et al., “Boosting language models reasoning with chain-of-knowledge prompting,” in _Association for Computational Linguistics_ , 2024. 

- [97] C. Zhou, Z. Wang, S. Chen, and et al., “Large language models based graph convolution for text-attributed networks,” in _International Conference on Learning Representations Submission_ , 2024. 

- [98] J. Tang, Y. Yang, W. Wei, and et al., “Graphgpt: Graph instruction tuning for large language models,” in _Conference on Information Retrieval_ , 2024. 

- [99] L. Wu, Z. Qiu, Z. Zheng, and et al., “Exploring large language model for graph data understanding in online job recommendations,” in _Conference on Artificial Intelligence_ , 2024. 

- [100] W. Zhang, Y. Zhu, M. Chen, and et al., “Structure pretraining and prompt tuning for knowledge graph transfer,” in _International World Wide Web Conference_ , 2023. 

- [101] Y. Tan, H. Lv, X. Huang, and et al., “Musegraph: Graph-oriented instruction tuning of large language models for generic graph mining,” _arXiv preprint arXiv:2403.04780_ , 2024. 

- [102] Z. Ji, Z. Liu, N. Lee, and et al., “Rho: Reducing hallucination in open-domain dialogues with knowledge grounding,” in _Findings of Association for Computational Linguistics_ , 2023. 

- [103] Y. Tian, H. Song, Z. Wang, and et al., “Graph neural prompting with large language models,” in _Conference on Artificial Intelligence_ , 2024. 

18 

- [104] R. Ye, C. Zhang, R. Wang, and et al., “Language is all a graph needs,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [105] R. Chen, T. Zhao, A. K. JAISWAL, and et al., “LLaGA: Large language and graph assistant,” in _International Conference on Machine Learning_ , 2024. 

- [106] N. Choudhary and C. K. Reddy, “Complex logical reasoning over knowledge graphs using large language models,” _arXiv preprint arXiv:2305.01157_ , 2023. 

- [107] X. Li, R. Zhao, Y. K. Chia, and et al., “Chain-of-knowledge: Grounding large language models via dynamic knowledge adapting over heterogeneous sources,” in _International Conference on Learning Representations_ , 2024. 

- [108] Y. Wen, Z. Wang, and J. Sun, “Mindmap: Knowledge graph prompting sparks graph of thoughts in large language models,” in _Association for Computational Linguistics_ , 2024. 

- [109] X. Wang, Q. Yang, Y. Qiu, and et al., “Knowledgpt: Enhancing large language models with retrieval and storage access on knowledge bases,” _arXiv preprint arXiv:2308.11761_ , 2023. 

- [110] L. Bahr, C. Wehner, J. Wewerka, and et al., “Knowledge graph enhanced retrieval-augmented generation for failure mode and effects analysis,” _arXiv preprint arXiv:2406.18114_ , 2024. 

- [111] G. Liu, Y. Zhang, Y. Li, and et al., “Explore then determine: A gnn-llm synergy framework for reasoning over knowledge graph,” _arXiv preprint arXiv:2406.01145_ , 2024. 

- [112] T. Zhou, Y. Chen, K. Liu, and et al., “Cogmg: Collaborative augmentation between large language model and knowledge graph,” _arXiv preprint arXiv:2406.17231_ , 2024. 

- [113] M. A. Arefeen, B. Debnath, and S. Chakradhar, “Leancontext: Costefficient domain-specific question answering using llms,” _Natural Language Processing Journal_ , 2024. 

- [114] Y. Wang, P. Li, M. Sun, and et al., “Self-knowledge guided retrieval augmentation for large language models,” in _Findings of Empirical Methods in Natural Language Processing_ , 2023. 

- [115] Z. Wang, J. Araki, Z. Jiang, and et al., “Learning to filter context for retrieval-augmented generation,” _arXiv preprint arXiv:2311.08377_ , 2023. 

- [116] X. Lyu, S. Grafberger, S. Biegel, and et al., “Improving retrievalaugmented large language models via data importance learning,” _arXiv preprint arXiv:2307.03027_ , 2023. 

- [117] X. Cheng, D. Luo, X. Chen, and et al., “Lift yourself up: Retrievalaugmented text generation with self-memory,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [118] H. Luo, Y.-S. Chuang, Y. Gong, and et al., “Sail: Search-augmented instruction learning,” in _Findings of Empirical Methods in Natural Language Processing_ , 2023. 

- [119] W. Jiang, S. Zhang, B. Han, and et al., “Piperag: Fast retrievalaugmented generation via algorithm-system co-design,” in _Conference on Knowledge Discovery and Data Mining_ , 2025. 

- [120] A. Lazaridou, E. Gribovskaya, W. Stokowiec, and et al., “Internetaugmented language models through few-shot prompting for opendomain question answering,” _arXiv preprint arXiv:2203.05115_ , 2022. 

- [121] Z. Li, R. Guo, and S. Kumar, “Decoupled context processing for context augmented language modeling,” in _Advances in Neural Information Processing Systems_ , 2022. 

- [122] M. De Jong, Y. Zemlyanskiy, N. FitzGerald, and et al., “Pre-computed memory or on-the-fly encoding? a hybrid approach to retrieval augmentation makes the most of your compute,” in _International Conference on Machine Learning_ , 2023. 

- [123] J. Liu, L. Li, T. Xiang, and et al., “Tcra-llm: Token compression retrieval augmented large language model for inference cost reduction,” in _Findings of Empirical Methods in Natural Language Processing_ , 2023. 

- [124] C. Jin, Z. Zhang, X. Jiang, and et al., “Ragcache: Efficient knowledge caching for retrieval-augmented generation,” _arXiv preprint arXiv:2404.12457_ , 2024. 

- [125] S. Pan, L. Luo, Y. Wang, and et al., “Unifying large language models and knowledge graphs: A roadmap,” _IEEE Transactions on Knowledge and Data Engineering_ , 2024. 

- [126] M. Jin, Q. Yu, D. Shu, and et al., “The impact of reasoning step length on large language models,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [127] A. Yates, M. Banko, M. Broadhead, and et al., “Textrunner: open information extraction on the web,” in _North American Chapter of the Association for Computational Linguistics: Human Language Technologies_ , 2007. 

- [128] G. Angeli, M. J. J. Premkumar, and C. D. Manning, “Leveraging linguistic structure for open domain information extraction,” in _Association for Computational Linguistics and International Joint Conference on Natural Language Processing_ , 2015. 

- [129] W. Li, H. Zhou, J. Dong, and et al., “Constructing low-redundant and high-accuracy knowledge graphs for education,” in _International Conference on Web-Based Learning (ICWL)_ , 2022. 

- [130] K. Kolluru, V. Adlakha, S. Aggarwal, and et al., “Openie6: Iterative grid labeling and coordination analysis for open information extraction,” in _Empirical Methods in Natural Language Processing_ , 2020. 

- [131] R. Cohen, M. Geva, J. Berant, and et al., “Crawling the internal knowledge-base of language models,” in _Findings of European Chapter of the Association for Computational Linguistics_ , 2023. 

- [132] H. Chen, X. Shen, Q. Lv, and et al., “Sac-kg: Exploiting large language models as skilled automatic constructors for domain knowledge graphs,” in _Association for Computational Linguistics_ , 2024. 

- [133] A. Fan, C. Gardent, C. Braud, and et al., “Using local knowledge graph construction to scale Seq2Seq models to multi-document inputs,” in _Empirical Methods in Natural Language Processing and International Joint Conference on Natural Language Processing_ , 2019. 

- [134] B. Chen and A. L. Bertozzi, “Autokg: Efficient automated knowledge graph generation for language models,” in _International Conference on Big Data (BigData)_ , 2023. 

- [135] Z. Xu, M. J. Cruz, M. Guevara, and et al., “Retrieval-augmented generation with knowledge graphs for customer service question answering,” in _Conference on Information Retrieval_ , 2024. 

- [136] E. Montiel-Ponsoda and V. Rodriguez-Doncel, “Lynx: building the legal knowledge graph for smart compliance services in multilingual europe,” in _International Conference on Language Resources and Evaluation_ , 2018. 

- [137] R. Wang, Y. Yan, J. Wang, and et al., “Acekg: A large-scale knowledge graph for academic data mining,” in _International Conference on Information and Knowledge Management_ , 2018. 

- [138] J. H. Morris, K. Soman, R. E. Akbas, and et al., “The scalable precision medicine open knowledge engine (spoke): a massive knowledge graph of biomedical information,” _Bioinformatics_ , 2023. 

- [139] D. Szklarczyk, A. L. Gable, D. Lyon, and et al., “String v11: protein– protein association networks with increased coverage, supporting functional discovery in genome-wide experimental datasets,” _Nucleic acids research_ , 2019. 

- [140] S. Auer, C. Bizer, and G. Kobilarov, “DBpedia: A nucleus for a web of open data,” in _The semantic web_ , 2007. 

- [141] F. M. Suchanek, M. Alam, T. Bonald, and et al., “Yago 4.5: A large and clean knowledge base with a rich taxonomy,” in _Conference on Information Retrieval_ , 2024. 

- [142] K. Soman, P. W. Rose, J. H. Morris, and et al., “Biomedical knowledge graph-optimized prompt generation for large language models,” _Bioinformatics_ , 2024. 

- [143] C. Shengyuan, Y. Cai, H. Fang, and et al., “Differentiable neurosymbolic reasoning on large-scale knowledge graphs,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [144] S. Chen, Q. Zhang, J. Dong, and et al., “Entity alignment with noisy annotations from large language models,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [145] ——, “Neuro-symbolic entity alignment via variational inference,” _arXiv preprint arXiv:2410.04153_ , 2024. 

- [146] Q. Zhang, J. Dong, K. Duan, and et al., “Contrastive knowledge graph error detection,” in _International Conference on Information and Knowledge Management_ , 2022. 

- [147] J. Dong, Q. Zhang, X. Huang, and et al., “Active ensemble learning for knowledge graph error detection,” in _International Conference on Web Search and Data Mining_ , 2023. 

- [148] Q. Zhang, J. Dong, Q. Tan, and et al., “Integrating entity attributes for error-aware knowledge graph embedding,” _IEEE Transactions on Knowledge and Data Engineering_ , 2023. 

- [149] S. Selva Birunda and R. Kanniga Devi, “A review on word embedding techniques for text classification,” _Innovative Data Communication Technologies and Application_ , 2021. 

- [150] T. Kenter and M. De Rijke, “Short text similarity with word embeddings,” in _International Conference on Information and Knowledge Management_ , 2015. 

- [151] Z. Lee, Z. Huang, Z. Yao, and et al., “Diakop: Dialogue-based knowledge-oriented programming for neural-symbolic knowledge base question answering,” in _International Conference on Information and Knowledge Management_ , 2024. 

19 

- [152] W. Hamilton, Z. Ying, and J. Leskovec, “Inductive representation learning on large graphs,” in _Advances in Neural Information Processing Systems_ , 2017. 

- [153] Y. Bei, S. Zhou, Q. Tan, and et al., “Reinforcement neighborhood selection for unsupervised graph anomaly detection,” in _International Conference on Data Mining (ICDM)_ , 2023. 

- [154] Z. Li, X. Sun, Y. Luo, Y. Zhu, D. Chen, Y. Luo, X. Zhou, Q. Liu, S. Wu, L. Wang _et al._ , “Gslb: the graph structure learning benchmark,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [155] Z. Liu, C. Shengyuan, K. Zhou, and et al., “Rsc: accelerate graph neural networks training via randomized sparse computations,” in _International Conference on Machine Learning_ , 2023. 

- [156] M. Zhang and Y. Chen, “Link prediction based on graph neural networks,” in _Advances in Neural Information Processing Systems_ , 2018. 

- [157] H. Chen, Y. Bei, W. Huang, and et al., “Graph cross-correlated network for recommendation,” _IEEE Transactions on Knowledge and Data Engineering_ , 2024. 

- [158] Y. Bei, H. Chen, S. Chen, and et al., “Non-recursive cluster-scale graph interacted model for click-through rate prediction,” in _International Conference on Information and Knowledge Management_ , 2023. 

- [159] C.-Y. Hsu, K. Cox, J. Xu, and et al., “Thought graph: Generating thought process for biological reasoning,” in _International World Wide Web Conference_ , 2024. 

- [160] P. Panda, A. Agarwal, C. Devaguptapu, and et al., “Holmes: Hyperrelational knowledge graphs for multi-hop question answering using llms,” in _Association for Computational Linguistics_ , 2024. 

- [161] Y. Li, Z. Li, P. Wang, and et al., “A survey of graph meets large language model: Progress and future directions,” in _International Joint Conference on Artificial Intelligence_ , 2024. 

- [162] Y. Li, P. Wang, Z. Li, and et al., “Zerog: Investigating cross-dataset zeroshot transferability in graphs,” in _Conference on Knowledge Discovery and Data Mining_ , 2024. 

- [163] J. Li, X. Sun, Y. Li, and et al., “Graph intelligence with large language models and prompt learning,” in _Conference on Knowledge Discovery and Data Mining_ , 2024. 

- [164] Z. Wang, S. X. Teo, J. Ouyang, and et al., “M-rag: Reinforcing large language model performance through retrieval-augmented generation with multiple partitions,” in _Association for Computational Linguistics_ , 2024. 

- [165] M. Kulkarni, P. Tangarajan, K. Kim, and et al., “Reinforcement learning for optimizing rag for domain chatbots,” _arXiv preprint arXiv:2401.06800_ , 2024. 

- [166] N. Gupta, S. Kirtania, P. Gupta, and et al., “Stackfeed: Structured textual actor-critic knowledge base editing with feedback,” _arXiv preprint arXiv:2410.10584_ , 2024. 

- [167] B. Sarmah, B. Hall, R. Rao, and et al., “Hybridrag: Integrating knowledge graphs and vector retrieval augmented generation for efficient information extraction,” _ACM International Conference on AI in Finance_ , 2024. 

- [168] W. Xie, X. Liang, Y. Liu, and et al., “Weknow-rag: An adaptive approach for retrieval-augmented generation integrating web search and knowledge graphs,” _arXiv preprint_ , 2024. 

- [169] R. Mendes, D. Oliveira, and V. Garcia, “Application of generative ai as an enterprise wikibase knowledge graph q&a system,” in _Knowledge Graphs and Large Language Models (KaLLM)_ , 2024. 

- [170] M. Alhanahnah, Y. Boshmaf, and B. Baudry, “Depesrag: Towards managing software dependencies using large language models,” _arXiv preprint arXiv:2405.20455_ , 2024. 

- [171] M. Dehghan, M. A. Alomrani, S. Bagga, and et al., “Ewek-qa: Enhanced web and efficient knowledge graph retrieval for citation-based question answering systems,” in _Association for Computational Linguistics_ , 2024. 

- [172] Y. Zhao, Z. Jin, F. Zhang, and et al., “Bridging the gap: Advancing commonsense question answering with integrated multi-modal knowledge,” in _Cognitive Science Society (CSS)_ , 2024. 

- [173] F. F. Bayat, K. Qian, B. Han, and et al., “Fleek: Factual error detection and correction with evidence retrieved from external knowledge,” in _Empirical Methods in Natural Language Processing_ , 2023. 

- [174] C. Deng, T. Zhang, Z. He, and et al., “K2: A foundation language model for geoscience knowledge understanding and utilization,” in _International Conference on Web Search and Data Mining_ , 2024. 

- [175] Y. Li, Y. Yu, C. Liang, and et al., “Loftq: LoRA-fine-tuning-aware quantization for large language models,” in _International Conference on Learning Representations_ , 2024. 

- [176] Z. Hu, L. Wang, Y. Lan, and et al., “Llm-adapters: An adapter family for parameter-efficient fine-tuning of large language models,” in _Empirical Methods in Natural Language Processing_ , 2023. 

- [177] X. Lin, W. Wang, Y. Li, and et al., “Data-efficient fine-tuning for llm-based recommendation,” in _Conference on Information Retrieval_ , 2024. 

- [178] J. He, M. Rungta, D. Koleczek, and et al., “Does prompt formatting have any impact on llm performance?” _arXiv preprint arXiv:2411.10541_ , 2024. 

- [179] I. Arawjo, C. Swoopes, P. Vaithilingam, and et al., “Chainforge: A visual toolkit for prompt engineering and llm hypothesis testing,” in _Conference on Human Factors in Computing Systems_ , 2024. 

- [180] X. Huang, K. Han, Y. Yang, and et al., “Can gnn be good adapter for llms?” in _International World Wide Web Conference_ , 2024. 

- [181] Y. Li, Z. Li, K. Zhang, and et al., “Chatdoctor: A medical chat model fine-tuned on a large language model meta-ai (llama) using medical domain knowledge,” _Cureus_ , 2023. 

- [182] C. Wu, W. Lin, X. Zhang, and et al., “Pmc-llama: toward building open-source language models for medicine,” _Journal of the American Medical Informatics Association_ , 2024. 

- [183] M. Fatehkia, J. K. Lucas, and S. Chawla, “T-rag: lessons from the llm trenches,” _arXiv preprint arXiv:2402.07483_ , 2024. 

- [184] Y. Zhang, K. Bao, M. Yan, and et al., “Text-like encoding of collaborative information in large language models for recommendation,” in _Association for Computational Linguistics_ , 2024. 

- [185] Y. Liang, K. Tan, T. Xie, and et al., “Aligning large language models to a domain-specific graph database,” in _International Conference on Information and Knowledge Management_ , 2024. 

- [186] C. Mavromatis and G. Karypis, “Gnn-rag: Graph neural retrieval for large language model reasoning,” in _Findings of Association for Computational Linguistics_ , 2025. 

- [187] F. Bianchini, M. Calamo, F. De Luzi, and et al., “Enhancing complex linguistic tasks resolution through fine-tuning llms, rag and knowledge graphs (short paper),” in _International Conference on Advanced Information Systems Engineering_ , 2024. 

- [188] D. Shu, T. Chen, M. Jin, and et al., “Knowledge graph large language model (kg-llm) for link prediction,” _arXiv preprint arXiv:2403.07311_ , 2024. 

- [189] N. Chen, Y. Li, J. Tang, and et al., “Graphwiz: An instruction-following language model for graph problems,” _arXiv preprint_ , 2024. 

- [190] H. Luo, Z. Tang, S. Peng, and et al., “Chatkbqa: A generate-then-retrieve framework for knowledge base question answering with fine-tuned large language models,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [191] Y. Yan, P. Zheng, and Y. Wang, “Enhancing large language model capabilities for rumor detection with knowledge-powered prompting,” _Engineering Applications of Artificial Intelligence_ , 2024. 

- [192] S. Yuan, E. Nie, M. Farber,¨ and et al., “Gnnavi: Navigating the information flow in large language models by graph neural network,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [193] R. Chen, T. Zhao, A. K. JAISWAL, and et al., “Llaga: Large language and graph assistant,” in _International Conference on Machine Learning_ , 2024. 

- [194] Z. Zhang, A. Zhang, M. Li, and et al., “Automatic chain of thought prompting in large language models,” in _International Conference on Learning Representations_ , 2023. 

- [195] L. Wang, W. Xu, Y. Lan, and et al., “Plan-and-solve prompting: Improving zero-shot chain-of-thought reasoning by large language models,” in _Association for Computational Linguistics_ , 2023. 

- [196] Z. Bi, N. Zhang, Y. Jiang, and et al., “When do program-of-thought works for reasoning?” in _Conference on Artificial Intelligence_ , 2024. 

- [197] L. Ma and J. Sanders, “Markov chains and hitting times for error accumulation in quantum circuits,” in _International Conference, VALUETOOLS 2021, Virtual Event_ , 2021. 

- [198] Y. Bai, F. Chen, H. Wang, and et al., “Transformers as statisticians: Provable in-context learning with in-context algorithm selection,” in _Advances in Neural Information Processing Systems_ , 2024. 

- [199] R. Li, Y. Wang, Z. Wen, and et al., “Different paths to the same destination: Diversifying llms generation for multi-hop open-domain question answering,” _Knowledge-Based Systems (KBS)_ , 2024. 

- [200] S. M. Xie, A. Raghunathan, P. Liang, and et al., “An explanation of in-context learning as implicit bayesian inference,” in _International Conference on Learning Representations_ , 2022. 

- [201] Y. Zhou, X. Li, Q. Wang, and et al., “Visual in-context learning for large vision-language models,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [202] Y. Zhang, K. Zhou, and Z. Liu, “What makes good examples for visual in-context learning?” in _Advances in Neural Information Processing Systems_ , 2023. 

20 

- [203] S. Xu, L. Pang, H. Shen, and et al., “Search-in-the-chain: Interactively enhancing large language models with search for knowledge-intensive tasks,” in _International World Wide Web Conference_ , 2024. 

- [204] S. Geng, Z. Fu, Y. Ge, and et al., “Improving personalized explanation generation through visualization,” in _Association for Computational Linguistics_ , 2022. 

- [205] A. Tariq, M. Luo, A. Urooj, and et al., “Domain-specific llm development and evaluation–a case-study for prostate cancer,” _medRxiv_ , 2024. 

- [206] J. Shen, N. Tenenholtz, J. B. Hall, and et al., “Tag-llm: Repurposing general-purpose llms for specialized domains,” in _International Conference on Machine Learning_ , 2024. 

- [207] C. Jeong, “Fine-tuning and utilization methods of domain-specific llms,” _arXiv preprint arXiv:2401.02981_ , 2024. 

- [208] K. Singhal, S. Azizi, T. Tu, and et al., “Large language models encode clinical knowledge,” _Nature_ , 2023. 

- [209] D. Yang, J. Rao, K. Chen, and et al., “Im-rag: Multi-round retrievalaugmented generation through learning inner monologues,” in _Conference on Information Retrieval_ , 2024. 

- [210] E. Tanwar, S. Dutta, M. Borthakur, and et al., “Multilingual llms are better cross-lingual in-context learners with alignment,” in _Association for Computational Linguistics_ , 2023. 

- [211] S. Yang, E. Gribovskaya, N. Kassner, and et al., “Do large language models latently perform multi-hop reasoning?” in _Association for Computational Linguistics_ , 2024. 

- [212] G. Liu, Y. Sun, R. Sun, and et al., “Agentps: Agentic process supervision for multi-modal content quality assurance through multi-round qa,” _arXiv preprint arXiv:2412.15251_ , 2024. 

- [213] A. Bordes, N. Usunier, S. Chopra, and et al., “Large-scale simple question answering with memory networks,” _arXiv preprint arXiv:1506.02075_ , 2015. 

- [214] J. Berant, A. Chou, R. Frostig, and et al., “Semantic parsing on freebase from question-answer pairs,” in _Empirical Methods in Natural Language Processing_ , 2013. 

- [215] A. Talmor and J. Berant, “The web as a knowledge-base for answering complex questions,” in _North American Chapter of the Association for Computational Linguistics_ , 2018. 

- [216] Y. Zhang, H. Dai, Z. Kozareva, A. Smola, and L. Song, “Variational reasoning for question answering with knowledge graph,” in _Conference on Artificial Intelligence_ , 2018. 

- [217] P. Trivedi, G. Maheshwari, M. Dubey, and et al., “Lc-quad: A corpus for complex question answering over knowledge graphs,” in _International Semantic Web Conference_ , 2017. 

- [218] S. Cao, J. Shi, L. Pan, and et al., “KQA pro: A dataset with explicit compositional programs for complex question answering over knowledge base,” in _Association for Computational Linguistics_ , 2022. 

- [219] M. Yang, Y. Wang, and Y. Gu, “Language-based reasoning graph neural network for commonsense question answering,” _Neural Networks_ , 2024. 

- [220] A. O. Saleh, G. Tur,¨ and Y. Saygin, “Sg-rag: Multi-hop question answering with large language models through knowledge graphs,” in _International Conference on Natural Language and Speech Processing (ICNLSP)_ , 2024. 

- [221] Z. Zhang, L. Wen, and W. Zhao, “A gail fine-tuned llm enhanced framework for low-resource knowledge graph question answering,” in _International Conference on Information and Knowledge Management_ , 2024. 

- [222] K. Zuo, Y. Jiang, F. Mo, and et al., “Kg4diagnosis: A hierarchical multi-agent llm framework with knowledge graph enhancement for medical diagnosis,” _arXiv preprint arXiv:2412.16833_ , 2024. 

- [223] X. Zhao, M. Blum, R. Yang, and et al., “Agentigraph: An interactive knowledge graph platform for llm-based chatbots utilizing private data,” _arXiv preprint arXiv:2410.11531_ , 2024. 

- [224] R. Yang, B. Yang, A. Feng, and et al., “Graphusion: A rag framework for knowledge graph construction with a global perspective,” in _International World Wide Web Conference_ , 2025. 

- [225] Z. Sepasdar, S. Gautam, C. Midoglu, and et al., “Soccer-graphrag: Applications of graphrag in soccer,” in _International Workshop on Graph-Based Approaches in Information Retrieval_ , 2024. 

- [226] P. Sen, A. F. Aji, and A. Saffari, “Mintaka: A complex, natural, and multilingual dataset for end-to-end question answering,” in _International Conference on Computational Linguistics_ , 2022. 

- [227] J. Kim, S. Park, Y. Kwon, and et al., “Factkg: Fact verification via reasoning on knowledge graphs,” in _Association for Computational Linguistics_ , 2023. 

- [228] W.-t. Yih, M. Richardson, C. Meek, and et al., “The value of semantic parse labeling for knowledge base question answering,” in _Association for Computational Linguistics_ , 2016. 

- [229] Y. Gu, S. Kase, M. Vanni, and et al., “Beyond iid: three levels of generalization for question answering on knowledge bases,” in _International World Wide Web Conference_ , 2021. 

- [230] Y. Lyu, Z. Li, S. Niu, and et al., “Crud-rag: A comprehensive chinese benchmark for retrieval-augmented generation of large language models,” _ACM Transactions on Information Systems (TIS)_ , 2024. 

- [231] H. Qian, P. Zhang, Z. Liu, and et al., “Memorag: Moving towards next-gen rag via memory-inspired knowledge discovery,” _arXiv preprint arXiv:2409.05591_ , 2024. 

- [232] Z. Xiang, C. Wu, Q. Zhang, S. Chen, Z. Hong, X. Huang, and J. Su, “When to use graphs in rag: A comprehensive analysis for graph retrievalaugmented generation,” _arXiv preprint arXiv:2506.05690_ , 2025. 

- [233] J. Wang, H. Lu, Y. Liu, and et al., “Llms for user interest exploration in large-scale recommendation systems,” in _Conference on Recommender Systems_ , 2024. 

- [234] S. Kim, H. Kang, S. Choi, and et al., “Large language models meet collaborative filtering: An efficient all-round llm-based recommender system,” in _Conference on Knowledge Discovery and Data Mining_ , 2024. 

- [235] Y. Xia, J. Kim, Y. Chen, and et al., “Understanding the performance and estimating the cost of llm fine-tuning,” in _International Symposium on Workload Characterization (ISWC)_ , 2024. 

- [236] W. Sun, J. Wang, Q. Guo, and et al., “Cebench: A benchmarking toolkit for the cost-effectiveness of llm pipelines,” _arXiv preprint arXiv:2407.12797_ , 2024. 

- [237] Z. Liu, X. He, Y. Tian, and et al., “Can we soft prompt llms for graph learning tasks?” in _International World Wide Web Conference_ , 2024. 

- [238] J. Huang, X. Zhang, Q. Mei, and et al., “Can llms effectively leverage graph structural information through prompts, and why?” _Transactions on Machine Learning Research (TMLR)_ , 2024. 

- [239] X. Wan, K. Xu, X. Liao, and et al., “Scalable and efficient full-graph gnn training for large graphs,” _Conference on Management of Data_ , 2023. 

- [240] B. Bi, S. Liu, Y. Wang, and et al., “Lpnl: Scalable link prediction with large language models,” in _Findings of Association for Computational Linguistics_ , 2024. 

- [241] W. Shi, S. Min, M. Yasunaga, and et al., “Replug: Retrieval-augmented black-box language models,” in _North American Chapter of the Association for Computational Linguistics_ , 2024. 

- [242] T. Schick, J. Dwivedi-Yu, R. Dess`ı, and et al., “Toolformer: Language models can teach themselves to use tools,” in _Advances in Neural Information Processing Systems_ , 2024. 

## APPENDIX 

_A. Open-Source Projects and Applications_ 
...
_1) Benchmark Datasets:_ We categorize representative datasets based on their complexity and specific characteristics: 

- _a) Simple Question Answering:_ 

- **SimpleQuestion** [213] consists of 100k questions constructed from the Freebase knowledge graph. The dataset emphasizes basic evidence retrieval without requiring complex reasoning chains. Its straightforward nature makes it ideal for evaluating baseline performance and 

21 

validating fundamental retrieval mechanisms in KGQA systems. The questions are designed to test direct fact extraction, making it a valuable resource for assessing retrieval accuracy. 
...
- **MetaQA** [216] specializes in movie domain knowledge, containing over 400k questions structured around a knowledge graph with 135k triples, 43k entities, and 9 relations. The dataset is organized into progressive difficulty levels based on reasoning hop requirements. Each question includes detailed annotations of head entities, answers, and the entities involved in the reasoning path, providing a controlled environment for evaluating multi-hop reasoning capabilities. 

- _c) Large-scale Complex QA:_ 
...
- _d) Domain-specific QA:_ 

- **Mintaka** [226] contains 20k questions across 8 different languages, focusing on logical refinement and answer revision capabilities. The dataset emphasizes cross-lingual reasoning and the ability to systematically improve answers based on available evidence. 

- **FACTKG** [227] encompasses 108,000 claims requiring validation against DBpedia. The binary classification task tests systems’ ability to verify factual accuracy 

using knowledge graph information, providing a unique perspective on knowledge validation capabilities. 

- **WebQSP** [228] features 4,737 natural language questions operating over a vast knowledge graph containing 164.6M facts and 24.9M entities. The questions are distributed across different reasoning types: 30% require two-hop aggregation, 7% involve constraint reasoning, and 63% need single-fact retrieval. 
...
- _e) Tailored GraphRAG Benchmarks:_ 
...
<u>TABLE IV: Open-source Projects and Applications.</u> 
...
|Soccer-GraphRAG [225]|i<br>Soccer|Graph Construction, QA|Soccer KG|SoccerNet-Echoes|
...
systems have proven particularly impactful. Models like KG-RAG [142], MED-RAG [61], MEG [87], and MedGraphRAG [74] are tailored to handle the intricate and highstakes nature of medical data. They rely on specialized datasets such as PubMedQA, MedQA, and UMLS, which provide rich sources of structured medical knowledge. By constructing graphs that capture relationships between medical concepts, these models can assist in a range of tasks, including medical QA, diagnostic support, and the integration of healthcare data. For instance, KG-RAG leverages knowledge graphs to link symptoms, diseases, and treatments, enabling healthcare professionals to access accurate and interpretable insights. Similarly, MedGraphRAG addresses the need for personalized healthcare solutions by integrating multimodal data from sources like MultiMedQA and DiverseHealth. 
...
lightweight architecture makes it ideal for deployment on edge devices or within constrained systems, such as IoT networks or small-scale reasoning platforms. 

Another notable adaptation is **Medical GraphRAG**<sup>6</sup> , a domain-specific variant tailored to the healthcare industry. This implementation integrates medical knowledge graphs, such as SNOMED and ICD, to provide contextually accurate and clinically relevant outputs. By bridging the gap between medical terminology and generative AI capabilities, Medical Graph RAG facilitates applications like clinical decision support and patient education, where precision and reliability are paramount. 
...
## _B. Limitations and Future Opportunities_ 
...
_2) Knowledge Integration in GraphRAG:_ The knowledge integration allows the users to inject domain-specific knowledge into LLMs. When integrating, the following aspects need to be considered for the design: 
...
Current RAG works usually involve retrieval methods such as k-nearest neighbor retrieval (KNN), term frequencyinverse document frequency (TF-IDF), and best matching 25 (BM25) to retrieve the relevant content. RETRO [31] employs KNN to extract approximate relevant neighbors from the conducted key-value database by calculating the L2 distance. RETROprompt [40] extends this approach with a few-shot knowledge store, tailoring it for more advanced prompts. 
...
the output and employ several preprocessing and efficiency strategies to improve its quality and efficiency. 
...
_3) Knowledge Integration:_ The knowledge integration phase in RAG frameworks is crucial for synthesizing coherent and accurate responses based on both retrieved and inherent knowledge. At this stage, researchers utilize LLM to generate 
