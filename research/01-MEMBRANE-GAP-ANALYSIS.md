...
**Current:** `memright-daily` is deliberately disabled at every gate boundary, so census, analysis, sync, and any future Observatory run only when invoked by hand. Gate discipline is legitimate; total operational blindness between manual runs is the cost.
**Close by:** the planned separately-named replication schedule after P3/P4 acceptance, plus scheduling the *read-only* Observatory/census lane independently now — it touches no DB writes, no sync, no `memright-daily`, so it does not violate the gate contract as documented ("read-only … does not enable or invoke memright-daily").
...
**Current:** delivery seals, read-only provenance verification, veto rail, append-only mirror, content-free exports — but: no authority levels; no influence-class separation on delivered blocks; no injection scan at `memright put`/Adapt intake (provenance regressions exist, scanning doesn't); mirror events **unsigned** (Ed25519 already on the surviving-suggestions list); quarantine is effectiveness-based, not provenance-based; recall has veto but no abstention state.
...
*(Verdict corrected from 🔴 2026-07-26 per Sol's review, confirmed against source: `memright-core/src/types.rs` implements `MemoryTier::Working → Episodic → Semantic` with promotion ranks. The tier is real; nothing automatically fills it at session end.)*
...
**Contract break found 2026-07-26 (Sol's "rebaseline required" note confirmed):** the Blueprint skill has migrated its store to a single SQLite `graph/graph.db` holding nodes, edges, docTruth, and the manifest envelope, with "no `graph.json` and no fallback to one" (`tools/skills/blueprint/SKILL.md:103`). Membrane still hard-codes the retired file contract in two readers: the federation provider reads `.agent/graph/manifest.json` / `.blueprint/manifest.json` for `generationId` (`engine/federation/providers/blueprint.py:86-87`, stub fallback at ~222) and the Rust freshness evaluator reads `.agent/graph/graph.json` for `graph_body_generation` (`engine/crates/memright/src/freshness.rs:538`, reader at 634, fixtures at `tests/freshness_test.rs:313-323`). On new-Blueprint repos the sealed-generation contract has nothing to bind, so the blueprint lane degrades lane-locally on every prompt. Repair (task filed: "Adapt membrane to Blueprint's graph.db store"): both readers pull the envelope/generation from the SQLite store with legacy paths as fallback, fixtures updated, verified by the freshness suite plus a dirty-tree federation smoke.
