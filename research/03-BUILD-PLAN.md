...
- Gate discipline is law: cohort/receipt machinery for anything behavior-changing; frozen runs never resumed; `memright-daily` stays disabled until its own gate closes. Read-only lanes may schedule independently.
...
| `tools/memright/` + `tools.memright.*` package | **retired** — those crates are now `membrane/engine/crates/memright*` | do not restore; a stale pre-migration path |
...
|---|---|---|
| B0 | `cargo test --manifest-path engine/Cargo.toml -p memright --test context_telemetry --test freshness_test` | registry/graph adapters and state manifest |
| B1 | `cd /Volumes/D/claude && uv run --with pytest --with jsonschema python tools/pipelines/memory/context-value-daily.py --db "$MEMRIGHT_DB" --output tools/.cache/metrics/context-value-daily/$(date -u +%F).json` | content-free token census JSON; `daily-sync.sh`/`context-observatory.sh` schedule it read-only |
| B2/B4 | `cd /Volumes/D/claude && uv run --with pytest --with jsonschema python tools/pipelines/memory/daily-analysis.py --db "$MEMRIGHT_DB" --output-dir tools/.cache/metrics/daily-analysis --days 1` | advisory budget crossings and cache-break diagnostics in JSON + Markdown |
| B3 | enable `MEMRIGHT_PUSH_MODE=shadow` or `on` for an approved cohort, then let `tools/hooks/post_tool_push.py` receive PostToolUse JSON | reversible spill files, audit JSONL, and opportunity-ledger receipts |
| B5 | `memright feedback ...` / existing post-turn terminal scan | `context_feedback` rows and utility-adjusted recall |
| B6 | `memright put ...`, `tools/hooks/ingest_memory.py`, and signed `sync.py` mirror events | accepted memories, content-free A0 quarantine records, and signature-verified mirror receipts |
...
1. Schedule the read-only analysis/census lane independently of `memright-daily` (02 §4 "separate schedules by risk").
