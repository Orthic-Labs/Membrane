...
def _records():
    return [
        {"id": f"morph-workflow-rule-{i:010x}",
...
def test_compile_core_emits_versioned_loadable_artifact(monkeypatch, tmp_path):
    records = _records()
    monkeypatch.setattr(core_compiler.morph_sessions, "scan_batch_for_secrets_str",
                        lambda _text: True)
    ...
    assert result["schema_version"] == morph_core.SCHEMA_VERSION
    assert result["policy_version"] == morph_core.POLICY_VERSION
    assert len(result["content_sha256"]) == 64
    loaded = morph_core.load_core(out)
    assert loaded is not None and len(loaded.source_ids) == 6


def test_compile_core_accepts_legacy_unclassified_morph_preferences(monkeypatch, tmp_path):
    records = _records()
    ...
    assert morph_core.load_core(out) is not None


def test_compile_core_rejects_unknown_provenance(monkeypatch, tmp_path):
    records = _records()
    bad = _response(records)
    bad["text"] = bad["text"].replace(records[0]["id"], "morph-unknown-source")
    monkeypatch.setattr(core_compiler.morph_sessions, "scan_batch_for_secrets_str",
                        lambda _text: True)
    try:
...
def test_morph_compile_core_cli_is_an_early_non_crypt_path(monkeypatch, tmp_path):
    out = tmp_path / "core.json"
    monkeypatch.setattr(sys, "argv", [
        "morph.py", "--compile-core", str(out), "--lane", "minimax",
        "--allow-external-lane",
    ])
    monkeypatch.setattr(morph_cli.ts, "scanner_available", lambda: True)
    monkeypatch.setattr(morph_cli.morph_llm, "lane_available", lambda _lane: True)
    monkeypatch.setattr(morph_cli, "load_rules", lambda: {r["id"]: r for r in _records()})
    calls = []
    monkeypatch.setattr(
        morph_cli.core_compiler, "compile_and_write",
        lambda records, target, **kwargs: calls.append((records, target, kwargs)) or {
            "rules": [1] * 6, "estimated_tokens": 100,
        },
    )
    monkeypatch.setattr(
        morph_cli, "_run_crypt",
        lambda _args: (_ for _ in ()).throw(AssertionError("core compile must not call Crypt")),
    )
    assert morph_cli.main() == 0
