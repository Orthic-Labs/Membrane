...
function stateText(section) { return section?.state === "unavailable" && section.reason === "not_instrumented" ? "not_configured" : section?.state || "unknown"; }
...
const stateLabel = state => ({ available: "Available", degraded: "Degraded", unavailable: "Unavailable", not_configured: "Not configured", running: "Running", offline: "Offline", unknown: "Unknown" }[state] || "Unknown");
...
export function renderOverview(snapshot, root, runtime) {
  ...
  const model = dashboardModel(data, runtime), sections = model.sections;
  const sourceRows = [["Providers", sections.providers], ["Blueprint repositories", sections.repositories]].map(([name, value]) => `<li><div><strong>${name}</strong><small>${esc(value?.reason || "No evidence")}</small></div>${pill(stateText(value))}</li>`).join("");
  const health = RESOURCES.map(name => `<li><span>${esc(name)}</span>${pill(stateText(sections[name]))}<small>${esc(sections[name]?.reason || "No evidence")}</small></li>`).join("");
