...
export const SUBSYSTEMS = ["pull", "push", "cortex", "blueprint", "guide", "adapt"];
...
export function renderOverview(snapshot, root, runtime) {
  ...
  const subsystemHealth = SUBSYSTEMS.map(name => `<li><span>${esc(name)}</span>${pill(stateText(subsystems[name]))}<small>${esc(subsystems[name]?.reason || "No evidence")}</small></li>`).join("");
