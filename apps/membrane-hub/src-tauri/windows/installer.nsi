; Membrane Hub custom NSIS template.
; Derived from Tauri's NSIS template pinned by @tauri-apps/cli 2.11.4.
; Local policy: automatic in-place same-version repair & upgrade; no maintenance choice.
...
Function PageReinstall
  ...
  ; RightKit contract: same-version repair & ordinary upgrades continue directly
  ; to Section Install.  No uninstall/reinstall maintenance choice is shown.
  ${If} $WixMode = 0
  ${AndIf} $R0 >= 0
    Abort
  ${EndIf}

  ; MEMBRANE_PATCH_START: non-WiX newer/unknown upgrades install in place.
