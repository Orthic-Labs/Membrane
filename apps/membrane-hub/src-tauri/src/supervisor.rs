...
use membrane_protocol::{
    ResidentHelloV1, ResidentLeaseV1, ResidentLifecycleFrameV1,
...
pub enum ServiceStatus { Running, Unavailable, CrashLoop }
...
impl Supervisor {
    ...
    pub fn backoff_delay(attempt: usize) -> Duration {
        ...
        BASE_BACKOFF.checked_mul(multiplier as u32).unwrap_or(MAX_BACKOFF).min(MAX_BACKOFF)
    }

    pub fn start(&self) -> Result<ServiceStatus, String> {
        {
            let mut state = self.state.lock().map_err(|_| "service_state_unavailable")?;
            if state.crash_loop { return Ok(ServiceStatus::CrashLoop); }
            if state.service.as_mut().is_some_and(|managed| managed.child.try_wait().ok().flatten().is_none()) {
        ...
        let exited_at = {
            ...
            let lifecycle_invalid = {
                ...
                while let Ok(frame) = service.frames.try_recv() {
                    match frame {
                        Ok(frame) if frame.kind == "register" && frame.state.as_deref() == Some("failed") => { invalid = true; break; }
                        Ok(frame) if frame.kind == "ack" && frame.fence == service.lease.fence => {},
                        Ok(_) | Err(_) => { invalid = true; break; }
                    }
                }
                invalid
            };
            if lifecycle_invalid { return Self::mark_unavailable(&mut state); }
    ...
    pub fn stop(&self) {
        ...
        if let Some(mut service) = service {
            let command = ResidentLifecycleFrameV1 {
                kind: "command".into(), state: None, command: Some("drain".into()),
                fence: service.lease.fence, endpoint: None, capability: Some(service.lease.capability.clone()),
            };
            ...
            while Instant::now() < deadline {
                if service.child.try_wait().ok().flatten().is_some() { return; }
    ...
    fn mark_unavailable(state: &mut State) -> ServiceStatus {
        let started_at = state.service.take().map(|mut service| {
    ...
    fn spawn(&self) -> Result<ManagedService, String> {
        if !self.executable.is_file() { return Err("membrane_hub_missing".into()); }
        let root = fs::canonicalize(&self.workspace_root).map_err(|_| "workspace_root_unavailable")?;
        let lease = self.mint_lease(&root)?;
        let hello = ResidentHelloV1 {
            kind: "hello".into(), lifecycle_version: 1, fence: lease.fence,
            installation_id: sha256_text(&root.to_string_lossy()), product_id: "membrane".into(),
            instance_id: lease.instance_id.clone(), release_generation: lease.release_generation.clone(),
            artifact_digest: lease.artifact_digest.clone(), declared_data_root: lease.declared_data_root.clone(),
        ...
        let mut child = Command::new(&self.executable)
            ...
            .spawn().map_err(|_| "membrane_hub_start_failed")?;
        let setup = (|| {
            ...
            Ok(ManagedService { child, stdin: Some(stdin), frames, lease, started_at: Instant::now() })
        })();
        match setup {
    ...
    fn mint_lease(&self, root: &Path) -> Result<ResidentLeaseV1, String> {
        let mut instance = [0_u8; 32]; let mut capability = [0_u8; 32];
        getrandom::getrandom(&mut instance).map_err(|_| "membrane_hub_random_unavailable")?;
        getrandom::getrandom(&mut capability).map_err(|_| "membrane_hub_random_unavailable")?;
        Ok(ResidentLeaseV1 {
            schema_version: RESIDENT_LEASE_SCHEMA_VERSION,
            instance_id: format!("sha256:{}", hex(&instance)), capability: hex(&capability),
    ...
    fn release_generation(&self) -> Result<String, String> {
        let output = Command::new(&self.executable).arg("build-info")
            .stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::null())
            .output().map_err(|_| "membrane_hub_release_unavailable")?;
        if !output.status.success() { return Err("membrane_hub_release_unavailable".into()); }
        serde_json::from_slice::<serde_json::Value>(&output.stdout).ok()
            .and_then(|value| value.get("release_generation").and_then(serde_json::Value::as_str).map(str::to_owned))
            .filter(|value| is_sha256_digest(value)).ok_or("membrane_hub_release_invalid".into())
    }
}
...
fn write_frame<T: serde::Serialize>(stdin: &mut ChildStdin, frame: &T) -> Result<(), String> {
    ...
    stdin.flush().map_err(|_| "membrane_hub_pipe_unavailable".into())
}

fn read_lifecycle_frames(stdout: ChildStdout) -> Receiver<Result<ResidentLifecycleFrameV1, String>> {
    let (sender, receiver) = mpsc::channel();
    std::thread::spawn(move || {
        let mut output = BufReader::new(stdout); let mut line = String::new();
        loop {
            line.clear();
            match output.read_line(&mut line) {
                Ok(0) => { let _ = sender.send(Err("membrane_hub_lifecycle_eof".into())); return; }
                Ok(_) => {
                    let frame = serde_json::from_str::<ResidentLifecycleFrameV1>(&line).map_err(|_| "membrane_hub_lifecycle_invalid".into());
                    if sender.send(frame).is_err() { return; }
                }
                Err(_) => { let _ = sender.send(Err("membrane_hub_lifecycle_read_failed".into())); return; }
            }
        }
    });
    receiver
...
fn wait_for_startup(frames: &Receiver<Result<ResidentLifecycleFrameV1, String>>, lease: &ResidentLeaseV1) -> Result<(), String> {
    let deadline = Instant::now() + LIFECYCLE_TIMEOUT;
    let starting = recv_frame(frames, deadline)?;
    if starting.kind != "register" || starting.state.as_deref() != Some("starting") || starting.command.is_some() || starting.endpoint.is_some() || starting.capability.is_some() || starting.fence != lease.fence { return Err("membrane_hub_starting_invalid".into()); }
    let ready = recv_frame(frames, deadline)?;
    let endpoint = ready.endpoint.as_ref();
    if ready.kind != "register" || ready.state.as_deref() != Some("ready") || ready.command.is_some() || ready.fence != lease.fence || ready.capability.as_deref() != Some(&lease.capability) || endpoint.is_none_or(|value| value.host != "127.0.0.1" || value.port < 1024) { return Err("membrane_hub_ready_invalid".into()); }
    Ok(())
}

fn recv_frame(frames: &Receiver<Result<ResidentLifecycleFrameV1, String>>, deadline: Instant) -> Result<ResidentLifecycleFrameV1, String> {
    frames.recv_timeout(deadline.saturating_duration_since(Instant::now()))
        .map_err(|_| "membrane_hub_lifecycle_timeout")?
}

fn sha256_text(value: &str) -> String { format!("sha256:{:x}", Sha256::digest(value.as_bytes())) }
fn sha256_file(path: &Path) -> Result<String, String> { fs::read(path).map(|bytes| format!("sha256:{:x}", Sha256::digest(bytes))).map_err(|_| "membrane_hub_artifact_unavailable".into()) }
fn hex(bytes: &[u8]) -> String { bytes.iter().map(|value| format!("{value:02x}")).collect() }
fn is_sha256_digest(value: &str) -> bool { value.len() == 71 && value.starts_with("sha256:") && value[7..].bytes().all(|byte| byte.is_ascii_digit() || matches!(byte, b'a'..=b'f')) }
